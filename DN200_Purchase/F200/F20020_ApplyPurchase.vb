...
Public Class F20020_ApplyPurchase
    ...
    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.ApplyPurchase_GetByOption(GetFindOtions)
        If msg.IsOk Then
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F20021_ApplyPurchase_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F20021_ApplyPurchase_Msg(FG1.SelectItem("ID"))
        With F
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim R As New R20021_ApplyPurchase_List
...
#Region "审核状态"
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.ApplyPurchase_Audited(_ID, True)
            'msg = Dao.ApplyPurchase_Audited(_ID)
            If msg.IsOk Then
                okCount = okCount + 1
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.ApplyPurchase_SelListById(IsNull(FG1.SelectItem("ID"), ""))
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
            SumFG2.ReSum()
        Else
...
Partial Friend Class Dao
    ...
    '===================申购单信息==============
    Public Const SQL_ApplyPurchase_Get_WithName = "select  P.*,Supplier_No,S.Supplier_Name from T20020_ApplyPurchase_Table P left join T10100_Supplier S on  P.Supplier_ID=S.ID "
    Private Const SQL_ApplyPurchase_OrderBy As String = " order by sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_GetConditionNames() As List(Of FindOption)
        ...
        fo.SQL = "exists(select * from T10001_WL TW,T20021_ApplyPurchase_List TL where %WL_Name% and TL.wl_id=TW.ID and P.id=TL.id)"
        fo.Sign = "%WL_Name%"
        foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_ApplyPurchase_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_ApplyPurchase_OrderBy)
