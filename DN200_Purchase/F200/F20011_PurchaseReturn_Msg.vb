...
Public Class F20011_PurchaseReturn_Msg
    Dim LId As String = ""
    ...
    Sub GetNewID()
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.PurchaseReturn, DTP_sDate.Value, Bill_Id)
                If R.IsOk Then
...
#Region "引用采购单"
    Private Sub Cmd_ChoosePurchase_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_ChoosePurchase.Click
        If (CB_Supplier.IDValue) = 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F20002_Purchase_Sel()
        With F
            .Mode = Mode_Enum.Modify
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_DelByid As String = "Delete from T20010_PurchaseReturn_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_GetByid As String = "select top 1 * from T20010_PurchaseReturn_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_SelByid As String = "select T.*,C.Supplier_Name From (Select top 1 * from T20010_PurchaseReturn_Table where ID=@ID)  T left join T10100_Supplier C on C.ID=T.Supplier_ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_GetStateByid As String = "select top 1 ID,State,State_User from T20010_PurchaseReturn_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_GetListByid As String = "select * from T20011_PurchaseReturn_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_PurchaseReturn_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Unit from (select * from T20011_PurchaseReturn_List  where ID=@ID)L left join T10001_WL WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_CheckID As String = "select count(*) from T20010_PurchaseReturn_Table  where ID=@ID"

    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_CheckIsModify As String = "select count(*) from T20011_PurchaseReturn_List  where ID=@ID and Line=@Line"
#End Region
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> PurchaseReturnId Then
                    DtToUpDate(msg, "Update T20010_PurchaseReturn_Table set id='" & PurchaseReturnId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function PurchaseReturn_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P20010_PurchaseReturn_Audited", paraMap, True)
        If R.IsOk Then
    ...
    Public Shared Function PurchaseReturn_CheckCanReturn(ByVal dtList As DataTable, Optional ByVal isAdd As Boolean = True) As MsgReturn
        ...
        Next
        sqlBuider.Append("'') ")
        sqlBuider.Append("  and  P.State=1 )L left join (Select Purchase_ID, WL_ID, sum(isnull(Qty,0)) as SumRLQty from  T20011_PurchaseReturn_List ")
        '如果是编辑状态,则不计算本退货单数量
        If isAdd = False Then
