...
Public Class F20000_ApplyPurchase
    ...
    Sub FormCheckRight()
        ...
        Control_CheckRight(ID, ClassMain.Audited, Cmd_Confirm)
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F20001_ApplyPurchase_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F20001_ApplyPurchase_Msg(FG1.SelectItem("ID"))
        With F
...
#Region "确认状态"
    ...
    Private Sub Cmd_Confirm_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Confirm.Click
        ...
        If idList.Count > 0 Then
            ShowConfirm("是否要确认申购单? ", AddressOf Confirm)
        Else
            ShowErrMsg("您没有选择申购单!")
        End If
    End Sub
    Protected Sub Confirm()
        ...
        For Each _ID As String In idList
            msg = Dao.Purchase_IfConfirm(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
            ...
            End If
        Next
        msgBudder.Insert(0, "申购单批量确认,共" & okCount & "张单确认成功," & failedCount & "张单确认失败!")
...
#Region "FG1事件"
    ...
    Private Sub FG1_SelectRowChange(ByVal Row As Integer) Handles FG1.SelectRowChange
        If FG1.SelectItem Is Nothing OrElse IsNull(FG1.SelectItem("ID"), "") = "" Then
            Exit Sub
        End If
        If IsNull(FG1.SelectItem("State"), Enum_State_Upgrade.AddNew) = Enum_State_Upgrade.AddNew Then
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.Purchase_SelListById(IsNull(FG1.SelectItem("ID"), ""))
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
            SumFG2.ReSum()
        Else
...
#End Region
    ...
    Private Sub Cmd_Payment_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Payment.Click
        ...
        If Pmlist.Count > 0 Then
            ShowConfirm("是否要将选定申购单设为已付款? ", AddressOf Payment)
        Else
            ShowErrMsg("您没有选择申购单!")
        End If
    End Sub
    ...
    Protected Sub Payment()
        ...
        For Each _ID As String In Pmlist
            msg = Dao.Purchase_Payment(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
            Else
                failedCount = failedCount + 1
            ...
            End If
        Next
        msgBudder.Insert(0, "申购单批量付款,共" & okCount & "张单付款成功," & failedCount & "张单付款失败!")
        If failedCount > 0 Then
            ShowErrMsg(msgBudder.ToString)
        Else
            ShowOk(msgBudder.ToString, False)
...
Partial Friend Class Dao
    ...
    '===================申购单信息==============
    'Public Const SQL_Purchase_Get_WithName = "select  P.*,Supplier_No,S.Supplier_Name from T20000_Puchase_Table P left join T10100_Supplier S on  P.Supplier_ID=S.ID "
    Public Const SQL_Purchase_Get_WithName = "select  P.* from T20000_Puchase_Table P  "
    Private Const SQL_Purchase_OrderBy As String = " order by sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Purchase_GetConditionNames() As List(Of FindOption)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Purchase_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    Public Shared Function Purchase_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        ...
        sqlBuider.Append(SQL_Purchase_Get_WithName)
