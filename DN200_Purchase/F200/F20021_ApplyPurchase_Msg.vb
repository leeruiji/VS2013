...
Public Class F20021_ApplyPurchase_Msg
    Dim LId As String = ""
    ...
    Protected Sub Me_Refresh()
        Dim msgTable As DtReturnMsg = Dao.ApplyPurchase_GetById(Bill_ID)
        If msgTable.IsOk Then
            ...
            If Mode = Mode_Enum.Modify AndAlso msgTable.Dt.Rows.Count = 0 Then
                ShowErrMsg("没有找到" & Dao.ApplyPurchase_DB_NAME & "[" & Bill_ID & "]", True)
                Exit Sub
            End If
            SetForm(dtTable)
        End If
        Select Case Mode
            Case Mode_Enum.Add
                If CheckRight(ID, ClassMain.Add) = False Then Me.ShowErrMsg("没有权限添加" & Dao.ApplyPurchase_DB_NAME & "!", True) : Exit Sub
                Dim msg As DtReturnMsg = Dao.ApplyPurchase_SelListById(0)
            ...
            Case Mode_Enum.Modify
                ...
                Dim msg As DtReturnMsg = Dao.ApplyPurchase_SelListById(Bill_ID)
                If msg.IsOk Then
                    dtList = msg.Dt
...
#Region "工具栏按钮"
    ...
    Protected Sub SavePurchase(Optional ByVal Audit As Boolean = False)
        ...
        If Me.Mode = Mode_Enum.Add Then
            R = Dao.ApplyPurchase_Add(Dt, dtList)
        Else
            R = Dao.ApplyPurchase_Save(LId, Dt, dtList)
        End If
        If R.IsOk Then
            LId = TB_ID.Text
            ...
            If Audit Then
                Dim msg As MsgReturn = Dao.ApplyPurchase_Audited(TB_ID.Text, True)
                'Dim msg As MsgReturn = Dao.ApplyPurchase_Audited(TB_ID.Text)
                If msg.IsOk Then
                    Bill_ID = TB_ID.Text
                    Me_Refresh()
                    ShowOk(msg.Msg)
...
#Region "审核状态"
    ...
    Protected Sub UnAudit()
        Dim msg As MsgReturn = Dao.ApplyPurchase_Audited(TB_ID.Text, False)
        'Dim msg As MsgReturn = Dao.ApplyPurchase_UnAudit(TB_ID.Text)
        If msg.IsOk Then
            Me_Refresh()
...
#Region "常量"
    Protected Friend Const ApplyPurchase_DB_NAME As String = "申购单"
    ''' <summary>
    ''' 删除单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_DelByid As String = "Delete from T20020_ApplyPurchase_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 审核单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_Audited As String = "update T20020_ApplyPurchase_Table set State=1  where ID=@ID "
    ''' <summary>
    ''' 反审单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_UnAudit As String = "update T20020_ApplyPurchase_Table set State=0  where ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_GetByid As String = "select top 1 * from T20020_ApplyPurchase_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_SelByid As String = "select T.*,C.Supplier_Name From (Select top 1 * from T20020_ApplyPurchase_Table where ID=@ID)  T left join T10100_Supplier C on C.ID=T.Supplier_ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_GetStateByid As String = "select top 1 ID,State,State_User from T20020_ApplyPurchase_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_GetListByid As String = "select * from T20021_ApplyPurchase_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_ApplyPurchase_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Unit from (select * from T20021_ApplyPurchase_List  where ID=@ID)L left join T10001_WL WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_CheckID As String = "select count(*) from T20020_ApplyPurchase_Table  where ID=@ID"
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ApplyPurchase_CheckIsModify As String = "select count(*) from T20021_ApplyPurchase_List  where @ID=ID and Line=@Line"
#End Region
...
#Region "单一张单内容查询"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_ApplyPurchase_GetByid, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_SelListById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_ApplyPurchase_SelListByid, "@ID", sId)
    End Function
#End Region
...
#Region "添加修改删除"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_CheckIsModify(ByVal ID As String, ByVal Line As Integer) As Boolean
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", ID)
        P.Add("Line", Line)
        Dim R As MsgReturn = SqlStrToOneStr(SQL_ApplyPurchase_CheckIsModify, P)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return True
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_CheckID(ByVal ID As String) As Boolean
        Dim R As MsgReturn = SqlStrToOneStr(SQL_ApplyPurchase_CheckID, "@ID", ID)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return False
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        ...
        Try
            sqlMap.Add("Table", SQL_ApplyPurchase_GetByid)
            sqlMap.Add("List", SQL_ApplyPurchase_GetListByid)
            ...
            If msg.DtList("Table").Rows.Count = 0 Then
                ...
                Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & PurchaseId & "'," & BillType.ApplyPurchase & ")"
                DtToUpDate(msg, TmSQL)
                R.Msg = "" & ApplyPurchase_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & ApplyPurchase_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & ApplyPurchase_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim PurchaseId As String = dtTable.Rows(0)("ID")
        If LID <> PurchaseId AndAlso ApplyPurchase_CheckID(PurchaseId) Then
            R.IsOk = False
            R.Msg = "" & ApplyPurchase_DB_NAME & "[" & PurchaseId & "]已经存在!请双击编号文本框,获取新编号!"
            Return R
        End If
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Audited Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & ApplyPurchase_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被审核不能再修改!"
                    Return R
                End If
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Invoid Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & ApplyPurchase_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
                    Return R
                End If
                DvToDt(dtList, msg.DtList("List"), New List(Of String))
                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & PurchaseId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.ApplyPurchase
                If LID <> PurchaseId Then
                    DtToUpDate(msg, "Update T20020_ApplyPurchase_Table set id='" & PurchaseId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
                    dtTable.Rows(0)("ID") = PurchaseId
                    R.Msg = "" & ApplyPurchase_DB_NAME & "[" & LID & "]已经修改为[" & PurchaseId & "]!"
                Else
                    DtToUpDate(msg)
                    R.Msg = "" & ApplyPurchase_DB_NAME & "[" & LID & "]修改成功!"
                End If
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & ApplyPurchase_DB_NAME & "[" & LID & "]不存在"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & ApplyPurchase_DB_NAME & "[" & LID & "]修改错误"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_Del(ByVal PurchaseId As String)
        Return RunSQL(SQL_ApplyPurchase_DelByid, "@ID", PurchaseId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        ...
        paraMap.Add("ID", _ID)
        Try
            sqlMap.Add("Table", SQL_ApplyPurchase_GetStateByid)
            msg = SqlStrToDt(sqlMap, paraMap)
            If msg.DtList("Table").Rows.Count = 1 Then
                If msg.DtList("Table").Rows(0)("State") <> Enum_State.Audited Then
                    If msg.DtList("Table").Rows(0)("State") <> State Then
                        ...
                        DtToUpDate(msg)
                        R.IsOk = True
                        R.Msg = "" & ApplyPurchase_DB_NAME & "[" & _ID & "]" & OStr & "成功!"
                    Else
                        R.IsOk = False
                        R.Msg = "" & ApplyPurchase_DB_NAME & "[" & _ID & "]已经被" & OStr & "!不能再" & OStr & "!"
                    End If
                Else
                    R.IsOk = False
                    R.Msg = "" & ApplyPurchase_DB_NAME & "[" & _ID & "]已经被审核,不能" & OStr & "!"
                End If
            Else
                R.IsOk = False
                R.Msg = "" & ApplyPurchase_DB_NAME & "[" & _ID & "]不存在,不能" & OStr & "!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & ApplyPurchase_DB_NAME & "[" & _ID & "]" & OStr & "错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    'Public Shared Function ApplyPurchase_Audited(ByVal PurchaseId As String)
    '    Return RunSQL(SQL_ApplyPurchase_Audited, "@ID", PurchaseId)
    'End Function

    'Public Shared Function ApplyPurchase_UnAudit(ByVal PurchaseId As String)
    '    Return RunSQL(SQL_ApplyPurchase_UnAudit, "@ID", PurchaseId)
    'End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ApplyPurchase_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("ID", _ID)
        paraMap.Add("IsAudited", IsAudited)
        paraMap.Add("State_User", User_Display)
        Dim R As MsgReturn = SqlStrToOneStr("P20020_ApplyPuchase_Audited", paraMap, True)
        If R.IsOk Then
