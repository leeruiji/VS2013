...
Public Class F20330_SGLingLiao
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        TSD_Audit.Value = BaseClass.ComFun.Autitedtime
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.LingLiao_GetConditionNames()
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.LingLiao_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.LingLiao_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub

    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.SGLingLiao_GetByOption(GetFindOtions)
        If msg.IsOk Then
...
#Region "控件事件"
    ...
    Private Sub Edit_Retrun()
        If ReturnObj Is Nothing Then
            ...
            End If
        Else
            Dim ot As List(Of String) = TryCast(ReturnObj, List(Of String))
            For i As Integer = 0 To ot.Count - 1
                ReturnId = ot(i)
                If ReturnId <> "" Then
                    SelectRetrun = True
                    Me_Refresh()
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F20331_SGLingLiao_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的" & Dao.SGLingLiao_DB_NAME)
            Exit Sub
        End If
        Dim F As New F20331_SGLingLiao_Msg(FG1.Item(FG1.RowSel, "ID"))
        With F
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Dao.SGLingLiao_DB_NAME & "[" & FG1.Item(FG1.RowSel, "ID") & "]?", AddressOf DelLingLiao)
    End Sub
    ...
    Protected Sub DelLingLiao()
        Dim msg As MsgReturn = Dao.SGLingLiao_Del(FG1.Item(FG1.RowSel, "ID"))
        If msg.IsOk Then
...
#Region "搜索工具栏"
    ...
    Private Sub CB_ConditionName2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_ConditionName2.SelectedIndexChanged
        ...
        Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_LingLiao_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & " and Type=" & BType & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
        If msg.IsOk Then
    ...
    Private Sub CB_Condition3_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles CB_Condition3.SelectedIndexChanged
        ...
        End If
    End Sub
    Private Sub TSD_Audit_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TSD_Audit.ValueChanged
        If MeIsLoad Then BaseClass.ComFun.Autitedtime = TSD_Audit.Value.Date
    End Sub
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType, Optional ByVal isFirst As Boolean = True)
        ...
        If check < 2 Then
            ...
            If CheckRight(ID, ClassMain.PrintMore) Then
                Dim Rr As New R20330_SGLingLiao
                Rr.Start(FG1.Item(FG1.RowSel, "ID"), DoOperator, isFirst)
            Else
                Dim dtPrint As DtReturnMsg = Dao.SGGetPrintState(FG1.Item(FG1.RowSel, "ID"))
        ...
        Dim R As New R20331_SGLingLiao
        R.Start(Get_id(), FG1.Item(FG1.RowSel, "ID"), DoOperator, isFirst)
    End Sub
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.SGLingLiao_SelListById(IsNull(FG1.Item(FG1.RowSel, "ID"), ""))
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
            SumFG2.ReSum()
        Else
...
Partial Friend Class Dao
    ...
    '===================出库单信息==============
    Public Const SQL_SGLingLiao_Get_WithName = "select T.*,C.Client_Name,BZ_Name from T20330_SGLingLiao_Table T left join T10110_Client C on T.Client_ID=C.ID  left join T10002_BZ BZ on BZ.ID=T.BZ_ID "

    Public Const SQL_SGLingLiao_OrderBy As String = " order by sDate "

    Public Const SQL_SGLingLiao_Get_ID As String = "Select * from  T20330_SGLingLiao_Table  where Produce_ID=@Produce_ID and IsJiaLiao=0 and  State=0  "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_GetConditionNames() As List(Of FindOption)
        ...
        fo.SQL = "exists(select * from T10001_WL TW,T20331_SGLingLiao_List TL where %WL_Name% and TL.wl_id=TW.ID and t.id=TL.id)"
        fo.Sign = "%WL_Name%"
        foList.Add(fo)
        ...
        fo.DB_Field = " "
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "(T.ID %Like% or  SumQty %Like% or SumAmount %Like% or Remark %Like%  or Produce_ID %Like% )"
        fo.Sign = "%Like%"
        foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_SGLingLiao_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(" and Type=20330 ")
        sqlBuider.Append(SQL_SGLingLiao_OrderBy)
    ...
    ''' <remarks></remarks>
    ''' 
    Public Shared Function SGLingLiag_GetID_ByGH(ByVal Produce_ID As String) As DtReturnMsg
        Dim R As DtReturnMsg = SqlStrToDt(SQL_SGLingLiao_Get_ID, "@Produce_ID", Produce_ID)
        Return R
    End Function
