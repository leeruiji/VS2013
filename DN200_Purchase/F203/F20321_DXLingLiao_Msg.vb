...
Public Class F20321_DXLingLiao_Msg
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        IsClosing = True
        ComFun.DelBillNewID(BillType.DXLingLiao, Bill_ID)
    End Sub
    ...
    Private Sub Form_Load() Handles Me.Me_Load
        ...
        If msgReason.IsOk Then
            Dim Row As DataRow = msgReason.Dt.NewRow
            Row("Remark") = "定型领料"
            msgReason.Dt.Rows.InsertAt(Row, 0)
...
#Region "表单信息"
    ...
    Protected Function GetForm() As DataTable
        ...
        dr("Temperature") = Val(TB_Temperature.Text)
        dr("Pressure") = Val(TB_Pressure.Text)
        dr("Type") = BType
    ...
    Protected Sub SetForm(ByVal dt As DataTable)
        ...
        If IsBidings = False Then
            ...
            TB_Temperature.DataBindings.Add("Text", dtTable, TB_Temperature.Name.Substring(3), False, DataSourceUpdateMode.OnPropertyChanged, 0)
            TB_Pressure.DataBindings.Add("Text", dtTable, TB_Pressure.Name.Substring(3), False, DataSourceUpdateMode.OnPropertyChanged, 0)

            IsBidings = True
        End If
        SetPrintMsg(Dr)
        LabelGY1.Tag = IsNull(dtTable.Rows(0)("GY_ID1"), 0)
        GetDXGY(LabelGY1.Tag)
        ...
        If IsJiaLiao = False Then
            JL_Qty = Dao.LingLiaoDX_CountJiaLiao(TB_Produce_ID.Text)
...
#Region "工具栏按钮"
    ...
    Private Sub Cmd_Modify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Modify.Click
        ...
        If CheckForm() Then
            If Mode = Mode_Enum.Add Then
                ShowConfirm("是否保存新" & Ch_Name & "?", AddressOf SaveDXLingLiao)
            Else
                If CheckIsModify() Then
                    ShowConfirm(Ch_Name & "[" & TB_ID.Text & "] 已经被其他人修改过,是否覆盖别人的修改?", AddressOf SaveDXLingLiao)
                Else
                    ShowConfirm("是否保存" & Ch_Name & "[" & TB_ID.Text & "] 的修改?", AddressOf SaveDXLingLiao)
                End If
            End If
        End If
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SaveDXLingLiao(Optional ByVal Audit As Boolean = False)
        Dim R As MsgReturn
        Dim Dt As DataTable = GetForm()
        '     Dt.Rows(0).Item("DXLingLiaoType") = Me.DXLingLiaoType
        If Me.Mode = Mode_Enum.Add Then
    ...
    Protected Function CheckForm() As Boolean
        ...
        '    ShowErrMsg("布种不能为空!", AddressOf CB_BZC.Focus)
        '    Return False
        'End If
    ...
    Sub GetNewID()
        ...
        If Mode = Mode_Enum.Add Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.DXLingLiao, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType, Optional ByVal isFirst As Boolean = True)
        ...
        If CheckRight(ID, ClassMain.PrintMore) Then
            Dim R As New R20320_DXLingLiao
...
#Region "审核状态"
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If JL_Qty > 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf DelDXLingLiao)
    End Sub
    ...
    Private Sub Cmd_SetInValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetInValid.Click
        If JL_Qty > 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf DXLingLiaoInValid)
    End Sub


    Sub DXLingLiaoInValid()
        Dao.DXLingLiao_DB_NAME = Ch_Name
    ...
    Private Sub Cmd_SetValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetValid.Click
        ShowConfirm("是否反作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf DXLingLiaoValid)
    End Sub
...
#Region "比例计算,按水位计算"
    ...
    Private Sub TB_BZ_ZL_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TB_BZ_ZL.LostFocus
        ...
        'CaculateSumAmount()
    End Sub
    Private Sub Tb_WaterLevel_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Tb_WaterLevel.LostFocus
        CacQty_ALL()
        CaculateSumAmount()
    End Sub
    ...
    Private Sub CacQty_ALL(Optional ByVal MustQty As Boolean = False)
        If MustQty OrElse Tb_WaterLevel.Tag Is Nothing OrElse Val(Tb_WaterLevel.Tag) <> Val(Tb_WaterLevel.Text) Then
            ...
            Next
            Tb_WaterLevel.Tag = Tb_WaterLevel.Text
            'CacWaterLevel()
        End If
    End Sub
    ...
    Private Sub CacQty(ByVal rowIndex As Integer)
        ...
        'Else
        '助剂数量=配布重*比例*浴比
        FG1.Item(rowIndex, "Qty") = Val(Tb_WaterLevel.Text) * Val(FG1.Item(rowIndex, "sPercent")) * 10 / I_isKg
        'End If
    ...
    Private Sub CacPercent(ByVal rowIndex As Integer)
        If Val(Tb_WaterLevel.Text) > 0 Then


            If FG1.Item(rowIndex, "WL_Unit_LL") = "g" Then
                ' FG1.Item(FG1.RowSel, "Qty") = Val(TB_BZ_ZL.Text) * 10 * Val(FG1.Item(FG1.RowSel, "sPercent"))
                FG1.Item(rowIndex, "sPercent") = Format(Val(FG1.Item(rowIndex, "Qty")) / (Val(Tb_WaterLevel.Text) * 10), FG1.Cols("sPercent").Format)
            ElseIf FG1.Item(rowIndex, "WL_Unit_LL") = "Kg" Then
                'FG1.Item(FG1.RowSel, "Qty") = Val(TB_BZ_ZL.Text) * 10 * Val(FG1.Item(FG1.RowSel, "sPercent")) / 1000

                FG1.Item(rowIndex, "sPercent") = Format((Val(FG1.Item(rowIndex, "Qty")) / (Val(Tb_WaterLevel.Text) * 10) * 1000), FG1.Cols("sPercent").Format)
            End If
        End If
    End Sub
...
#Region "根据色号，布种获取染部配方"
    Private Sub GetRBPF()
        'If TB_BZ_ID.Tag IsNot Nothing AndAlso TB_BZC_ID.Tag IsNot Nothing Then
        '    Dim msg As DtReturnMsg = Dao.LingLiao_GetList_ByBZC(Val(TB_BZC_ID.Tag), Val(TB_BZ_ID.Tag))
        '    If msg.IsOk Then
        ...
        '        Else
        '            Dim R As DtReturnMsg = Dao.LingLiao_GetGYID_ByBZC(Val(TB_BZC_ID.Tag), msg.Dt.Rows(0)("ID"))
...
#Region "获取缸号资料"
    ...
    Private Sub TB_Produce_ID_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TB_Produce_ID.KeyPress
        If TB_Produce_ID.ReadOnly = False AndAlso e.KeyChar = vbCr Then
            ...
            Dim Rt As MsgReturn = Dao.LingLiaoDX_Count(s)
    ...
    Sub LoadFromGH(ByVal s As String)
        ...
        If R.IsOk AndAlso R.Dt.Rows.Count > 0 Then
            Try
                If IsNull(R.Dt.Rows(0)("IsFD"), False) Then
                    ...
                    If R.Dt.Rows(0)("State") >= Enum_ProduceState.PeiBu Then
                        ...
                        CanLingliao = True
                    Else
                        ShowErrMsg("缸号[" & s & "]未配布,不能开领料单!", Tb_WaterLevel.Focus)
                        CanLingliao = False
                    End If
                ...
                Else
                    ...
                    If R.Dt.Rows(0)("State") >= Enum_ProduceState.PeiBu Then
                        ...
                        CanLingliao = True
                    Else
                        ShowErrMsg("缸号[" & s & "]未配布,不能开领料单!", AddressOf Tb_WaterLevel.Focus)
                        CanLingliao = False
                    End If
                End If
...
#Region "添加工艺"
    ...
    Private Function SelectGY() As ViewForm
        Dim F As BaseForm = LoadFormIDToChild(10335, Me)
    ...
    Protected Sub SetFGGY(ByVal StartRow As Integer)
        Try
            ...
            End If
            Dim dr As DataRow = ReturnObj
            Dim msg As DtReturnMsg = ComFun.DXGY_GetList_ByID(IsNull(dr("ID"), 0))
            If msg.Dt.Rows.Count = 0 Then
    ...
    Sub GetDXGY(ByVal ID As Integer)
        Dim R As DtReturnMsg = ComFun.DXGY_GetById(ID)
        If R.IsOk AndAlso R.Dt.Rows.Count > 0 Then
            LabelGY1.Text = IsNull(R.Dt.Rows(0)("DXGY_Name"), "") & ",机速:" & IsNull(R.Dt.Rows(0)("DXGY_JS"), "") & ",温度:" & IsNull(R.Dt.Rows(0)("DXGY_WD"), "")
        End If
    End Sub
...
#Region "加料"
    ...
    Private Sub Cmd_JiaLiao_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_JiaLiao.Click
        ...
        CopyMe(True)
        LoadFromGH(TB_Produce_ID.Text)
        'Dim F As New F20321_DXLingLiao_Msg(Bill_Id, True)
        'With F
        '    .Mode = Mode_Enum.Add
        '    .P_F_RS_ID = ""
        '    .P_F_RS_ID2 = ""
    ...
    Private Sub Cmd_ShowJiaLiao_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_ShowJiaLiao.Click
        If Mode = Mode_Enum.Add Then
            ...
            Exit Sub
        End If
        Dim F As New F20322_DXJiaLiao_Sel(TB_Produce_ID.Text)
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
...
#Region "双击单号"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub LB_Note_MouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles LB_Note.MouseDoubleClick
        If IsJiaLiao Then
            Dim F As New F20321_DXLingLiao_Msg(MainBill)
            With F
                .Mode = Mode_Enum.Modify
                .P_F_RS_ID = ""
                .P_F_RS_ID2 = ""
...
#End Region
    ...
    Private Sub LabelGY1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles LabelGY1.Click
    ...
    Private Sub Tb_WaterLevel_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles Tb_WaterLevel.KeyPress
        If e.KeyChar = vbCr Then
            TB_Temperature.Focus()
        End If
    End Sub

    Private Sub TB_Temperature_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TB_Temperature.KeyPress
        If e.KeyChar = vbCr Then
            TB_Pressure.Focus()
        End If
    End Sub

    Private Sub TB_BZ_Qty_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TB_BZ_Qty.KeyPress
        If e.KeyChar = vbCr Then
            Tb_WaterLevel.Focus()
        End If
    End Sub
...
Partial Friend Class Dao
    Protected Friend Shared DXLingLiao_DB_NAME As String = "定型领料单"

    Public Shared Function LingLiao_GetZL(ByVal GH As String) As Double
        Return Val(SqlStrToOneStr("SELECT TOP 1 BZ_ZL FROM T20310_LingLiao_Table WHERE (Produce_ID = @GH) AND (IsJiaLiao = 0) AND (Type = '20320')", "GH", GH))
    End Function
    '#Region "常量"
    ...
    '    ''' </summary>
    '    ''' <remarks></remarks>
    '    Private Const SQL_DXLingLiao_DelByid As String = "Delete from T20310_LingLiao_Table  where ID=@ID and State=0"
    '    ''' <summary>
    '    ''' 获取单头
    '    ''' </summary>
    '    ''' <remarks></remarks>
    '    Private Const SQL_DXLingLiao_GetByid As String = "select top 1 * from T20310_LingLiao_Table  where ID=@ID "

    '    ''' <summary>
    '    ''' 获取单头
    '    ''' </summary>
    '    ''' <remarks></remarks>
    '    Private Const SQL_DXLingLiao_GetByid_WithName As String = "select top 1T. * ,C.Client_Name,(Select top 1 GY_No + '.' + GY_Name from  T10004_GY where T10004_GY.ID=T.GY_ID1) as GY_Name1,(Select top 1 GY_No + '.' + GY_Name from  T10004_GY where T10004_GY.ID=T.GY_ID2) as GY_Name2 from T20310_LingLiao_Table T Left join T10110_Client C on T.Client_ID=C.ID  where T.ID=@ID "
    '    ''' <summary>
    '    ''' 获取单头
    '    ''' </summary>
    '    ''' <remarks></remarks>
    '    Private Const SQL_DXLingLiao_SelByid As String = " select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name,BZ.BZ_Name ,C.Client_Name ,C.Client_No" & _
    ...
    '    ''' </summary>
    '    ''' <remarks></remarks>
    '    Private Const SQL_DXLingLiao_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T20310_LingLiao_Table  where ID=@ID"
    '    ''' <summary>
    '    ''' 获取单体
    '    ''' </summary>
    '    ''' <remarks></remarks>
    '    Private Const SQL_DXLingLiao_GetListByid As String = "select * from T20311_LingLiao_List  where ID=@ID "
    '    ''' <summary>
    '    ''' 获取单体,用于显示
    '    ''' </summary>
    '    ''' <remarks></remarks>0
    '    Private Const SQL_DXLingLiao_SelListByid As String = "select L.*,L.WL_Unit_LL as WL_Unit_LL2, WL.WL_No,WL.WL_Name,WL.WL_Spec ,WL.WL_Percent,WL.WL_Cost from (select * from T20311_LingLiao_List  where ID=@ID)L left join T10001_WL WL on WL.id=l.WL_ID order by L.ID ,L.Line "
    '    ''' <summary>
    '    ''' 检查ID的重复性
    '    ''' </summary>
    '    ''' <remarks></remarks>
    '    Private Const SQL_DXLingLiao_CheckID As String = "select count(*) from T20310_LingLiao_Table  where ID=@ID"
    ...
    '    ''' </summary>
    '    ''' <remarks></remarks>
    '    Private Const SQL_DXLingLiao_CheckIsModify As String = "select count(*) from T20311_LingLiao_List  where ID=@ID and Line=@Line"
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_GetById(ByVal sId As String) As DtReturnMsg
    '        Return PClass.PClass.SqlStrToDt(SQL_DXLingLiao_GetByid_WithName, "@ID", sId)
    '    End Function
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_SelListById(ByVal sId As String) As DtReturnMsg
    '        Return PClass.PClass.SqlStrToDt(SQL_DXLingLiao_SelListByid, "@ID", sId)
    '    End Function
    '#End Region
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_CheckIsModify(ByVal ID As String, ByVal Line As Integer) As Boolean
    '        Dim P As New Dictionary(Of String, Object)
    '        P.Add("ID", ID)
    '        P.Add("Line", Line)
    '        Dim R As MsgReturn = SqlStrToOneStr(SQL_DXLingLiao_CheckIsModify, P)
    '        If R.IsOk Then
    '            If Val(R.Msg) = 0 Then
    '                Return True
    '            Else
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_CheckID(ByVal ID As String) As Boolean
    '        Dim R As MsgReturn = SqlStrToOneStr(SQL_DXLingLiao_CheckID, "@ID", ID)
    '        If R.IsOk Then
    '            If Val(R.Msg) = 0 Then
    '                Return False
    '            Else
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
    '        Dim msg As New DtListReturnMsg
    '        Dim R As New MsgReturn
    '        Dim sqlMap As New Dictionary(Of String, String)
    '        Dim paraMap As New Dictionary(Of String, Object)
    '        Dim DXLingLiaoId As String = dtTable.Rows(0)("ID")
    '        Dim IsInsert As Boolean = False
    '        paraMap.Add("ID", DXLingLiaoId)
    '        Try
    ...
    '            '        R.IsOk = False
    '            '        R.Msg = DXLingLiao_DB_NAME & "[" & DXLingLiaoId & "]添加失败!" & vbCrLf & Rx.Msg
    '            '        Return R
    '            '    End If
    '            'End If

    '            sqlMap.Add("Table", SQL_DXLingLiao_GetByid)
    '            sqlMap.Add("List", SQL_DXLingLiao_GetListByid)
    ...
    '                Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & DXLingLiaoId & "'," & BillType.DXLingLiao & ")"
    '                DtToUpDate(msg, TmSQL)
    '                R.Msg = "" & DXLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
    '                R.IsOk = True
    '            Else
    '                R.IsOk = False
    '                R.Msg = "" & DXLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
    '            End If
    '            Return R
    '        Catch ex As Exception
    '            R.IsOk = False
    '            R.Msg = "" & DXLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
    '            DebugToLog(ex)
    '            Return R
    '        Finally
    '        End Try
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
    '        Dim msg As New DtListReturnMsg
    '        Dim R As New MsgReturn
    '        Dim sqlMap As New Dictionary(Of String, String)
    '        Dim paraMap As New Dictionary(Of String, Object)
    '        Dim DXLingLiaoId As String = dtTable.Rows(0)("ID")
    '        If LID <> DXLingLiaoId AndAlso DXLingLiao_CheckID(DXLingLiaoId) Then
    '            R.IsOk = False
    '            R.Msg = "" & DXLingLiao_DB_NAME & "[" & DXLingLiaoId & "]已经存在!请双击编号文本框,获取新编号!"
    '            Return R
    '        End If
    ...
    '                    msg.Cnn.Dispose()
    '                    R.IsOk = False
    '                    R.Msg = "" & DXLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被审核不能再修改!"
    '                    Return R
    '                End If
    ...
    '                    msg.Cnn.Dispose()
    '                    R.IsOk = False
    '                    R.Msg = "" & DXLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
    '                    Return R
    '                End If
    '                DvToDt(dtList, msg.DtList("List"), New List(Of String))
    '                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
    '                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & DXLingLiaoId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.DXLingLiao
    '                If LID <> DXLingLiaoId Then
    '                    DtToUpDate(msg, "Update T20310_LingLiao_Table set id='" & DXLingLiaoId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    '                    dtTable.Rows(0)("ID") = DXLingLiaoId
    '                    R.Msg = "" & DXLingLiao_DB_NAME & "[" & LID & "]已经修改为[" & DXLingLiaoId & "]!"
    '                Else
    '                    DtToUpDate(msg)
    '                    R.Msg = "" & DXLingLiao_DB_NAME & "[" & LID & "]修改成功!"
    '                End If
    '                R.IsOk = True
    '            Else
    '                R.IsOk = False
    '                R.Msg = "" & DXLingLiao_DB_NAME & "[" & LID & "]不存在"
    '            End If
    '            Return R
    '        Catch ex As Exception
    '            R.IsOk = False
    '            R.Msg = "" & DXLingLiao_DB_NAME & "[" & LID & "]修改错误"
    '            DebugToLog(ex)
    '            Return R
    '        Finally
    '        End Try
    ...
    '    ''' 删除出库单
    '    ''' </summary>
    '    ''' <param name="DXLingLiaoId"></param>
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_Del(ByVal DXLingLiaoId As String)
    '        Return RunSQL(SQL_DXLingLiao_DelByid, "@ID", DXLingLiaoId)
    '    End Function
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
    '        Dim msg As New DtListReturnMsg
    '        Dim R As New MsgReturn
    '        Dim sqlMap As New Dictionary(Of String, String)
    '        Dim paraMap As New Dictionary(Of String, Object)
    ...
    '        paraMap.Add("ID", _ID)
    '        Try
    '            sqlMap.Add("Table", SQL_DXLingLiao_GetStateByid)
    '            msg = SqlStrToDt(sqlMap, paraMap)
    '            If msg.DtList("Table").Rows.Count = 1 Then
    '                If msg.DtList("Table").Rows(0)("State") <> Enum_State.Audited Then
    '                    '检查加工单状态
    ...
    '                    '    If Rx.IsOk = False Then
    '                    '        R.IsOk = False
    '                    '        R.Msg = DXLingLiao_DB_NAME & "[" & _ID & "]" & OStr & "失败!" & vbCrLf & Rx.Msg.Replace("不能被引用", "")
    '                    '        Return R
    '                    '    End If
    '                    'End If
    ...
    '                        DtToUpDate(msg)
    '                        R.IsOk = True
    '                        R.Msg = "" & DXLingLiao_DB_NAME & "[" & _ID & "]" & OStr & "成功!"
    '                    Else
    '                        R.IsOk = False
    '                        R.Msg = "" & DXLingLiao_DB_NAME & "[" & _ID & "]已经被" & OStr & "!不能再" & OStr & "!"
    '                    End If
    '                Else
    '                    R.IsOk = False
    '                    R.Msg = "" & DXLingLiao_DB_NAME & "[" & _ID & "]已经被审核,不能" & OStr & "!"
    '                End If
    '            Else
    '                R.IsOk = False
    '                R.Msg = "" & DXLingLiao_DB_NAME & "[" & _ID & "]不存在,不能" & OStr & "!"
    '            End If
    '            Return R
    '        Catch ex As Exception
    '            R.IsOk = False
    '            R.Msg = "" & DXLingLiao_DB_NAME & "[" & _ID & "]" & OStr & "错误!"
    '            DebugToLog(ex)
    '            Return R
    '        Finally
    '        End Try
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
    '        Dim paraMap As New Dictionary(Of String, Object)
    '        paraMap.Add("ID", _ID)
    '        paraMap.Add("IsAudited", IsAudited)
    '        paraMap.Add("State_User", User_Display)

    '        Dim R As MsgReturn = SqlStrToOneStr("P20310_DXLingLiao_Audited", paraMap, True)
    '        If R.IsOk Then
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_GetList_ByBZC(ByVal BZC_ID As Integer, ByVal BZ_ID As Integer) As DtReturnMsg
    '        Dim P As New Dictionary(Of String, Object)
    '        P.Add("BZC_ID", BZC_ID)
    '        P.Add("BZ_ID", BZ_ID)
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_GetGYID_ByBZC(ByVal BZC_ID As Integer, ByVal ID As Integer) As DtReturnMsg
    '        Dim P As New Dictionary(Of String, Object)
    '        P.Add("BZC_ID", BZC_ID)
    '        P.Add("ID", ID)
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_GetGH(ByVal Produce_ID As String) As DtReturnMsg
    '        Dim P As New Dictionary(Of String, Object)
    '        P.Add("GH", Produce_ID)
    '        Return SqlStrToDt("select top 1 GH,State,BZC_ID,BZ_ID,Client_ID,PB_ZLSum,PB_CountSum from T30000_Produce_Gd where GH=@GH", P)
    '    End Function
    '#End Region
    ...
    '    ''' <returns></returns>
    '    ''' <remarks></remarks>
    '    Public Shared Function DXLingLiao_CountJiaLiao(ByVal _ID As String) As Integer
    '        Dim R As MsgReturn = SqlStrToOneStr("select Count(*) from T20310_LingLiao_Table where MainBill=@ID and IsJiaLiao=1 ", "ID", _ID)
    '        Return Val(R.Msg)
    '    End Function
End Class