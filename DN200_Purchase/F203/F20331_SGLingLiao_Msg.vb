...
Public Class F20331_SGLingLiao_Msg
    Dim BType As BillType = BillType.SGLingLiao
    Dim LId As String = ""
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        IsClosing = True
        ComFun.DelBillNewID(BillType.SGLingLiao, Bill_ID)
    End Sub

    Private Sub Form_Load() Handles Me.Me_Load
        FormCheckRight()
        Ch_Name = GetBillTypeName(ID)
        Dao.SGLingLiao_DB_NAME = Ch_Name
        Me.Title = Ch_Name
    ...
    Protected Sub Me_Refresh()
        Dim msgTable As DtReturnMsg = Dao.SGLingLiao_GetById(Bill_ID)
        ...
        Select Case Mode
            Case Mode_Enum.Add
                ...
                Dim msg As DtReturnMsg = Dao.SGLingLiao_SelListById(0)
            ...
            Case Mode_Enum.Modify
                ...
                Dim msg As DtReturnMsg = Dao.SGLingLiao_SelListById(Bill_ID)
...
#Region "表单信息"
    ...
    Protected Function GetForm() As DataTable
        ...
        '获取色号ID
        dr("SGGY_GSD") = TB_GSD.Text   'Val(TB_BZC_ID.Tag) 
        dr("SGGY_WD") = TB_WD.Text
        dr("SGGY_JS") = TB_JS.Text
        dr("SGGY_ZCYL") = TB_YL.Text
        dr("SGGY_Name") = TB_SGGY_Name.Text
    ...
    Protected Sub SetForm(ByVal dt As DataTable)
        ...
        Else
            ...
            Select Case State
                ...
                Case Enum_State.Invoid '作废
                    ...
                    Cmd_JiaLiao.Visible = False
                    Cmd_ShowJiaLiao.Visible = False
                    If IsNull(Dr("IsPageTwo"), False) = True AndAlso Dao.SGLingLiao_Get_State(Dr("MainBill"), Enum_State.Invoid) = True Then
                        Cmd_Rebuild.Visible = Cmd_Rebuild.Tag
                        Cmd_Rebuild.Enabled = False
                        Cmd_SetValid.Visible = Cmd_SetValid.Tag
                        Cmd_SetValid.Enabled = False
        ...
        If IsBidings = False Then
            ...
            TB_SGGY_Name.DataBindings.Add("Text", dtTable, TB_SGGY_Name.Name.Substring(3), False, DataSourceUpdateMode.OnPropertyChanged, "")
        ...
        'CB_BZC.Enabled = True
        TB_GSD.Text = IsNull(Dr("SGGY_GSD"), 0)
        TB_WD.Text = IsNull(Dr("SGGY_WD"), 0)
        TB_JS.Text = IsNull(Dr("SGGY_JS"), 0)
        TB_YL.Text = IsNull(Dr("SGGY_ZCYL"), 0)
        TB_SGGY_Name.Text = IsNull(Dr("SGGY_Name"), 0)
        ' CB_BZC.IDValue = Val(IsNull(Dr("BZC_ID"), 0))
        CB_BZC.Text = CB_BZC.GetByTextBoxTag
        ' TB_SGGY_Name.Text = IsNull(Dr("BZC_Name"), "")
        ...
        If IsJiaLiao = False Then
            JL_Qty = Dao.SGLingLiao_CountJiaLiao(TB_Produce_ID.Text)
...
#Region "工具栏按钮"
    ...
    Protected Sub SaveLingLiao(Optional ByVal Audit As Boolean = False)
        ...
        If Me.Mode = Mode_Enum.Add Then
            If FG1.Item(FG1.Rows.Count - 1, "IsPageTwo") AndAlso Text_Old.Text = "" Then
                ...
                R = Dao.SGLingLiao_Add(Dt, getlist(dtList, "IsPageTwo"))
                ReGetNewID()
                dtchang(Dt)
                R = Dao.SGLingLiao_Add(Dt, getlist(dtList, "IsPageTwo", TB_ID.Text))

            Else

                R = Dao.SGLingLiao_Add(Dt, dtList)
            End If
        Else
            R = Dao.SGLingLiao_Save(LId, Dt, dtList)
        End If
        If R.IsOk Then
            LId = TB_ID.Text
            ...
            If Audit Then
                Dim msg As MsgReturn = Dao.SGLingLiao_Audited(TB_ID.Text, True)
                If msg.IsOk Then
                    Bill_ID = TB_ID.Text
                    Mode = Mode_Enum.Modify
                    Me_Refresh()
    ...
    Protected Function CheckForm() As Boolean
        ...
        If TB_ID.Text = "" Then
            ...
            Return False
        End If
        If (Mode = Mode_Enum.Add OrElse (Mode = Mode_Enum.Modify AndAlso LId <> TB_ID.Text)) AndAlso Dao.SGLingLiao_CheckID(TB_ID.Text) Then
    ...
    Sub GetNewID()
        ...
        If Mode = Mode_Enum.Add Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.SGLingLiao, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "报表事件"
    ...
    Private Sub Cmd_Preview2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Print(OperatorType.Preview, False)
    End Sub

    Private Sub Cmd_Print2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType, Optional ByVal isFirst As Boolean = True)
        ...
        If CheckRight(ID, ClassMain.PrintMore) Then
            Dim R As New R20330_SGLingLiao
...
#Region "审核状态"
    ...
    Protected Sub UnAudit()
        Dao.SGLingLiao_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.SGLingLiao_Audited(TB_ID.Text, False)
        If msg.IsOk Then
            ShowOk(msg.Msg)
            Me_Refresh()
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        ...
        If LB_Page.Text = "显示第二页" Then
            If Dao.SGLingLiao_Get_State(LB_Page.Tag, Enum_State.Audited) = True Then
    ...
    Protected Sub DelLingLiao()
        ...
        Dao.SGLingLiao_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.SGLingLiao_Del(TB_ID.Text)
        If LB_Page.Text = "显示第二页" AndAlso LB_Page.Tag IsNot Nothing Then
            msg = Dao.SGLingLiao_Del(LB_Page.Tag)
            If msg.IsOk Then
    ...
    Private Sub Cmd_SetInValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetInValid.Click
        ...
        End If
        ShowConfirm("是否作废" & Ch_Name & " [" & TB_ID.Text & "]? " & vbCrLf & Show, AddressOf SGLingLiaoInValid)
    End Sub


    Sub SGLingLiaoInValid()
        Dao.SGLingLiao_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.SGLingLiao_InValid(TB_ID.Text, True)
        If LB_Page.Text = "显示第二页" AndAlso LB_Page.Tag IsNot Nothing Then
            msg = Dao.SGLingLiao_InValid(LB_Page.Tag, True)
            If msg.IsOk Then
                Dim li As New List(Of String)
                li.Add(TB_ID.Text)
                li.Add(LB_Page.Tag)
    ...
    Private Sub Cmd_SetValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetValid.Click
        ...
        Dim R As DtReturnMsg = Dao.SGLingLiao_OldID_Count(TB_ID.Text)
        ...
        ShowConfirm("是否反作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf SGLingLiaoValid)
    End Sub
    ...
    Sub SGLingLiaoValid()
        Dao.SGLingLiao_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.SGLingLiao_InValid(TB_ID.Text, False)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
...
#Region "比例计算"
    ...
    Private Sub TB_BZ_ZL_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TB_BZ_ZL.LostFocus
        ...
        CaculateSumAmount()
    End Sub
    Private Sub CB_Ratio_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        CacWaterLevel()
    End Sub
    ...
    Private Sub CacWaterLevel()
        Dim a As Integer = Val(TB_BZ_ZL.Text)
        ...
        For i As Integer = 0 To 999999
            ...
            End If
        Next
        If TB_GSD.Text = "干定" Then
            Tb_WaterLevel.Text = Format(b, FormatSharp("waterlevel"))
        Else
            Dim c As Integer = 0
            Dim d As Integer = 50
            c = b * 0.5
            For m As Integer = 0 To 999999
                If d < c Then
                    d = d + 50
                Else
                    Exit For
                End If
            Next
            Tb_WaterLevel.Text = Format(d, FormatSharp("waterlevel"))
        End If
    End Sub
    ...
    Private Sub CacQty(ByVal rowIndex As Integer)
        ...
        Else
            '助剂数量=配布重*比例*浴比
            FG1.Item(rowIndex, "Qty") = Val(TB_BZ_ZL.Text) * Val(FG1.Item(rowIndex, "sPercent")) * Val(0) / I_isKg
        End If
...
#Region "根据色号，布种获取染部配方"
    Private Sub GetRBPF()
        If TB_Produce_ID.Text <> "" Then
            Dim msg As DtReturnMsg = Dao.SGLingLiao_GetList_ByBZC(TB_Produce_ID.Text)
            If msg.IsOk Then
                SetGoodsFromRBPF(msg.Dt)
                If msg.Dt.Rows.Count = 0 Then
                    If IsClosing = False Then ShowErrMsg("没有找到与【" & TB_SGGY_Name.Text & "】【" & TB_BZ_Name.Text & "】相符的手感配方！")
                Else
                    'Dim R As DtReturnMsg = Dao.SGLingLiao_GetGYID_ByBZC(CB_BZC.IDAsInt, msg.Dt.Rows(0)("ID"))
                    'If R.IsOk AndAlso R.Dt.Rows.Count > 0 Then
...
#Region "获取缸号资料"
    ...
    Sub GoLingLiao(Optional ByVal JiaLiao As Boolean = False)
        If Old_ID <> "" Then
            If Mode = Mode_Enum.Add Then ComFun.DelBillNewID(BillType.SGLingLiao, Bill_ID)
            Mode = Mode_Enum.Modify
            Bill_ID = Old_ID
    ...
    Private Sub TB_Produce_ID_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TB_Produce_ID.KeyPress
        If TB_Produce_ID.ReadOnly = False AndAlso e.KeyChar = vbCr Then
            ...
            Dim Rt As MsgReturn = Dao.SGLingLiao_Count(s)
    ...
    Sub LoadFromGH(ByVal s As String)
        Dim R As DtReturnMsg = Dao.SGLingLiao_GetGH(s)
        ...
        If R.IsOk AndAlso R.Dt.Rows.Count > 0 Then
            ...
            Else
                Try
                    ...
                    TB_GSD.Text = IsNull(R.Dt.Rows(0)("SGGY_GSD"), 0)
                    CB_BZC.Text = CB_BZC.GetByTextBoxTag()
                    TB_SGGY_Name.Text = CB_BZC.NameValue

                    TB_Produce_ID.Text = R.Dt.Rows(0)("GH")

                    TB_ClientName.Tag = R.Dt.Rows(0)("Client_ID")
                    TB_ClientName.Text = R.Dt.Rows(0)("Client_Name")
                    TB_WD.Text = IsNull(R.Dt.Rows(0)("SGGY_WD"), 0)
                    TB_JS.Text = IsNull(R.Dt.Rows(0)("SGGY_JS"), 0)
                    TB_YL.Text = IsNull(R.Dt.Rows(0)("SGGY_ZCYL"), 0)
                    TB_SGGY_Name.Text = IsNull(R.Dt.Rows(0)("SGGY_Name"), 0)
                    ...
                    End If
                    CaculateSumAmount()
                    If R.Dt.Rows(0)("State1") >= Enum_ProduceState.PeiBu Then
                        TB_BZ_ZL.Focus()
                        CanLingliao = True
                    Else
...
#Region "根据缸号查询染料单页数"
    ...
    Private Sub GetPage(ByVal GH As String)
        Dim R As DtReturnMsg = Dao.SGLingLiao_Page_Count(GH)
        If R.IsOk Then
            LB_Page.Visible = False
    ...
    Protected Sub ShowModify()
        If LB_Page.Tag = "" Then
            ...
            Exit Sub
        End If
        Dim F As New F20331_SGLingLiao_Msg(LB_Page.Tag)
        With F
            .Mode = Mode_Enum.Modify
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
...
#Region "添加工艺"
    Private Sub Cmd_AddGy_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    Private Sub Cmd_InsertGY_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim VF As ViewForm = SelectGY()
        If VF IsNot Nothing Then
...
#Region "加料"
    ...
    Private Sub Cmd_JiaLiao_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_JiaLiao.Click
        ...
        CopyMe(True)
        LoadFromGH(TB_Produce_ID.Text)
        TB_SGGY_Name.ReadOnly = False
        TB_YL.ReadOnly = False
        TB_WD.ReadOnly = False
        TB_JS.ReadOnly = False
...
#Region "双击单号"
    ...
    Private Sub LB_Note_MouseDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs)
        If IsJiaLiao Then
            Dim F As New F20331_SGLingLiao_Msg(MainBill)
            With F
                .Mode = Mode_Enum.Modify
                .P_F_RS_ID = ""
                .P_F_RS_ID2 = ""
...
#End Region
    ...
    Private Sub Rebuild()
        ...
        If newID <> "" Then
            ShowConfirm(Dao.SGLingLiao_DB_NAME & "[" & TB_ID.Text & "]" & "已重建为[" & newID & "],是否打开[" & newID & "]", AddressOf ShowNewBill)
            Exit Sub
        End If
        Mode = Mode_Enum.Add
    ...
    Public Shared Function Flow_IsBillRenewed(ByVal billID As String) As String
        Dim sql As String = "select top 1 ID from T20330_SGLingLiao_Table where OldID=@OldID"
    ...
    Private Sub ShowBill(ByVal _ID As String)
        Dim F As New F20331_SGLingLiao_Msg(_ID)
        With F
            .Mode = Mode_Enum.Modify
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Private Sub Cmd_GetGY_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        GetRBPF()
    End Sub
...
#Region "常量"
    ...
    Protected Friend Shared SGLingLiao_DB_NAME As String = "出库单"
    ''' <summary>
    ''' 删除单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SGLingLiao_DelByid As String = "Delete from T20330_SGLingLiao_Table  where ID=@ID and State=0 "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SGLingLiao_GetByid As String = "select top 1 * from T20330_SGLingLiao_Table  where ID=@ID "

    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SGLingLiao_GetByid_WithName As String = "select top 1 T. * ,C.Client_Name,(Select top 1 GY_No + '.' + GY_Name from  T10004_GY where T10004_GY.ID=T.GY_ID1) as GY_Name1,(Select top 1 GY_No + '.' + GY_Name from  T10004_GY where T10004_GY.ID=T.GY_ID2) as GY_Name2 from T20330_SGLingLiao_Table T Left join T10110_Client C on T.Client_ID=C.ID  where T.ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SGLingLiao_SelByid As String = " select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name,BZ.BZ_Name ,C.Client_Name ,C.Client_No" & _
                                                   " from T20330_SGLingLiao_Table S " & _
                                                    ...
                                                    "left join T10002_BZ BZ on BZ.id=S.BZ_ID " & _
                                                     "where S.ID=@ID "
    'select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T20330_SGLingLiao_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SGLingLiao_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T20330_SGLingLiao_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SGLingLiao_GetListByid As String = "select * from T20331_SGLingLiao_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_SGLingLiao_SelListByid As String = "select L.*, WL.WL_No,WL.WL_Name from (select * from T20331_SGLingLiao_List  where ID =@ID)L  left join T10001_WL WL on WL.ID=l.WL_ID order by L.ID ,L.Line "
    'Private Const SQL_LingLiao_SelListid As String = "select L.*, WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Cost from (select * from T20331_SGLingLiao_List  where ID IN (@ID))L left join T10001_WL WL on WL.id=l.WL_ID order by L.ID ,L.Line "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SGLingLiao_CheckID As String = "select count(*) from T20330_SGLingLiao_Table  where ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_SGLingLiao_CheckIsModify As String = "select count(*) from T20331_SGLingLiao_List  where ID=@ID and Line=@Line"

    ''' <summary>
    ''' 根据色号获取染部配方
    ''' </summary>
    ''' <remarks></remarks>
    Public Const SQL_SGRBPF_PF_GeList_WithName = "SELECT L.SG_ID  AS ID,SG.WL_ID,SG.Qty as sPercent ,WL.WL_No,WL.WL_Name,WL.WL_Unit,WL.WL_Unit_LL,WL.WL_Spec,WL.WL_Cost as Cost,WL.WL_Cost as Price,WL.IsZJ ,WL.WL_Percent FROM (SELECT * FROM T30000_Produce_Gd WHERE GH =@GH) L LEFT JOIN T10036_SGGYList SG ON SG.ID =L.SG_ID LEFT JOIN T10001_WL WL ON WL.ID =SG.WL_ID Where Color='杂色' "
    ''' <summary>
    ''' 根据ID获取工艺图
    ''' </summary>
    ''' <remarks></remarks>
    Public Const SQL_SGGetGYID_ByBZC = "select top 1 GY_ID1,GY_ID2,(Select top 1 GY_No + '.' + GY_Name from  T10004_GY where T10004_GY.ID=T.GY_ID1) as GY_Name1,(Select top 1 GY_No + '.' + GY_Name from  T10004_GY where T10004_GY.ID=T.GY_ID2) as GY_Name2 from T10012_RB_PF T where T.BZC_ID=@BZC_ID and T.ID=@ID"

    ''' <summary>
    ''' 判断作废单是否有重建单
    ''' </summary>
    ''' <remarks></remarks>
    Public Const SQL_SGGetOldID_Count = " SELECT top 1 * FROM T20330_SGLingLiao_Table where oldid=@oldid"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Const SQL_SGGet_first_State = " SELECT top 1 State FROM T20330_SGLingLiao_Table where ID=@ID"

    ''' <summary>
    ''' 查找同一缸号的单据
    ''' </summary>
    ''' <remarks></remarks>
    Public Const SQL_SGGet_ID = " SELECT * FROM T20330_SGLingLiao_Table where Produce_ID=@Produce_ID and State=0"

    ''' <summary>
    ''' 查找单号是否已打印
    ''' </summary>
    ''' <remarks></remarks>
    Public Const SQL_SGGetPrint = " SELECT PrintOne,PrintTwo FROM T20330_SGLingLiao_Table where ID=@ID"
...
#Region "单一张单内容查询"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_SGLingLiao_GetByid_WithName, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_SelListById(ByVal sId As String) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", sId)
        Return PClass.PClass.SqlStrToDt(SQL_SGLingLiao_SelListByid, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGGetPrintState(ByVal sId As String) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", sId)
        Return PClass.PClass.SqlStrToDt(SQL_SGGetPrint, "@ID", sId)
    End Function
...
#Region "添加修改删除"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_CheckIsModify(ByVal ID As String, ByVal Line As Integer) As Boolean
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", ID)
        P.Add("Line", Line)
        Dim R As MsgReturn = SqlStrToOneStr(SQL_SGLingLiao_CheckIsModify, P)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return True
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_CheckID(ByVal ID As String) As Boolean
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", ID)
        Dim R As MsgReturn = SqlStrToOneStr(SQL_SGLingLiao_CheckID, P)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return False
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        ...
        sqlMap.Add("Table", SQL_SGLingLiao_GetByid)
        sqlMap.Add("List", SQL_SGLingLiao_GetListByid)
        ...
        Using H As New DtHelper(sqlMap, paraMap)
            If H.IsOk Then
                If H.DtList("Table").Rows.Count = 0 Then
                    ...
                    ''当前单据为领料单是判断是否重复开单
                    If IsNull(dtTable.Rows(0)("IsJiaLiao"), False) = False AndAlso Val(H.SelectOneValue("select Count(*) from T20330_SGLingLiao_Table where Produce_ID=@GH and type=@type and IsJiaLiao=0 and IsPageTwo=0 and state>-1", paraMap).Msg) > 1 Then
                        R.Msg = "本缸号已存在领料单只能开加料单!"
                        R.IsOk = False
                        Return R
                    End If
                    DvToDt(dtList, H.DtList("List"), New List(Of String), True)
                    Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & LingLiaoId & "'," & BillType.SGLingLiao & ")"
                    H.UpdateAll(True, TmSQL)
                    R.Msg = "" & SGLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
                    R.IsOk = True
                Else
                    R.IsOk = False
                    R.Msg = "" & SGLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
                End If
                Return R
            Else
                R.IsOk = False
                R.Msg = "" & SGLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        ...
        If LID <> LingLiaoId AndAlso SGLingLiao_CheckID(LingLiaoId) Then
            R.IsOk = False
            R.Msg = "" & SGLingLiao_DB_NAME & "[" & LingLiaoId & "]已经存在!请双击编号文本框,获取新编号!"
            Return R
        End If
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Audited Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & SGLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被审核不能再修改!"
                    Return R
                End If
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Invoid Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & SGLingLiao_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
                    Return R
                End If
                DvToDt(dtList, msg.DtList("List"), New List(Of String))
                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & LingLiaoId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.SGLingLiao
                If LID <> LingLiaoId Then
                    DtToUpDate(msg, "Update T20330_SGLingLiao_Table set id='" & LingLiaoId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
                    dtTable.Rows(0)("ID") = LingLiaoId
                    R.Msg = "" & SGLingLiao_DB_NAME & "[" & LID & "]已经修改为[" & LingLiaoId & "]!"
                Else
                    DtToUpDate(msg)
                    R.Msg = "" & SGLingLiao_DB_NAME & "[" & LID & "]修改成功!"
                End If
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & SGLingLiao_DB_NAME & "[" & LID & "]不存在"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & SGLingLiao_DB_NAME & "[" & LID & "]修改错误"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_Del(ByVal LingLiaoId As String)
        Dim P As New Dictionary(Of String, Object)
        P.Add("@ID", LingLiaoId)
        Return RunSQL(SQL_SGLingLiao_DelByid, P)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        ...
        paraMap.Add("ID", _ID)
        Try
            sqlMap.Add("Table", SQL_SGLingLiao_GetStateByid)
            msg = SqlStrToDt(sqlMap, paraMap)
            If msg.DtList("Table").Rows.Count = 1 Then
                If msg.DtList("Table").Rows(0)("State") <> Enum_State.Audited Then
                    '检查加工单状态
                    ...
                    If msg.DtList("Table").Rows(0)("State") <> State Then
                        ...
                        DtToUpDate(msg)
                        R.IsOk = True
                        R.Msg = "" & SGLingLiao_DB_NAME & "[" & _ID & "]" & OStr & "成功!"
                    Else
                        R.IsOk = False
                        R.Msg = "" & SGLingLiao_DB_NAME & "[" & _ID & "]已经被" & OStr & "!不能再" & OStr & "!"
                    End If
                Else
                    R.IsOk = False
                    R.Msg = "" & SGLingLiao_DB_NAME & "[" & _ID & "]已经被审核,不能" & OStr & "!"
                End If
            Else
                R.IsOk = False
                R.Msg = "" & SGLingLiao_DB_NAME & "[" & _ID & "]不存在,不能" & OStr & "!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & SGLingLiao_DB_NAME & "[" & _ID & "]" & OStr & "错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P20330_SGLingLiao_Audited", paraMap, True)
        If R.IsOk Then
...
#Region "生产加工单"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Produce_SGSelListById(ByVal sId As String) As DtReturnMsg
...
#Region "根据色号获取染部配方"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_GetList_ByBZC(ByVal _GH As String) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("GH", _GH)
        Return SqlStrToDt(SQL_SGRBPF_PF_GeList_WithName, P)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_GetGYID_ByBZC(ByVal BZC_ID As Integer, ByVal ID As Integer) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("BZC_ID", BZC_ID)
        P.Add("ID", ID)
        Return SqlStrToDt(SQL_SGGetGYID_ByBZC, P)
    End Function
#End Region
...
#Region "获取缸号资料"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_GetGH(ByVal Produce_ID As String) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("GH", Produce_ID)
        Return SqlStrToDt("select top 1 SG.*,G.GH,G.State,G.BZC_ID,G.BZ_ID,G.Client_ID,G.PB_ZLSum,G.PB_CountSum,G.IsFD,(Select top 1 Client_Name from T10110_Client where ID=G.Client_ID) as Client_Name from T30000_Produce_Gd G left join T10035_SGGY SG ON SG.ID=G.SG_ID where G.GH=@GH  ", P)
    End Function
#End Region
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiaoDX_CountJiaLiao(ByVal _ID As String) As Integer
        Dim R As MsgReturn = SqlStrToOneStr("select Count(*) from T20330_SGLingLiao_Table where Produce_ID=@ID and type=20320 and IsJiaLiao=1 and state>-1", "ID", _ID)
        Return Val(R.Msg)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_CountJiaLiao(ByVal _ID As String) As Integer
        Dim R As MsgReturn = SqlStrToOneStr("select Count(*) from T20330_SGLingLiao_Table where Produce_ID=@ID and type=20310 and IsJiaLiao=1 and state>-1", "ID", _ID)
        Return Val(R.Msg)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_Page_Count(ByVal _ID As String) As DtReturnMsg
        Dim R As DtReturnMsg = SqlStrToDt("select * from T20330_SGLingLiao_Table where Produce_ID=@ID and type=20310 and IsJiaLiao=0 and state>-1 and  not IsPageTwo is null order by IsPageTwo", "ID", _ID)
        Return R
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiaoDX_Count(ByVal GH As String) As MsgReturn
        Dim R As DtReturnMsg = SqlStrToDt("select top 1 ID from T20330_SGLingLiao_Table where Produce_ID=@ID and type=20320  and IsJiaLiao=0 and state>-1", "ID", GH)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_Count(ByVal GH As String) As MsgReturn
        Dim R As DtReturnMsg = SqlStrToDt("select top 1 ID from T20330_SGLingLiao_Table where Produce_ID=@ID and type=20310  and IsJiaLiao=0 and state>-1", "ID", GH)
        Dim Rt As New MsgReturn
        If R.IsOk AndAlso R.Dt.Rows.Count > 0 Then
            Rt.Msg = R.Dt.Rows(0)(0)
            Rt.IsOk = True
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_OldID_Count(ByVal _ID As String) As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_Get_State(ByVal _ID As String, ByVal State As Enum_State) As Boolean
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SGLingLiao_Get_ID(ByVal _ID As String) As DtReturnMsg
