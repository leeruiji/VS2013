...
Imports System.Text
Imports BaseClass
Public Class F10080_department_Sel
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Department_GetConditionNames()
        Me_Refresh()
    ...
    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Department_GetByOption(GetFindOtions)
        If msg.IsOk Then
            FG1.DtToFG(msg.Dt)
            FG1.SortByLastOrder()
            FG1.RowSetForce("Dept_No", ReturnId)
        End If
    End Sub
...
#Region "控件事件"
    ...
    '        Exit Sub
    '    End If
    '    Dim F As New F10081_department_Msg(FG1.SelectItem("Dept_No"))
    '    With F
    ...
    Protected Sub SetRemark()
        If TB_Preview.Text = "" Then
            TB_Preview.Text = FG1.SelectItem("Dept_Name")
        Else
            TB_Preview.Text = TB_Preview.Text & Me.P_F_RS_ID2 & FG1.SelectItem("Dept_Name")
        End If
    End Sub

    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的部门")
            Exit Sub
        End If
        ShowConfirm("是否删除部门 [" & FG1.SelectItem("Dept_Name") & "]?", AddressOf DelReProject)
    End Sub
    ...
    Protected Sub DelReProject()
        Dim msg As MsgReturn = Dao.ReProject_Disable(FG1.SelectItem("Dept_No"))
        If msg.IsOk Then
            Me_Refresh()
        Else
            ShowErrMsg("部门[" & FG1.SelectItem("Dept_Name") & "]删除错误！")
        End If
    End Sub
...
#Region "搜索工具栏"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Department_GetConditionNames() As List(Of FindOption)
...
#End Region
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Function Department_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append("select Dept_No,Dept_Name From T15000_Department WHere LEN(Dept_No)=4 ")
        'sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append("order by Dept_No")
