...
Public Class F10040_RBPF
    ...
    Private Sub F10000_BZC_Form_KeyPress(ByVal sender As Object, ByRef e As System.Windows.Forms.KeyPressEventArgs) Handles Me.Form_KeyPress
        Select Case e.KeyChar
            Case vbCr
            Case Else
                If TB_ConditionValue1.Focused = False AndAlso TSC_BZ.Focused = False AndAlso TSC_Client.Focused = False AndAlso CB_ConditionValue2.Focused = False Then
                    If e.KeyChar = Chr(System.Windows.Forms.Keys.Back) Then
                        If TB_ConditionValue1.Text.Length > 0 Then TB_ConditionValue1.Text = TB_ConditionValue1.Text.Substring(0, TB_ConditionValue1.Text.Length - 1)
                        TB_ConditionValue1.Focus()
                    Else
...
#Region "刷新FG"
    ...
    Protected Sub GetData(ByVal oList As OptionList)
        Dim msg As DtReturnMsg = Dao.BZC_GetByOption(oList)
...
#Region "控件事件"
    ...
    Protected Sub ModifyGoods()
        If FG1.RowSel <= 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F10041_RBPF_Msg(FG1.Item(FG1.RowSel, "ID"))
        With F
    ...
    Protected Sub DelGoods()
        Dim msg As MsgReturn = Dao.BZC_Del(FG1.SelectItem("ID"))
        If msg.IsOk Then
            Me_Refresh()
        Else
            ShowErrMsg(msg.Msg)
...
#Region "FG事件"
    ...
    Public Sub GetPF()
        If FG1.RowSel >= FG1.Rows.Fixed Then
            Dim msg2 As DtReturnMsg = Dao.RBPF_GetRBPFListByBZCID(FG1.Item(FG1.RowSel, "ID"), False)
            If msg2.IsOk Then
                MeIsLoad = False
    ...
    Protected Sub GetPFLIst()
        If Fg2.RowSel >= Fg2.Rows.Fixed Then
            Dim msg3 As DtReturnMsg = Dao.RBPF_PF_GetList(FG1.Item(FG1.RowSel, "ID"), Fg2.Item(Fg2.RowSel, "ID"))
            If msg3.IsOk Then
                CreateTreeiew(msg3.Dt, Fg3, MapGY, Me, "DyingStep")
            End If
        Else
    ...
    Protected Sub ModifyPF(ByVal _PFID As Integer, ByVal _BZC_ID As Integer)
        Dim F As New F10042_RBPF_PF(_PFID, _BZC_ID)
        With F
            .Mode = Mode_Enum.Modify
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
...
#Region "搜索工具栏"
    ...
    Private Sub TB_ConditionValue1_TextBoxTextAlignChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TB_ConditionValue1.TextBoxTextAlignChanged
