...
Public Class F10006_CostChange_Msg
    ...
    Private Sub FormLoad() Handles Me.Me_Load
        ...
        If Me.Mode = Mode_Enum.Add Then
            If Me.goodsId <> "" Then
                Dim msgGoods As DtReturnMsg = WL_GetById(goodsId)
    ...
    Sub GetNewID()
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> PID_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.CostChange, DTP_sDate.Value, PID)
                If R.IsOk Then
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_CostChange_GetBYID_WithName = "select P.*,WL_No,WL_Name,WL_Unit,WL_Spec,WL_Price,WL_Type_ID from T10005_CostChange P left join T10001_WL W on P.WL_ID=W.ID where P.ID=@ID"
#End Region
    ...
    Public Shared Function CostChange_Save(ByVal dt As DataTable) As MsgReturn
        ...
        sqlbuider.AppendLine("insert into T10005_CostChange (ID,WL_ID,Cost_Old,Cost_New,Audit_User,Audit_Date,WL_Qty,Remark) values(")
        sqlbuider.Append("@ID,@WL_ID,@Cost_Old,@Cost_New,@Audit_User,getDate(),@WL_Qty,@Remark)")




        sqlbuider.AppendLine("insert into T10050_Store_Detail (BillType,ID,BillName,sDate,Audited_Date,WL_ID,Price,Cost,Store_ID,StoreInorOut,QTY,Supplier_Id,ClientTpye) values(")
        ...
        sqlbuider.AppendLine(" update T10001_WL set WL_Cost= ")
