...
Imports System.Data
Imports System.Text
Public Class F26011_GoodsType_Msg
...
#Region "工具栏按钮"
    ...
    Private Sub Btn_ChoseParent_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_ChoseParent.Click
        Dim F As New F26010_GoodsType_Sel
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
...
#Region "商品分类"
    ...
    '===================商品分类==============
    Public Const SQL_GoodsType_GetAll As String = "select * from T26000_GoodsType order by GoodsType_ID"

    'Public Const SQL_GoodsType_GetChildrenBYParentID As String = "select * from T26000_GoodsType order by GoodsType_ID"


    Public Const SQL_GoodsType_GetByID As String = "select * from T26000_GoodsType where GoodsType_ID=@GoodsType_ID"

    Public Const SQL_GoodsType_Del As String = "Delete from T26000_GoodsType where GoodsType_ID=@GoodsType_ID"


    Public Const SQL_GoodsType_CheckDel = "SELECT(SELECT Count(*)" & vbCrLf & _
                                    "FROM T26000_GoodsType" & vbCrLf & _
                                    "WHERE (GoodsType_ID LIKE @GoodsType_ID + '%' and GoodsType_ID<>@GoodsType_ID))" & vbCrLf & _
                                    "+(select Count(*) from T26001_WLCoal where WL_Type_ID=@GoodsType_ID)"
#End Region
...
#Region "分类"
    ...
    Public Shared Function GoodsType_GetChildrenByParentID(ByVal parentID As String) As DtReturnMsg
        If Not parentID Is Nothing OrElse Not parentID = "" Then
            Dim sql As String = "select * from T26000_GoodsType where GoodsType_ID like '" & parentID & "%'  order by GoodsType_ID"
    ...
    Public Shared Function GoodsType_Save(ByVal oGid As String, ByVal dt As DataTable) As MsgReturn
        ...
        Try
            ...
            If msg.IsOk AndAlso msg.Dt.Rows.Count = 1 Then
                ...
                If gId <> newID Then
                    sql = "update T26000_GoodsType set GoodsType_ID =replace( GoodsType_ID,'" & oGid & "','" & newID & "') where GoodsType_ID like '" & oGid & "%' " & _
                          "update T26001_WLCoal set WL_Type_ID= replace( WL_Type_ID,'" & oGid & "','" & newID & "') where WL_Type_ID like '" & oGid & "%' "
    ...
    Public Shared Function GoodsType_GetNewID(ByVal parentId As String) As String
        Try
            ...
            If parentId.Length >= 5 Then
                sql = "select top 1 GoodsType_ID  from T26000_GoodsType where GoodsType_ID like '" & parentId & "%'  and len(GoodsType_ID)>" & parentId.Length & " order by len(GoodsType_ID) asc,GoodsType_ID desc"
            Else
                parentId = "GT"
                sql = "select top 1 GoodsType_ID  from T26000_GoodsType where len( GoodsType_ID)=5  order by GoodsType_ID desc"
            End If
