...
Public Class F26006_CostChange_Msg
    ...
    Private Sub FormLoad() Handles Me.Me_Load
        ...
        If Me.Mode = Mode_Enum.Add Then
            If Me.goodsId <> "" Then
                Dim msgGoods As DtReturnMsg = Dao.WLCoalWL_GetByID(goodsId)
    ...
    Sub GetNewID()
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> PID_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.CoCostChange, DTP_sDate.Value, PID)
                If R.IsOk Then
...
#Region "选择物料"
    ...
    Private Sub Btn_ChoseWl_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Btn_ChoseWl.Click
        Dim F As New F26002_WLCoal_Sel()
...
#Region "常量"
    ...
    ''' <remarks></remarks>
    ''' 
    Private Const SQL_CostChange_GetBYID = "select * from T26005_CostChange "
    Private Const SQL_CostChange_GetBYID_WithName = "select P.*,WL_No,WL_Name,WL_Unit,WL_Spec,WL_Price,WL_Type_ID from T26005_CostChange P left join T26001_WLCoal W on P.WL_ID=W.ID where P.ID=@ID"
#End Region
    ...
    ''' <remarks></remarks>
    ''' 
    Public Shared Function WLCoalWL_GetByID(ByVal _id As String) As DtReturnMsg
    ...
    Public Shared Function CostChange_Save(ByVal dt As DataTable) As MsgReturn
        ...
        sqlbuider.AppendLine("insert into T26005_CostChange (ID,WL_ID,Cost_Old,Cost_New,Audit_User,Audit_Date,WL_Qty,Remark) values(")
        sqlbuider.Append("@ID,@WL_ID,@Cost_Old,@Cost_New,@Audit_User,getDate(),@WL_Qty,@Remark)")




        sqlbuider.AppendLine("insert into T26003_Store_Detail (BillType,ID,BillName,sDate,Audited_Date,WL_ID,Price,Cost,Store_ID,StoreInorOut,QTY,Supplier_Id,ClientTpye) values(")
        ...
        sqlbuider.AppendLine(" update T26001_WLCoal set WL_Cost= ")
