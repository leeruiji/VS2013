...
Public Class F26201_StoreIn_Msg
    Dim LId As String = ""
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        ComFun.DelBillNewID(BillType.StoreIn_MT, Bill_ID)
    End Sub
    ...
    Sub GetNewID()
        ...
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.StoreIn_MT, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_DelByid As String = "Delete from T26200_StoreIn_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_GetByid As String = "select top 1 * from T26200_StoreIn_Table  where ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_SelByid As String = "select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T26200_StoreIn_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T26200_StoreIn_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_GetListByid As String = "select * from T26201_StoreIn_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_StoreIn_SelListByid As String = "select L.*,L.WL_Unit_LL as WL_Unit_LL2,L.WL_Unit_LL as WL_Unit_LL2,WL.WL_No,WL.WL_Name,WL.WL_Spec from (select * from T26201_StoreIn_List  where ID=@ID)L left join T26001_WLCoal WL on WL.id=l.WL_ID"
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_CheckID As String = "select count(*) from T26200_StoreIn_Table  where ID=@ID"

    ''' <summary>
    ''' 检查其他人有没有修改过
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_CheckIsModify As String = "select count(*) from T26201_StoreIn_list  where ID=@ID and Line=@Line"

    Private Const SQL_WLCoal_GetByNo As String = "select Top 1 Id,WL_No,WL_Name,WL_Unit,WL_Spec,WL_Store_ID,WL_Cost,WL_Type_ID,WL_Price from T26001_WLCoal where WL_No=@WL_No and isnull(WL_Disable,0)<>1"
...
#End Region
    ...
    Public Shared Function WLCoal_GetByNo(ByVal No As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_WLCoal_GetByNo, "@WL_No", No)
    End Function
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> StoreInId Then
                    DtToUpDate(msg, "Update T26200_StoreIn_Table set id='" & StoreInId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function StoreIn_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P26200_StoreIn_Audited", paraMap, True)
        If R.IsOk Then
