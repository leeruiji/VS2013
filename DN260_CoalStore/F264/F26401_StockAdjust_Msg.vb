...
Public Class F26401_StockAdjust_Msg
    Dim LId As String = ""
...
#Region "FG 事件"
    ...
    Protected Sub GetWLByNo(ByVal _No As String)
        Dim msg As DtReturnMsg = ComFun.Metal_GetByNo(_No)
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        ...
        Dim R As New R26400_StockAdjust_List
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_DelByid As String = "Delete from T26400_StockAdjust_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_GetByid As String = "select top 1 * from T26400_StockAdjust_Table  where ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_SelByid As String = "select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T26400_StockAdjust_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T26400_StockAdjust_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_GetListByid As String = "select * from T26401_StockAdjust_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_StockAdjust_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec from (select * from T26401_StockAdjust_List  where ID=@ID)L left join T26001_WLCoal WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_CheckID As String = "select count(*) from T26400_StockAdjust_Table  where ID=@ID"
    ...
    '''' </summary>
    '''' <remarks></remarks>
    '  Private Const SQL_StockAdjust_CheckIsModify As String = "select count(*) from T26401_StockAdjust_List  where ID=@ID and WL_ID=@WL_ID"
#End Region
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> StockAdjustId Then
                    DtToUpDate(msg, "Update T26400_StockAdjust_Table set id='" & StockAdjustId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function StockAdjust_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P26400_StockAdjust_Audited", paraMap, True)
        If R.IsOk Then
...
#Region "计算选择日期的库存"
    ...
    Public Shared Function StockAdjust_GetWLQty(ByVal IdList As List(Of Long), ByVal sDate As Date) As DtReturnMsg
        ...
        sql.AppendLine("select sum(Qty*StoreInOrOut) as WL_Qty,WL_ID from T26003_Store_Detail where sDate<=@sDate and WL_ID in ( ")
        sql.Append(IdList(0))
