...
Public Class F26400_StockAdjust
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.StockAdjust_GetConditionNames()
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.StockAdjust_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.StockAdjust_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub

    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.StockAdjust_GetByOption(GetFindOtions)
        If msg.IsOk Then
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F26401_StockAdjust_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的" & Dao.StockAdjust_DB_NAME)
            Exit Sub
        End If
        Dim F As New F26401_StockAdjust_Msg(FG1.Item(FG1.RowSel, "ID"))
        With F
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Dao.StockAdjust_DB_NAME & "[" & FG1.Item(FG1.RowSel, "ID") & "]?", AddressOf DelStockAdjust)
    End Sub
    ...
    Protected Sub DelStockAdjust()
        Dim msg As MsgReturn = Dao.StockAdjust_Del(FG1.Item(FG1.RowSel, "ID"))
        If msg.IsOk Then
...
#Region "搜索工具栏"
    ...
    Private Sub CB_ConditionName2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_ConditionName2.SelectedIndexChanged
        ...
        Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_StockAdjust_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
        If msg.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        'Dim R As New R26400_StockAdjust
        'R.Start(FG1.Item(FG1.RowSel, "ID"), DoOperator, Me.StockAdjustType)
    End Sub
...
#Region "审核状态"
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.StockAdjust_Audited(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
            Else
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.StockAdjust_SelListById(IsNull(FG1.Item(FG1.RowSel, "ID"), ""))
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
        Else
        End If
...
Partial Friend Class Dao
    ...
    '  ===================出库单信息==============
    Public Const SQL_StockAdjust_Get_WithName = "select * from T26400_StockAdjust_Table "

    Public Const SQL_StockAdjust_OrderBy As String = " order by sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_GetConditionNames() As List(Of FindOption)
        ...
        fo.SQL = "exists(select * from T10001_WL TW,T20301_StockAdjust_List TL where %WL_Name% and TL.wl_id=TW.ID and T26400_StockAdjust_Table.id=TL.id)"
        fo.Sign = "%WL_Name%"
        foList.Add(fo)
        ...
        fo.DB_Field = " "
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "(T26400_StockAdjust_Table.ID %Like% or  SumQty %Like% or SumLoss %Like% or Remark %Like%  )"
        fo.Sign = "%Like%"
        foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_StockAdjust_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_StockAdjust_OrderBy)
