...
Public Class F26101_Purchase_Msg
    Dim LId As String = ""
    ...
    Sub GetNewID()
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.Purchase_CO, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        If Me.Mode = Mode_Enum.Add Then
            Exit Sub
        End If
        Dim R As New R26101_Purchase_List
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_DelByid As String = "Delete from T26100_Puchase_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetByid As String = "select top 1 * from T26100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_SelByid As String = "select T.*,C.Supplier_Name From (Select top 1 * from T26100_Puchase_Table where ID=@ID)  T left join T26004_Supplier C on C.ID=T.Supplier_ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetStateByid As String = "select top 1 ID,State,State_User from T26100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetListByid As String = "select * from T26101_Puchase_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_Purchase_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Unit from (select * from T26101_Puchase_List  where ID=@ID)L left join T26001_WLCoal WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_CheckID As String = "select count(*) from T26100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_CheckIsModify As String = "select count(*) from T26101_Puchase_List  where @ID=ID and Line=@Line"
#End Region
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> PurchaseId Then
                    DtToUpDate(msg, "Update T26100_Puchase_Table set id='" & PurchaseId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function Purchase_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P26100_Puchase_Audited", paraMap, True)
        If R.IsOk Then
