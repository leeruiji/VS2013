...
Public Class F26102_Purchase_Sel
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim F As New F26101_Purchase_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F26101_Purchase_Msg(FG1.SelectItem("ID"))
        With F
...
#Region "SQL"
    Public Const SQL_PurchaseReturn_GetPurchase As String = "Select P.*,isnull(RL.SumRLQty,0) as SumRLQty,Supplier_No,S.Supplier_Name from  T26100_Puchase_Table P left join (Select Purchase_ID,  sum(isnull(Qty,0)) as SumRLQty from  T26111_PurchaseReturn_List  group  by Purchase_ID) RL on P.ID=RL.Purchase_ID left join T26004_Supplier S on  P.Supplier_ID=S.ID where P.State=1 and  P.SumQty>isnull(RL.SumRLQty,0) "

    '    Public Const SQL_PurchaseReturn_GetLines As String = "Select P.*,isnull(RL.SumRLQty,0) as SumRLQty ,L.Qty-isnull(SumRLQty,0) as CanReturn  from  T26101_Puchase_List P left join (Select Purchase_ID, WL_ID, sum(isnull(Qty,0)) as SumRLQty from  T26111_PurchaseReturn_List  group  by Purchase_ID,WL_ID) RL on P.ID=RL.Purchase_ID and P.WL_ID=RL.WL_ID where  p.Qty>isnull(RL.SumRLQty,0) "
#End Region
    ...
    Public Shared Function Purchase_SelListByIdList(ByVal idList As List(Of String)) As DtReturnMsg
        ...
        sqlBuider.Append(" select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Unit, L.Qty-isnull(SumRLQty,0) as CanReturn  from (select * from T26101_Puchase_List ")
        ...
        Next
        sqlBuider.Append("'') ")
        sqlBuider.Append(")L left join (Select Purchase_ID, WL_ID, sum(isnull(Qty,0)) as SumRLQty from  T26111_PurchaseReturn_List  group  by Purchase_ID,WL_ID) RL on L.ID=RL.Purchase_ID and L.WL_ID=RL.WL_ID ")
