...
Public Class F30930_PB_Match
    Private WithEvents R As New R30930_PB_Match()
    ...
    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.PB_Match_GetByOption(GetFindOtions)
        If msg.IsOk Then
            dtTable = msg.Dt
            R.Start(Excelout, DP_Start.Value.Date, Dp_End.Value.Date, msg.Dt)
        End If
    End Sub
...
Partial Class Dao
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PB_Match_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_GetPB)
        sqlBuider.Append("  WHERE  exists( select ID from  T40101_PBRK_List where GH=GD.GH and Old_PB >0)  ")
