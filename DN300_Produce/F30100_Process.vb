...
Public Class F30100_Process
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.Process_GetConditionNames()

        CB_State.ComboBox.DisplayMember = "Field"
        CB_State.ComboBox.ValueMember = "DB_Field"
        CB_State.ComboBox.DataSource = Dao.Process_GetState
    ...
    '    Dim msg As DtReturnMsg = Dao.Process_GetByOption(GetFindOtions)
    '    If msg.IsOk Then
    '        SetFg(msg.Dt)
    ...
    Protected Sub GetData(ByVal oList As OptionList)
        Dim msg As DtReturnMsg = Dao.Process_GetByOption(oList)
        If msg.IsOk Then
...
#Region "Fg事件"
    Private Sub FG1_AfterEdit(ByVal sender As Object, ByVal e As C1.Win.C1FlexGrid.RowColEventArgs) Handles FG1.AfterEdit
        ...
        Select Case FG1.ColSel
            ...
            Case FG1.Cols("LRemark").Index
                UpdateRemark(FG1.Item(FG1.RowSel, "LRemark"))
        End Select
        If IsUpdated Then
            updateRow()
        End If
    ...
    Protected Function CheckForm() As Boolean
        If IsNull(FG1.Item(FG1.RowSel, "Store_CPRK"), "") <> "" And FG1.Item(FG1.RowSel, "TuoShui") = "" Then
            ShowErrMsg("此缸号为半成品入库，如需更改状态，请先取消入库！")
            Return False
        End If
        If IsNull(FG1.Item(FG1.RowSel, "Store_CPRK"), "") <> "" Then
            ShowErrMsg("成品入库不能更改状态！")
            Return False
        End If
        Return True
    End Function
    Private Sub FG1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles FG1.Click
        If Cmd_CancelEdit.Visible = False Then
            Exit Sub
        End If
        If CheckForm() Then
            ...
            Select Case FG1.ColSel
                Case FG1.Cols("PeiBu").Index
                    ...
                    End If
                    If FG1.Item(FG1.RowSel, "PeiBu") = "" Then
                        ShowErrMsg("单号[" & FG1.Item(FG1.RowSel, "GH") & "]的配布状态要在配布单中设置")
                        Exit Sub
                    Else
                        LastState = Enum_ProduceState.XiaDan
                        ShowErrMsg("单号[" & FG1.Item(FG1.RowSel, "GH") & "]已经配布,只能在配布单‘取消配布’按钮中取消")
                        Exit Sub
                    End If
...
#Region "加载FG"
    Private Sub updateRow()
        Dim R As DtReturnMsg = Dao.Process_GetByGH(FG1.Item(FG1.RowSel, "GH"))
    ...
    Private Sub SetRow(ByVal dr As DataRow, ByVal rowIndex As Integer)
        ...
        FG1.Item(rowIndex, "LRemark") = IsNull(dr("LRemark"), "")
        ...
        FG1.Item(rowIndex, "Store_CPRK") = dr("Store_CPRK")
        ...
        If IsNull(dr("State"), Enum_ProduceState.AddNew) >= Enum_ProduceState.SongHuo Then
            FG1.Item(rowIndex, "Date_JiaoHuo") = dr("Date_JiaoHuo")
        End If
...
#Region "新增,修改"
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的" & Dao.Process_DB_NAME)
            Exit Sub
        End If
        Dim F As New F30001_Produce_Gd_Msg(FG1.Item(FG1.RowSel, "GH"))
        With F
...
#Region "控件事件"
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel <= 0 Then
            ShowMsg("请选择一个要删除的" & Dao.Process_DB_NAME, "")
            Exit Sub
        End If
        ShowConfirm("是否删除" & Dao.Process_DB_NAME & " [" & FG1.Item(FG1.RowSel, "GH") & "]?", AddressOf DelProduce)
    End Sub
        ...
        If CB_State.ComboBox.SelectedIndex >= 0 Then
            Select Case CB_State.ComboBox.SelectedIndex
                Case 0
                    fo = New FindOption
                    fo.DB_Field = "G.State"
                    fo.Value = Enum_ProduceState.AddNew
                    fo.Value2 = Enum_ProduceState.RuKu
                    fo.Field_Operator = Enum_Operator.Operator_Between
                    oList.FoList.Add(fo)
                ...
                Case 3
                    fo = New FindOption
                    fo.DB_Field = "State"
                    fo.Value = Enum_ProduceState.Invalid
                    fo.Field_Operator = Enum_Operator.Operator_Equal
                    oList.FoList.Add(fo)
                Case 4
                    fo = New FindOption
                ...
                Case 5
                    ...
                    fo.DB_Field = "G.State"
                    fo.Value = Enum_ProduceState.AddNew
                    fo.Value2 = Enum_ProduceState.XiaDan
                    fo.Field_Operator = Enum_Operator.Operator_Between
                    oList.FoList.Add(fo)
                ...
                Case 6
                    ...
                    fo = New FindOption
                    fo.DB_Field = "G.State"
                    fo.Value = Enum_ProduceState.ChuGang
                    fo.Value2 = Enum_ProduceState.PeiBu
                    fo.Field_Operator = Enum_Operator.Operator_Equal
                    oList.FoList.Add(fo)
...
#Region "加急"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub Cmd_JiaJi_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_JiaJi.Click
    ...
    Private Sub Cmd_UnJiaJi_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_UnJiaJi.Click
        Dim sqlBuider As New StringBuilder
        Dim P As New Dictionary(Of String, Object)
        P.Add("UPD_User", User_Name)
        For i As Int16 = 1 To FG1.Rows.Count - 1
...
#Region "编辑状态"
    Private Sub Cmd_Eidt_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Eidt.Click
        FG1.CanEditing = True
    ...
    Private Sub Cmd_CancelEdit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_CancelEdit.Click
        FG1.FinishEditing()
        FG1.CanEditing = False
        Cmd_CancelEdit.Visible = False
...
#End Region
    ...
    Private Sub Cmd_ShowWork_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_ShowWork.Click
    ...
    Private Sub Cmd_CanCelWork_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_CanCelWork.Click
        SplitContainer1.Panel2Collapsed = True
...
Partial Friend Class Dao
    ...
    Public Const Process_DB_NAME As String = "进度表"

    '===================进度表信息==============
    Public Const SQL_Process_Get_WithName = " select G.Store_CPRK,G.GH,G.State,G.RanSeGH,G.Date_JiaoHuo,G.Date_KaiDan,G.BZC_ID,G.ClientBZC,G.BZ_ID,G.Contract,G.CR_LuoSeBzCount,G.JiaJi,G.IsFD,BzcMsg,G.PB_CountSum,G.CPName,G.CR_LuoSeBzZL,G.PB_ZLSum, " & _
                                            "G.State, G.Client_ID,C.Client_No,C.Client_Name,G.LRemark ,G.CR_LuoSeBzCount,G.CR_LuoSeBzZL," & _
                                            "BZ.BZ_No,BZ.BZ_Name,ClientBZC,BZC_No,BZC_Name,BZC_PF" & _
    ...
    Public Const SQL_Process_OrderBy As String = " order by  G.Client_ID,G.GH"

    Public Const SQL_Worklist_Get_GH As String = "Select * T30003_Produce_Worklist where GH=@GH"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_GetConditionNames() As List(Of FindOption)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_GetState() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "未出货"
        fo.DB_Field = Enum_ProduceState.SongHuo
        ...
        fo = New FindOption
        fo.Field = "已作废"
        fo.DB_Field = Enum_ProduceState.Invalid
        foList.Add(fo)
        ...
        fo = New FindOption
        fo.Field = "未配布"
        fo.DB_Field = Enum_ProduceState.PeiBu
        fo.Field_Operator = Enum_Operator.Operator_Less
        foList.Add(fo)
        ...
        fo = New FindOption
        fo.Field = "出缸"
        fo.DB_Field = Enum_ProduceState.ChuGang
        fo.Field_Operator = Enum_Operator.Operator_Equal
        foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_Process_Get_WithName)
        sqlBuider.Append("  WHERE  1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_Process_OrderBy)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_GetByGH(ByVal _Gh As String) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("GH", _Gh)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_Process_Get_WithName)
    ...
    ''' <remarks></remarks>
    ''' 
    Public Shared Function GetWorkState_ByTime(ByVal Start As Boolean, ByVal Finish As Boolean) As String
