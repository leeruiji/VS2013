...
Public Class F30110_DProcess
    ...
    Private Sub F30110_DProcess_AfterLoad() Handles Me.AfterLoad
        FG1.FG_ColResize()
        SumFG1.AddSum()
    End Sub
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.BZ_GetConditionNames()


        CB_State.ComboBox.DisplayMember = "Field"
        CB_State.ComboBox.ValueMember = "DB_Field"
        CB_State.ComboBox.DataSource = Dao.DProcess_GetCondition3()
    ...
    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.BZ_GetByOption(GetFindOtions)
        If msg.IsOk Then
            msg.Dt.Columns.Add("Ben", GetType(String))
            ...
            For Each Dr As DataRow In msg.Dt.Rows
                If IsNull(Dr("Bengci"), True) = True Then
                    Dr("Ben") = "甲班"
                Else
                    Dr("Ben") = "乙班"
                End If
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F30111_DProcess_Msg()
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F30111_DProcess_Msg(FG1.Item(FG1.RowSel, "GH"), FG1.Item(FG1.RowSel, "Line"))
        With F
    ...
    ''' <remarks></remarks>
    Protected Sub DelSQ()
        Dim msg As MsgReturn = Dao.Sql_DelectGH(FG1.Item(FG1.RowSel, "GH"), FG1.Item(FG1.RowSel, "line"))
        If msg.IsOk Then
            Me_Refresh()
        Else
            ShowErrMsg("删除失败！")
...
#Region "搜索工具栏"
    ...
    Private Sub TB_ConditionValue1_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles TB_ConditionValue1.TextChanged
        If TB_ConditionValue1.TextLength > 5 Then
            Me_Refresh()
        End If
    End Sub
...
#End Region
    ...
    Private Sub Cmd_WordName_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_WordName.Click
        ...
        Dim dtwork As DtReturnMsg = Dao.Sql_GetWorkName(DP_Start.Value, DP_End.Value)
        If dtwork.IsOk Then
    ...
    Private Sub Cmd_JTTJ_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_JTTJ.Click
        ...
        dr("eDate") = DP_End.Value
        dt.Rows.Add(dr)
        Dim DtJTS As DtReturnMsg = Dao.Sql_GetJTH(DP_Start.Value, DP_End.Value)
        If DtJTS.IsOk Then
    ...
    Private Sub Cmd_Invoid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Invoid.Click
        ...
        If InList.Count > 0 Then
            ShowConfirm("是否要将选定定型工艺作废? ", AddressOf Invoid)
        Else
            ShowErrMsg("您没有选择定型工艺!")
        End If
    End Sub
    ...
    Protected Sub Invoid()
        ...
        For Each _ID As String In InList
            msg = Dao.DProcess_InValid(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
            Else
            ...
            End If
        Next
        msgBudder.Insert(0, "定型工艺批量作废,共" & okCount & "张单作废成功," & failedCount & "张单作废失败!")
    ...
    Protected Sub UnInvoid()
        ...
        msgBudder.AppendLine("")
        For Each _ID As String In InList
            msg = Dao.DProcess_InValid(_ID, False)
            If msg.IsOk Then
                okCount = okCount + 1
            Else
                failedCount = failedCount + 1
            ...
            End If
        Next
        msgBudder.Insert(0, "定型工艺批量反作废,共" & okCount & "张单反作废成功," & failedCount & "张单反作废失败!")
        If failedCount > 0 Then
            ShowErrMsg(msgBudder.ToString)
        Else
            ShowOk(msgBudder.ToString, False)
    ...
    Private Sub Cmd_UnInvoid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_UnInvoid.Click
        ...
        Next
        If InList.Count > 0 Then
            ShowConfirm("是否要将选定定型工艺反作废? ", AddressOf UnInvoid)
        Else
            ShowErrMsg("您没有选择定型工艺!")
        End If
    End Sub
...
Partial Friend Class Dao
    Public Const DProcess_DB_NAME = "定型工艺"
    'Public Const SQL_SQ_Get_WithName = " select D.*,W.Work_Name from T30110_DProcess D left join T10022_Work W ON D.Remark=W.Work_No "
    Public Const SQL_SQ_Get_WithName = "SELECT D.*, W.Work_Name,BZ_No,BzcMsg,IsFD, GD.CR_LuoSeBzCount, GD.PB_ZLSum, GD.Client_Name, GD.BZ_Name, GD.BZC_Name,GD.BZC_No,GD.ClientBzc FROM " & _
                                       " T30110_DProcess D LEFT OUTER JOIN T10022_Work W ON D.Remark = W.Work_No LEFT OUTER JOIN " & _
                                    "(SELECT G.GH,G.ClientBzc,BZ_No,BzcMsg,IsFD, G.CR_LuoSeBzCount, G.PB_ZLSum, C.Client_Name, BZ.BZ_Name, BZC.BZC_Name,BZC.BZC_No " & _
                                     " FROM T30000_Produce_Gd G LEFT JOIN " & _
                                      ...
                                      " T10003_BZC BZC ON BZC.id = G.BZC_id) GD ON D.GH = GD.GH"

    Public Const SQL_DelGh = "DELETE from T30110_DProcess where GH=@GH and Line=@Line"
    Public Const SQL_SQ_OrderBy As String = " order by sDate "
    ...
    Public Const SQL_GetWorkName_Sum As String = "SELECT W.Work_Name, isnull(SUM(CR_LuoSeBzCount), 0) AS CR_LuoSeBzCount, " & vbCrLf & _
   "      isnull(SUM(PB_ZLSum), 0) AS PB_ZLSum" & vbCrLf & _
   "FROM (SELECT *" & vbCrLf & _
   "        FROM T10022_Work" & vbCrLf & _
   "        WHERE Dept_No = 'D006') W LEFT OUTER JOIN" & vbCrLf & _
   "          (SELECT *" & vbCrLf & _
   "         FROM T30110_DProcess" & vbCrLf & _
   "         WHERE sDate BETWEEN @sDate1 AND @sDate2 and isInvoid=0) T ON " & vbCrLf & _
   "      W.Work_No = T .Remark LEFT JOIN" & vbCrLf & _
   "      T30000_Produce_Gd G ON G.GH = T .GH" & vbCrLf & _
   "GROUP BY W.Work_Name"


    Public Const SQL_GetJTS As String = " SELECT JTH, Bengci, Sum(DxCount) AS DxCount, Sum(DSZL) AS Dszl " & _
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BZ_GetConditionNames() As List(Of FindOption)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function DProcess_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BZ_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_SQ_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_SQ_OrderBy)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Sql_DelectGH(ByVal GH As String, ByVal line As Integer)
        Dim p As New Dictionary(Of String, Object)
        p.Add("GH", GH)
        p.Add("Line", line)
        Return RunSQL(SQL_DelGh, p)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Sql_GetWorkName(ByVal sDate As Date, ByVal eDate As Date) As DtReturnMsg
        Dim p As New Dictionary(Of String, Object)
        p.Add("@sDate1", sDate)
        p.Add("@sDate2", eDate)
        Return PClass.PClass.SqlStrToDt(SQL_GetWorkName_Sum, p)
    End Function




    Public Shared Function Work_Get(ByVal GH As String) As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function DProcess_InValid(ByVal _ID As Long, ByVal isInvoid As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("Line", _ID)
        paraMap.Add("isInvoid", isInvoid)

        Dim R As MsgReturn = SqlStrToOneStr("Update T30110_DProcess set isInvoid=@isInvoid  where Line=@Line  ", paraMap)
        Dim s As String = IIf(isInvoid, "已作废", "还原")
        If R.IsOk Then
            R.Msg = DProcess_DB_NAME & s & "成功！"
        Else
            R.Msg = DProcess_DB_NAME & s & "失败！" & R.Msg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Sql_GetJTH(ByVal sDate As Date, ByVal eDate As Date) As DtReturnMsg
        Dim p As New Dictionary(Of String, Object)
        p.Add("@sDate1", sDate)
        p.Add("@sDate2", eDate)
        Return PClass.PClass.SqlStrToDt(SQL_GetJTS, p)
    End Function
