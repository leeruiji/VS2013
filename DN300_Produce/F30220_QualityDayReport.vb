...
Public Class F30220_QualityDayReport
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.QualityDayReport_GetConditionNames()
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.QualityDayReport_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.QualityDayReport_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub

    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.QualityDayReport_GetByOption(GetFindOtions)
        If msg.IsOk Then
            ...
            For i As Integer = 0 To msg.Dt.Rows.Count - 1
                If i = 0 Then
                    msg.Dt.Rows(i)("SumBJCL") = msg.Dt.Rows(i)("DayBJCL")
                    msg.Dt.Rows(i)("SumLYTB") = msg.Dt.Rows(i)("DayLYTB")
                Else
                    msg.Dt.Rows(i)("SumBJCL") = msg.Dt.Rows(i)("DayBJCL") + msg.Dt.Rows(i - 1)("SumBJCL")
                    msg.Dt.Rows(i)("SumLYTB") = msg.Dt.Rows(i)("DayLYTB") + msg.Dt.Rows(i - 1)("SumLYTB")
                End If
                If IsNull(msg.Dt.Rows(i)("DayBJCL"), 0) <> 0 Then
                    msg.Dt.Rows(i)("DayLYTBL") = msg.Dt.Rows(i)("DayLYTB") / msg.Dt.Rows(i)("DayBJCL") * 100
                End If
                If IsNull(msg.Dt.Rows(i)("SumBJCL"), 0) <> 0 Then
                    msg.Dt.Rows(i)("SumLYTBL") = msg.Dt.Rows(i)("SumLYTB") / msg.Dt.Rows(i)("SumBJCL") * 100
                End If
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim dtmsg As MsgReturn = Dao.SQL_ChecksDateZL(DP_KD.Value)
        If dtmsg.IsOk AndAlso Val(dtmsg.Msg) > 0 Then
            ShowConfirm("日期［" & DP_KD.Value & "］质量日报表已存在是否打开", AddressOf ShowQualityModify)
            Exit Sub
        End If

        Dim F As New F30221_QualityDayReport_Msg(DP_KD.Value)
        With F
            .Mode = Mode_Enum.Add
            .P_ID = ID
            .P_F_RS_ID = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的" & Dao.QualityDayReport_DB_NAME)
            Exit Sub
        End If
        QualityModify(FG1.SelectItem("ID"))
    End Sub


    Private Sub QualityModify(ByVal sID As String)
        Dim F As New F30221_QualityDayReport_Msg(sID)
        With F
    ...
    Private Sub ShowQualityModify()
        Dim dtQua As DtReturnMsg = Dao.SQL_GetQuaByDay(DP_KD.Value)
        If dtQua.IsOk AndAlso dtQua.Dt.Rows.Count > 0 Then
            QualityModify(dtQua.Dt.Rows(0)("ID"))
        End If
    End Sub
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Dao.QualityDayReport_DB_NAME & "[" & FG1.SelectItem("ID") & "]?", AddressOf DelQualityDayReport)
    End Sub
    ...
    Protected Sub DelQualityDayReport()
        Dim msg As MsgReturn = Dao.QualityDayReport_Del(FG1.SelectItem("ID"))
        If msg.IsOk Then
...
#Region "搜索工具栏"
    ...
    Private Sub CB_ConditionName2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_ConditionName2.SelectedIndexChanged
        ...
        Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_QualityDayReport_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
        If msg.IsOk Then
...
#Region "审核状态"
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.QualityDayReport_Audited(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.QualityDayReport_GetRe_byid(IsNull(FG1.SelectItem("ID"), ""))
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
            SumFG2.ReSum()
        Else
...
Partial Friend Class Dao
    ...
    '===================采购单信息==============
    Public Const SQL_QualityDayReport_Get_WithName = "select T.* from T30220_QualityDayReport_Table T "
    Private Const SQL_QualityDayReport_OrderBy As String = " order by sDate "
    Public Const SQL_CheckIDByDay As String = "Select Count(*)as Qty from T30220_QualityDayReport_Table Where sDate=@sDate "
    Public Const SQL_QuaGetID As String = "Select ID From T30220_QualityDayReport_Table where sDate=@sDate"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function QualityDayReport_GetConditionNames() As List(Of FindOption)
        ...
        fo.SQL = "exists(select * from  T30222_QualityDayReport_QPList QP where %LYCM% and T.ID=QP.ID)"
        fo.Sign = "%LYCM%"
        foList.Add(fo)
        ...
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "exists(select * from T10110_Client C,T30223_QualityDayReport_LJMSList LJ where %Client_Name% and LJ.Client_ID=C.ID and T.id=LJ.id)"
        fo.Sign = "%Client_Name%"
        foList.Add(fo)
        ...
        fo.DB_Field = "BZ_Name"
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "exists(select * from T10002_BZ BZ,T30223_QualityDayReport_LJMSList LJ where %BZ_Name% and LJ.BZ_ID=BZ.ID and T.id=LJ.id)"
        fo.Sign = "%BZ_Name%"
        foList.Add(fo)
        ...
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "exists(select * from T10003_BZC B,T30223_QualityDayReport_LJMSList LJ where %BZC_Name% and LJ.BZC_ID=B.ID and T.id=LJ.id)"
        fo.Sign = "%BZC_Name%"
        foList.Add(fo)
        ...
        fo.DB_Field = "GH"
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "exists(select *  from T30223_QualityDayReport_LJMSList LJ where %GH% and  T.id=LJ.id)"
        fo.Sign = "%GH%"
        foList.Add(fo)
        ...
        fo.DB_Field = " "
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "(T.ID %Like% or Remarks %Like% or JYRemarks %Like% or LJRemarks %Like%  )"
        fo.Sign = "%Like%"
        foList.Add(fo)
        Return foList
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function QualityDayReport_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function QualityDayReport_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_QualityDayReport_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_QualityDayReport_OrderBy)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SQL_ChecksDateZL(ByVal sDate As Date) As MsgReturn
        Dim p As New Dictionary(Of String, Object)
        p.Add("@sDate", sDate)
        Return PClass.PClass.SqlStrToOneStr(SQL_CheckIDByDay, p)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function SQL_GetQuaByDay(ByVal sDate As Date) As DtReturnMsg
        Dim p As New Dictionary(Of String, Object)
        p.Add("@sDate", sDate)
        Return PClass.PClass.SqlStrToDt(SQL_QuaGetID, p)
    End Function
