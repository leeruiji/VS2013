...
Public Class F30105_PG_Msg
    ...
    Public PG_State As String
    Public GHType As PGEnum_GH_Type = PGEnum_GH_Type.Normal
    ...
    '        If TB_Remark.Focused = False Then
    '            keyData = Keys.Tab
    '        End If
    '    End If
    ...
    Private NowDate As Date = GetTime()
    Private Sub F30105_PG_Me_Load() Handles Me.Me_Load
        FormCheckRight()
        ...
        'CB_State.DataSource = Dao.PGStateName_GetConditionNames

        'CB_WorkList.DisplayMember = "WorkList_Name"
        'CB_WorkList.ValueMember = "ID"
        'Dim dtWorkList As DtReturnMsg = Dao.PGGet_AllWorkList
        ...
        'NowDate = PClass.PClass.GetTime
        'DP_Date_JiaoHuo.Value = NowDate
        'DP_Date_KaiDan.Value = NowDate
        'DP_sDate.Value = NowDate
        DP_JGSJ.Value = NowDate
        DP_CGSJ.Value = NowDate
    ...
    Protected Sub LoadForm()
        ...
        If Mode = Mode_Enum.Add Then
            Dim msg As DtReturnMsg = Dao.PGGd_GetEmptyRow()
            If msg.IsOk Then
            ...
            End If
            Dim worklist As DtReturnMsg = Dao.PG_Worklist(0)
            Dim dtChare_Msg As DtReturnMsg = Dao.PG_GetCharge(0)
        ...
        Else
            Dim msg As DtReturnMsg = Dao.PGGd_GetById(Bill_ID)
            Dim worklist As DtReturnMsg = Dao.PG_Worklist(Bill_ID)
            Dim dtChare_Msg As DtReturnMsg = Dao.PG_GetCharge(Bill_ID)
            If dtChare_Msg.IsOk Then
                Dt_Charge = dtChare_Msg.Dt
            End If
...
#Region "相关缸号"
    ...
    'Private Sub LabelGH_DoubleClick(ByVal sender As Object, ByVal e As System.EventArgs)
...
#Region "自动跳格"
    ...
    Sub AddAutoGoNext()
        ...
        'AddHandler DP_sDate.KeyDown, AddressOf AutoGoNext_KeyDown
...
#Region "表单信息"
    ...
    Protected Sub GetForm()
        If Me.Dt_Produce IsNot Nothing Then
            ...
            'End If
            'Dr("GHType") = GHType
            'Dr("IsFD") = IIf(GHType = PGEnum_GH_Type.FD, True, False)
            'Dr("IsTP") = IIf(GHType = PGEnum_GH_Type.TP, True, False)


            'Dr("Client_ID") = Val(TB_ClientID.Tag)
            'Dr("Client_No") = TB_ClientID.Text
            'Dr("Client_Name") = TB_ClientName.Text
            Dr("RGBH") = TB_RGBH.Text
            'Dr("Line") = TB_Line.Text
            ...
            'ComFun.GetColValue(Dr, TB_CGRemark, "", "", Lock)
            ...
            'ComFun.GetColValue(Dr, DP_sDate, NowDate, "", Lock)
            'ComFun.GetColValue(Dr, DP_Date_LuoSe, NowDate, "", Lock)
            ComFun.GetColValue(Dr, DP_JGSJ, NowDate, "", Lock)
            ComFun.GetColValue(Dr, DP_CGSJ, NowDate, "", Lock)
    ...
    Protected Sub SetForm()
        If Me.Dt_Produce IsNot Nothing Then
            ...
            'GHType = IsNull(Dr("GHType"), PGEnum_GH_Type.Normal)
            PG_State = IsNull(Dr("RGBH"), "")
            If PG_State = "" Then
                LB_State.Text = "未排缸"
            Else
                LB_State.Text = "已排缸"
            ...
            '    GHType = PGEnum_GH_Type.TP
            'Else
            '    GHType = PGEnum_GH_Type.Normal
            'End If
            'Select Case GHType
            '    Case PGEnum_GH_Type.FD '返定
            ...
            TB_RGBH.Text = IsNull(Dr("RGBH"), "")
            'TB_Line.Text = Dr("Line")
            TB_BZC.Text = IsNull(Dr("BzcMsg"), "")
            TB_BZ.Text = Dr("BZ_No") & Dr("BZ_Name")
            ...
            'ComFun.SetColValue(Dr, TB_CGRemark, "", "", Lock)
            SetColValue(Dr, DP_JGSJ, NowDate, "", Lock)
            SetColValue(Dr, DP_CGSJ, NowDate, "", Lock)
            ...
            'ComFun.SetColValue(Dr, DP_sDate, NowDate, "", Lock)
    ...
    '            Cmd_Modify.Enabled = False
    '            'Cmd_WorkList_Save.Enabled = Not Cmd_Modify.Enabled
    '            'Cmd_ChangeColor.Visible = Cmd_ChangeColor.Tag
    '            Cmd_Invalid.Visible = Cmd_Invalid.Tag
    '            Cmd_SetValid.Visible = False
    '            'Cmd_FG.Visible = False
    '            'If Cmd_ChangeColor.Tag = False Then
...
#Region "DateTimePicker"
    Public Sub SetColValue(ByVal Dr As DataRow, ByRef DTPicker As DateTimePicker, Optional ByVal IsNullText As Object = Nothing, Optional ByVal Field As String = "", Optional ByVal Lock As Boolean = False)
        If Field = "" Then
            Field = DTPicker.Name.Substring(DTPicker.Name.IndexOf("_") + 1)
        End If
        If DTPicker.ShowCheckBox Then
            If IsDBNull(Dr.Item(Field)) Then
                DTPicker.Value = GetLocalTime()
                DTPicker.Checked = False
            Else
                If IsNullText Is Nothing Then
                    IsNullText = GetLocalTime()
                End If

                DTPicker.Value = IsNull(Dr.Item(Field), IsNullText)
                If Lock = False Then DTPicker.Enabled = False
            End If
        Else
            DTPicker.Value = IsNull(Dr.Item(Field), IsNullText)
...
#Region "工具栏事件"
    ...
    Private Sub Cmd_Mod_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Mod.Click
        DP_JGSJ.Enabled = True
        DP_CGSJ.Enabled = True
    End Sub
...
#End Region
    ...
    Protected Sub SaveProduce()
        ...
        If Me.Mode = Mode_Enum.Add Then
            R = Dao.PGGd_Add(Dt_Produce, Dt_WorkList, Dt_Charge)
        Else
            R = Dao.PGGd_Save(Dt_Produce, Dt_WorkList, Dt_Charge)
        End If
...
#Region "form 验证"
    ...
    '    'If Cb_BZC.IDAsInt = 0 AndAlso GHType = PGEnum_GH_Type.Normal Then
    ...
    '    'DP_JGSJ.Value = DP_JGSJ.Value.Date
    '    'DP_JGSJ.Value = DP_CGSJ.Value.Date
    '    'DP_sDate.Value = DP_sDate.Value.Date
...
#Region "获取新单号"
    ...
    Private Sub DP_KaiDan_Validated(ByVal sender As Object, ByVal e As System.EventArgs)
        GetNewID()
    End Sub
...
#Region "双击获取新编号"
    ...
    End Sub
#End Region
    'Private Sub LB_Remark_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    ShowRemarkList()
    'End Sub
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    'Private Sub Cmd_ChooseRemark_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    ShowRemarkList()
    'End Sub
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    'Private Sub LB_ProcessType_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
...
#Region "搜索色号"
    ...
    Private Sub Cb_BZC_Col_Sel(ByVal Col_No As String, ByVal Col_Name As String, ByVal ID As String)
        ...
        'CB_BZ.Enabled = True
        Dim msgBz As DtReturnMsg = Dao.PGGd_GetBZListByBzcID(Val(ID))
        'If msgBz.IsOk And msgBz.Dt.Rows.Count > 0 Then
        '    Dim msg As DtReturnMsg = Dao.PGGd_GetClientBZC(Val(ID), 0)
        ...
        Dim msgClient As DtReturnMsg = Dao.PGGd_GetClientByBzcID(Val(ID))
    ...
    '    Dim R As DtReturnMsg = Dao.PG_GetBZCPF(BZC_ID)
...
#Region "报表事件"
    ...
    Private Sub Report_PrintCompleted() Handles Report.PrintCompleted
        Dim R As DtReturnMsg = Dao.PG_GetState(TB_ID.Text)
...
#Region "作废，还原"
    ...
    Protected Sub DelProduce()
        Dim msg As MsgReturn = Dao.PGGd_Del(TB_ID.Text)
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub Cmd_Invalid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If Mode = Mode_Enum.Modify Then
    ...
    Protected Sub InvalidProduce()
        ...
        Dim msg As MsgReturn = Dao.PG_Invalid(TB_ID.Text)
        If msg.IsOk Then
    ...
    '    Dim R As DtReturnMsg = Dao.PGGd_GetById(TB_ID.Text)
    '    If R.IsOk AndAlso R.Dt.Rows(0)("State") < Enum_ProduceState.RanSe Then
    '        Dim msg As MsgReturn = Dao.PGGd_UpdateState(TB_ID.Text, Enum_ProduceState.AddNew)
    '        If msg.IsOk = False Then
    '            ShowErrMsg(msg.Msg)
    '        Else
    '            '  Me.Close()
...
#End Region
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    'Private Sub Radio_Num_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    If Radio_Num.Checked Then
...
#Region "增加布种"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    'Private Sub Cmd_AddBZ_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
...
#Region "布种修改名字"
    'Private Sub Cmd_BZName_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    ShowInput("修改布种[" & CB_BZ.Text & "]的名称:", AddressOf ChangeBZ_Name, TB_BZName.Text)
    'End Sub

    'Sub ChangeBZ_Name(ByVal s As String)
    '    If s <> "" Then
    '        Dim R As MsgReturn = Dao.PGBz_Rename(CB_BZ.IDAsInt, s)
    '        If R.IsOk Then
    '            CB_BZ.Text = CB_BZ.GetByTextBoxTag()
    '            TB_BZName.Text = CB_BZ.NameValue
    '        Else
    ...
    'End Sub
    '
    'Private Sub Cmd_Cmd_BZNo_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    ShowInput("修改布种[" & Cmd_BZNo.Text & "]的名称:", AddressOf ChangeBZNo, CB_BZ.Text)
    'End Sub

    'Sub ChangeBZNo(ByVal s As String)
    '    If s <> "" Then
    '        Dim R As MsgReturn = Dao.PGBz_No_Rename(CB_BZ.IDAsInt, s)
    '        If R.IsOk Then
    '            CB_BZ.Text = CB_BZ.GetByTextBoxTag()
    '            TB_BZName.Text = CB_BZ.NameValue
    '        Else
    ...
    'Private Sub Cmd_BZCName_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    '    ShowInput("修改色号[" & Cb_BZC.Text & "]的颜色名称:", AddressOf ChangeBZCName, TB_BZCName.Text)
    'End Sub

    'Sub ChangeBZCName(ByVal s As String)
    '    If s <> "" Then
    '        Dim R As MsgReturn = Dao.PGBzc_Rename(Cb_BZC.IDAsInt, s)
    '        If R.IsOk Then
    ...
    Private Sub CB_BZ_Col_Sel(ByVal Col_No As String, ByVal Col_Name As String, ByVal ID As String)
        'TB_BZName.Text = Col_Name

        'Dim msg As DtReturnMsg = Dao.PGGd_GetClientBZC(Cb_BZC.IDValue, ID)
        'If msg.IsOk And msg.Dt.Rows.Count > 0 Then
        ...
        If Mode = Mode_Enum.Add AndAlso TB_ClientID.Tag IsNot Nothing Then
            Dim R As DtReturnMsg = Dao.PG_GetClientRemark(TB_ClientID.Tag, ID)
            If R.IsOk AndAlso R.Dt.Rows.Count > 0 Then
...
#Region "复制"
    ...
    '    If Not GHType = PGEnum_GH_Type.Normal OrElse IsNull(Dt_Produce.Rows(0)("IsFG"), False) = True Then
...
#End Region
    Private Sub Cmd_ShowList_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
...
#End Region
    ...
    '    Dim R As MsgReturn
    '    GetWork_List()
    '    R = Dao.PGWorkList_Save(Dt_WorkList, Dt_Charge)
    '    If R.IsOk Then
    '        ShowOk(R.Msg, False)
    '    Else
    '        ShowErrMsg(R.Msg)
    ...
    '    Dim msgWork_Get As DtReturnMsg = Dao.PGWork_Get()
...
#Region "方法"
    ...
    '    Dim msgList As DtReturnMsg = Dao.PGList_GetById(id)
...
#Region "合缸"
    ...
    '    Dim R As MsgReturn
    '    If CB_GH.Text.EndsWith("R", StringComparison.CurrentCultureIgnoreCase) OrElse IsNumeric(CB_GH.Text.Substring(CB_GH.Text.Length - 1)) Then
    '        R = Dao.PG_HG(CB_GH.Text, TB_ID.Text)
    '    Else
    '        R = Dao.PG_HG(TB_ID.Text, CB_GH.Text)
    '    End If
    '    If R.IsOk Then
    '        ShowOk("合缸成功!")
    '        ReturnId = R.Msg
...
#Region "成品名"
    'Private Sub CB_CPName_Click(ByVal sender As Object, ByVal e As System.EventArgs)
    '    If CB_CPName.Checked Then
...
#End Region
    ...
    'Private Sub CheckKC()
    '    Dim Msg As MsgReturn = Dao.PGGET_BZID_Qty(CB_BZ.IDAsInt)
    '    If Msg.IsOk Then
...
Partial Class Dao
#Region "合缸"
    'Public Shared Function PG_HG(ByVal GH1 As String, ByVal GH2 As String) As MsgReturn
...
#Region "生产运转单"
    Public Const SQL_PGB_Get_WithName As String = "select P.*,C.Client_Name,BZ_Name ,BZC_Name,BZ_Spec from T30000_Produce_Gd P left join T10110_Client C on P.Client_No=C.Client_No left join T10002_BZ BZ on P.BZ_No=BZ.BZ_No  left join T10003_BZC BZC on P.BZC_No=BZC.BZC_No"

    Public Const SQL_PG_GetByID As String = "select * from T30000_Produce_Gd where GH=@GH"

    Public Const SQL_PG_GetByID_WithName As String = "select P.*,C.Client_Name,BZ_Name,BZC_Name,BZ_Spec  from T30000_Produce_Gd P left join T10110_Client C on P.Client_No=C.Client_No left join T10002_BZ BZ on P.BZ_No=BZ.BZ_No left join T10003_BZC BZC on P.BZC_No=BZC.BZC_No where GH=@GH "

    Public Const SQL_PG_GetEmptyRow As String = "select top 0  P.*,C.Client_Name,BZ_Name,BZC_Name  from T30000_Produce_Gd P left join T10110_Client C on P.Client_No=C.Client_No left join T10002_BZ BZ on P.BZ_No=BZ.BZ_No left join T10003_BZC BZC on P.BZC_No=BZC.BZC_No  "

    Public Const SQL_PGBz_Link_GetByID_Save As String = "select * from T10009_ProduceLinkBz where GH=@GH"

    Public Const SQL_PG_DelByid As String = "Delete from  T30000_Produce_Gd where GH=@GH "
    Public Const SQL_PGB_OrderBy As String = " order by  GH "



    Public Const SQL_PG_GetBZByBZCID As String = "select distinct( BZ.BZ_No),BZ_Name,BZ_FindHelper  from  (select BZ_No from T10009_BzcLinkBz where BZC_No=@BZC_No union all select BZ_No from T10003_BZC where BZC_No=@BZC_No) BL left join T10002_BZ  BZ on BZ.BZ_No=BL.BZ_No "
    Public Const SQL_PG_GetClientByBZCID As String = "select B.Client_No, Client_Name from T10003_BZC B left join T10110_Client C on  B.Client_No=C.Client_No where BZC_No=@BZC_No "

#End Region

#Region "排缸单跟单"
    Public Const PG_Gd_Name As String = "排缸单"
    Public Const SQL_PGGd_Get_WithName As String = "select P.*,Client_No, C.Client_Name,BZ_No,BZ_Name ,BZC_No,BZC_Name,BZ_Spec,E.Employee_name as GenDanName,(select Sum(SumAmount)SumAmount from T20310_LingLiao_Table where Produce_id = GH and state >-1 )SumAmount from T30000_Produce_Gd P left join T10110_Client C on P.Client_ID=C.ID left join T10002_BZ BZ on P.BZ_ID=BZ.ID left join T10003_BZC BZC on P.BZC_ID=BZC.ID left join T15001_Employee E on  P.GengDan=E.ID"

    Public Const SQL_PGGd_GetByID As String = "select Top 1 * from T30000_Produce_Gd where GH=@GH "
    Public Const SQL_PGGd_GetCGByID As String = "select Top 1 * from T30000_Produce_Gd where GH=@CG"

    Public Const SQL_PGGd_GetByID_WithName As String = "Select top 1 P.* ,Client_No, C.Client_Name,BZ_No,BZ_Name ,BZC_No,BZC_Name,BZ_Spec,E.Employee_Name as GengDanName  from  T30000_Produce_Gd P  left join T10110_Client C on P.Client_ID=C.ID left join T10002_BZ BZ on P.BZ_ID=BZ.ID left join T10003_BZC BZC on P.BZC_ID=BZC.ID left join T15001_Employee E on  P.GengDan =E.ID where P.GH=@GH "

    Public Const SQL_PGGd_GetEmptyRow As String = "Select top 0 P.*,Client_No, C.Client_Name,BZ_No,BZ_Name ,BZC_No,BZC_Name,BZ_Spec,E.Employee_Name as GengDanName  from T30000_Produce_Gd P left join T10110_Client C on P.Client_ID=C.ID left join T10002_BZ BZ on P.BZ_ID=BZ.ID left join T10003_BZC BZC on P.BZC_ID=BZC.ID left join T15001_Employee E on  P.GengDan =E.ID"


    Public Const SQL_PGGd_DelByid As String = "Delete from  T30000_Produce_Gd where GH=@GH "

    Public Const SQL_PGGd_UpdateState As String = "update  T30000_Produce_Gd set UPD_User=@UPD_User,UPD_Date=GetDate(),State=@State where GH=@GH "

    Public Const SQL_PGGd_OrderBy As String = " order by  GH "


    Public Const SQL_PGGd_GetBZByBZCID As String = "select  distinct( BZ_ID),Client_Bzc  from  (select BZ_ID,Client_Bzc from T10009_BzcLinkBz where BZC_ID=@BZC_ID union all select BZ_ID,Client_Bzc from T10003_BZC where ID=@BZC_ID) BL  "

    Public Const SQL_PGGd_GetClientByBZCID As String = "select B.Client_ID,Client_No, Client_Name from T10003_BZC B left join T10110_Client C on  B.Client_ID=C.ID where B.ID=@BZC_ID "
...
#Region "常量"
    Private Const PG_PBComplete As String = "  update T30000_Produce_Gd set state =@State, UPD_User=@UPD_User,UPD_Date=GetDate() where GH= @GH"
    Private Const PG_UpdRemark As String = "  update T30000_Produce_Gd set LRemark =@LRemark , UPD_User=@UPD_User,UPD_Date=GetDate() where GH= @GH"
    Public Const SQL_PGWorkList_GetListByID As String = "select WL.*,W.Dept_Name,W.Work_Name from T10024_WorkList WL left join T10022_Work W ON WL.Work_No=W.Work_No where Work_ID=@Work_ID order by WorkList_Index"
    Public Const SQL_PGWork_GetALL As String = "select * from T10022_Work "
    Public Const SQL_PGWorkListGetByGH As String = "Select * from T30003_Produce_Worklist where GH=@GH"
    Public Const SQL_PGWorkListByGH As String = "select WL.*,W.Dept_Name,W.Work_Name from T30003_Produce_Worklist WL left join T10022_Work W ON WL.Work_No=W.Work_No where GH=@GH order by WorkList_Index"
    Public Const SQL_PGWorkListGetCharge As String = "Select * from T30004_Produce_Charge where GH=@GH"
    Public Const SQL_PGWorkList_GetAll As String = "Select WorkList_No, ID,WorkList_Name from T10023_WorkTable "
    Public Const SQL_PGGetBZ_ID_Store As String = "SELECT COUNT(*) FROM T40510_PB_Stock WHERE BZ_ID = @BZ_ID  "
...
#Region "获取状态"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PG_GetState(ByVal GH As String) As DtReturnMsg
...
#Region "更新状态"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PG_Invalid(ByVal GH As String) As MsgReturn
        Dim R As New MsgReturn
        ...
        Using H As New DtHelper(S, P)
            If H.IsOk = True Then
                If H.DtList("G").Rows.Count > 0 Then
                    ...
                    Else
                        R.IsOk = False
                        R.Msg = "作废失败,缸号[" & GH & "]当前状态是[" & ComFun.GetProduceStateName(H.DtList("G").Rows(0)("State")) & "],不能作废!"
                    End If
                Else
                    R.IsOk = False
    ...
    Public Shared Function PG_UpdateRemark(ByVal _GH As String, ByVal Remark As String)
        Dim p As New Dictionary(Of String, Object)
    ...
    End Function
    Public Shared Function PG_UpdateState(ByVal _GH As String, ByVal _State As Enum_ProduceState)
        Dim p As New Dictionary(Of String, Object)
        p.Add("State", _State)
        p.Add("GH", _GH)
        p.Add("UPD_User", PClass.PClass.User_Name)
    ...
    Public Shared Function PG_UpdateState_RanSeGH(ByVal _GH As String, ByVal RanseGH As String, ByVal State As Enum_ProduceState, Optional ByVal UpdateState As Boolean = False)
        Dim p As New Dictionary(Of String, Object)
    ...
    Public Shared Function PG_UpdateState_ChuHuo(ByVal _GH As String, ByVal Date_ChuHuo As Date, ByVal State As Enum_ProduceState)
        Dim p As New Dictionary(Of String, Object)
...
#Region "运转单跟单"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GetConditionNames() As List(Of FindOption)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGStateName_GetConditionNames() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetAll() As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetConditionNames() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "请选择"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetByOptionString(ByVal s As String) As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetEmptyRow() As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_PGGd_GetEmptyRow)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetById(ByVal sId As String) As DtReturnMsg
        Dim para As New Dictionary(Of String, Object)
        'para.Add("@Line", line)
        para.Add("@GH", sId)
        Return PClass.PClass.SqlStrToDt(SQL_PGGd_GetByID_WithName, para)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_Add(ByVal Dt_Produce As DataTable, ByVal DT_WorkList As DataTable, ByVal Dt_Charge As DataTable) As MsgReturn
        ...
        sqlMap.Add("Produce", SQL_PGGd_GetByID)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_Save(ByVal Dt_Produce As DataTable, ByVal Dt_Worklist As DataTable, ByVal Dt_Charge As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim returnMsg As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        If Dt_Produce.Rows.Count <> 1 Then '检查参数
            returnMsg.IsOk = False
            returnMsg.Msg = "修改排缸单失败!"
            Return returnMsg
        End If
        paraMap.Add("@GH", Dt_Produce.Rows(0)("GH"))
        'paraMap.Add("@Line", Dt_Produce.Rows(0)("Line"))
        sqlMap.Add("Produce", SQL_PGGd_GetByID)
        'sqlMap.Add("WorkList", SQL_WorkListGetByGH)
        'sqlMap.Add("Charge", SQL_WorkListGetCharge)
        msg = SqlStrToDt(sqlMap, paraMap)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    'Public Shared Function PGWorkList_Save(ByVal Dt_Worklist As DataTable, ByVal Dt_Charge As DataTable) As MsgReturn
    '    Dim msg As New DtListReturnMsg
    '    Dim returnMsg As New MsgReturn
    '    Dim sqlMap As New Dictionary(Of String, String)
    '    Dim paraMap As New Dictionary(Of String, Object)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_Del(ByVal _GH As String)
        Return RunSQL(SQL_PGGd_DelByid, "@GH", _GH)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_UpdateState(ByVal _Gh As String, ByVal state As Enum_ProduceState)
        Dim para As New Dictionary(Of String, Object)
        para.Add("State", state)
        para.Add("@GH", _Gh)
        para.Add("UPD_User", PClass.PClass.User_Name)
        Return RunSQL(SQL_PGGd_UpdateState, para)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetNewID(ByVal D As Date) As RetrunNewIdMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetClientBZC(ByVal BZC_ID As String, ByVal BZ_ID As String) As DtReturnMsg
        Dim sqlBuider As New StringBuilder
        Dim P As New Dictionary(Of String, Object)
        P.Add("BZC_ID", BZC_ID)
        If Not BZ_ID = 0 Then
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetBZListByBzcID(ByVal bzcID As String) As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGd_GetClientByBzcID(ByVal bzcID As Integer) As DtReturnMsg

        Return PClass.PClass.SqlStrToDt(SQL_PGGd_GetClientByBZCID, "@BZC_ID", bzcID)
    End Function
#End Region
...
#Region "获取色办集合"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PG_GetBZCPF(ByVal BZc_ID As Integer) As DtReturnMsg
...
#Region "获取客户备注"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PG_GetClientRemark(ByVal Client_ID As Integer, ByVal BZ_ID As Integer) As DtReturnMsg
        Dim p As New Dictionary(Of String, Object)
...
#Region "修改布种名称"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGBz_Rename(ByVal BZ_ID As Integer, ByVal BZ_Name As String) As MsgReturn
        Dim p As New Dictionary(Of String, Object)
        p.Add("BZ_ID", BZ_ID)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGBz_No_Rename(ByVal BZ_ID As Integer, ByVal BZ_No As String) As MsgReturn
        Dim p As New Dictionary(Of String, Object)
        p.Add("BZ_ID", BZ_ID)
        p.Add("BZ_No", BZ_No)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGBzc_Rename(ByVal BZC_ID As Integer, ByVal BZC_Name As String) As MsgReturn
        Dim p As New Dictionary(Of String, Object)
        p.Add("BZC_ID", BZC_ID)
...
#Region "获取加工列表"
    ...
    Public Shared Function PGGet_AllWorkList() As DtReturnMsg
    ...
    Public Shared Function PGList_GetById(ByVal sId As String) As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGWork_Get() As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_PGWork_GetALL)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PG_Worklist(ByVal GH As String) As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PG_GetCharge(ByVal GH As String) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("@GH", GH)
...
#Region "获取指定布种库存"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PGGET_BZID_Qty(ByVal BZ_ID As Integer) As MsgReturn
