...
Public Class F30231_RetrunInform
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.RetrunInform_GetConditionNames()
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.RetrunInform_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.RetrunInform_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub

    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.RetrunInform_GetByOption(GetFindOtions)
        If msg.IsOk Then
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F30232_RetrunInform_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的" & Dao.InForm_DB_NAME)
            Exit Sub
        End If
        Dim F As New F30232_RetrunInform_Msg(FG1.SelectItem("ID"))
        With F
    ...
    '    If FG1.RowSel < 0 Then
    '        ShowErrMsg("请选择一个要修改的" & Dao.RetrunInform_DB_NAME)
    '        Exit Sub
    '    End If
    '    ShowConfirm("是否删除" & Dao.RetrunInform_DB_NAME & "[" & FG1.SelectItem("ID") & "]?", AddressOf DelRetrunInform)
    'End Sub
    ...
    '''' </summary>
    '''' <remarks></remarks>
    'Protected Sub DelRetrunInform()
    '    Dim msg As MsgReturn = Dao.RetrunInform_Del(FG1.SelectItem("ID"))
    '    If msg.IsOk Then
...
#Region "搜索工具栏"
    ...
    Private Sub CB_ConditionName2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_ConditionName2.SelectedIndexChanged
        ...
        Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_RetrunInform_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
        If msg.IsOk Then
...
#Region "审核状态"
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.InForm_Audited(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
...
Partial Friend Class Dao
    ...
    '===================采购单信息==============
    Public Const SQL_RetrunInform_Get_WithName = "select  * from T30231_RetrunInform P "
    Private Const SQL_RetrunInform_OrderBy As String = " order by sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function RetrunInform_GetConditionNames() As List(Of FindOption)
        ...
        'fo.DB_Field = " "
        'fo.Field_Operator = Enum_Operator.Operator_Like_Both
        'fo.SQL = "(P.ID %Like% or Supplier_Name  %Like% or ClientBill %Like% or SumQty %Like% or SumAmount %Like% or Remark %Like%)"
        'fo.Sign = "%Like%"
        'foList.Add(fo)
        Return foList
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function RetrunInform_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function RetrunInform_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_RetrunInform_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_RetrunInform_OrderBy)
