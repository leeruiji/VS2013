...
Public Class F30228_GHSQ_Msg
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        ComFun.DelBillNewID(BillType.GHSQ, Bill_ID)
    End Sub
    ...
    Protected Sub Me_Refresh()
        Dim msgTable As DtReturnMsg = Dao.GHSQ_GetById(Bill_ID)
        If msgTable.IsOk Then
            ...
            If Mode = Mode_Enum.Modify AndAlso msgTable.Dt.Rows.Count = 0 Then
                ShowErrMsg("没有找到" & Dao.GHSQ_DB_NAME & "[" & Bill_ID & "]", True)
                Exit Sub
            End If
            SetForm(dtTable)
        End If
        Select Case Mode
            Case Mode_Enum.Add
                If CheckRight(ID, ClassMain.Add) = False Then Me.ShowErrMsg("没有权限添加" & Dao.GHSQ_DB_NAME & "!", True) : Exit Sub
                Dim msg As DtReturnMsg = Dao.GHSQ_SelById(0)
            ...
            Case Mode_Enum.Modify
                ...
                Dim msg As DtReturnMsg = Dao.GHSQ_SelById(Bill_ID)
                If msg.IsOk Then
                    dtList = msg.Dt
...
#Region "表单信息"
    ...
    Protected Function GetForm() As DataTable
        ...
        For i As Integer = 1 To Fg1.Rows.Count - 1
            ...
            dr("ZL") = Fg1.Item(i, "ZL")
            'dr("CR_KeZhong") = Fg1.Item(i, "CR_KeZhong")
            'dr("CR_BianDuiBian") = Fg1.Item(i, "CR_BianDuiBian")
    ...
    Protected Sub SetForm(ByVal dt As DataTable)
        ...
        If dt.Rows.Count = 0 Then
            ...
            'Fg1.Item(i, "CR_KeZhong") = Dr("CR_KeZhong")
...
#Region "工具栏按钮"
    ...
    Private Sub Cmd_Modify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Modify.Click
        ...
        If CheckForm() Then
            If Mode = Mode_Enum.Add Then
                ShowConfirm("是否保存新" & Ch_Name & "?", AddressOf SaveGHSQ)
            Else
                If CheckIsModify() Then
                    ShowConfirm(Ch_Name & "[" & TB_ID.Text & "] 已经被其他人修改过,是否覆盖别人的修改?", AddressOf SaveGHSQ)
                Else
                    ShowConfirm("是否保存" & Ch_Name & "[" & TB_ID.Text & "] 的修改?", AddressOf SaveGHSQ)
                End If
            End If
        End If
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SaveGHSQ(Optional ByVal Audit As Boolean = False)
        Dim R As MsgReturn
        ...
        If Me.Mode = Mode_Enum.Add Then
            R = Dao.GHSQ_Add(Dt, dtList)
        Else
            R = Dao.GHSQ_Save(LId, Dt, dtList)
        End If
        If R.IsOk Then
            LId = TB_ID.Text
            ...
            If Audit Then
                Dim msg As MsgReturn = Dao.GHSQ_Audited(TB_ID.Text, True)
                If msg.IsOk Then
                    Bill_ID = TB_ID.Text
                    Me_Refresh()
                    ShowOk(msg.Msg)
    ...
    Protected Function CheckForm() As Boolean
        ...
        If TB_ID.Text = "" Then
            ...
            Return False
        End If
        If (Mode = Mode_Enum.Add Or (Mode = Mode_Enum.Modify And LId <> TB_ID.Text)) AndAlso Dao.GHSQ_CheckID(TB_ID.Text) Then
...
#Region "FG 事件"
    ...
    Private Sub Fg1_AfterEdit(ByVal sender As Object, ByVal e As C1.Win.C1FlexGrid.RowColEventArgs) Handles Fg1.AfterEdit
        If Fg1.ColSel = Fg1.Cols("GH").Index AndAlso Fg1.LastKey = Keys.Enter Then
            ...
            Dim msg As DtReturnMsg = Dao.BBGHSQ_GetById(gh)
    ...
    Sub SetProduce(ByVal dr As DataRow)
        ...
        If Not dr Is Nothing AndAlso Fg1.RowSel > 0 Then
            ...
            Fg1.Item(Fg1.RowSel, "CR_KeZhong") = dr("CR_KeZhong")

            Fg1.Item(Fg1.RowSel, "CR_BianDuiBian") = dr("CR_BianDuiBian")
    ...
    Private Sub Fg1_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Fg1.KeyDown
        ...
        If Fg1.CanEditing = True Then
            If e.KeyCode = Keys.Enter Then
                ...
                End If
            ElseIf e.KeyCode = Keys.Add AndAlso Fg1.Cols(Fg1.ColSel).Name = "GH" Then
                ShowGoodsSel(Fg1.Item(Fg1.RowSel, "GH"))
            End If
        End If
    End Sub

    Private Sub Fg1_KeyDownEdit(ByVal sender As Object, ByVal e As C1.Win.C1FlexGrid.KeyEditEventArgs) Handles Fg1.KeyDownEdit
        If e.KeyCode = Keys.Add AndAlso Fg1.Cols("GH").Index = e.Col AndAlso Fg1.CanEditing Then
            ShowGoodsSel(Fg1.Item(Fg1.RowSel, "GH"))
        End If
    End Sub
    ...
    Sub GetNewID()
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.GHSQ, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "审核状态"
    ...
    Protected Sub UnAudit()
        Dim msg As MsgReturn = Dao.GHSQ_Audited(TB_ID.Text, False)
        If msg.IsOk Then
            Me_Refresh()
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        ShowConfirm("是否删除" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf DelGHSQ)
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub DelGHSQ()
        Dim msg As MsgReturn = Dao.GHSQ_Del(TB_ID.Text)
        If msg.IsOk Then
            LastForm.ReturnId = TB_ID.Text
            Me.Close()
        Else
    ...
    Private Sub Cmd_SetInValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetInValid.Click
        ShowConfirm("是否作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf GHSQInValid)
    End Sub


    Sub GHSQInValid()
        Dim msg As MsgReturn = Dao.GHSQ_InValid(TB_ID.Text, True)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
    ...
    Private Sub Cmd_SetValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetValid.Click
        ShowConfirm("是否反作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf GHSQValid)
    End Sub
    ...
    ''' <remarks></remarks>
    Sub GHSQValid()
        Dim msg As MsgReturn = Dao.GHSQ_InValid(TB_ID.Text, False)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
...
#Region "检查其他人有没有修改过"
    ...
    Private Function CheckIsModify() As Boolean
        If Mode <> Mode_Enum.Add Then
            Return Dao.GHSQ_CheckIsModify(LId, LastLine)
        End If
    End Function
...
#Region "是否已付款"
    ...
    Private Sub SetPayment()
        Dim msg As MsgReturn = Dao.GHSQ_Payment(TB_ID.Text, True)
        If msg.IsOk Then
    ...
    Private Sub UnPayment()
        Dim msg As MsgReturn = Dao.GHSQ_Payment(TB_ID.Text, False)
        If msg.IsOk Then
            LastForm.ReturnId = Me.TB_ID.Text
            Bill_Id = TB_ID.Text
            Mode = Mode_Enum.Modify
...
#Region "常量"
    Protected Friend Const GHSQ_DB_NAME As String = "改黑申请单"
    ''' <summary>
    ''' 删除单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_GHSQ_DelByid As String = "Delete from T30227_GHSQ_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_GHSQ_GetByid As String = "select top 1 * from T30227_GHSQ_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    'Private Const SQL_GHSQ_SelByid As String = "select  * from T30225_GHSQ_Table  where ID=@ID"

    Private Const SQL_GHSQ_SelByid As String = "select BL.*,T.CR_BianDuiBian,T.CR_KeZhong,T.Client_Name,T.BZ_Name,T.BZC_Name,T.BZC_No,BT.sReason,BT.YFCS,BT.YFCS_ZG,BT.YFCS_Date,BT.CFCS,BT.CFCS_ZG,BT.CFCS_Date,BT.CBPS,BT.sReason_ZG,BT.sReason_Date from T30228_GHSQ_List BL left join T30227_GHSQ_Table BT ON BL.ID=BT.ID left join (select P.CR_BianDuiBian,P.CR_KeZhong,P.GH,P.Client_ID,P.BZ_ID,P.BZC_ID,BZ_Name,BZC_Name,BZC_No,Client_Name from  T30000_Produce_Gd P  left join T10110_Client C on P.Client_ID=C.ID left join T10002_BZ BZ on P.BZ_ID=BZ.ID left join T10003_BZC BZC on P.BZC_ID=BZC.ID ) T on T.GH=BL.GH  where BL.ID=@ID"

    ''' <summary>
    ''' 获取单头 用于作废和审核
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_GHSQ_GetStateByid As String = "select top 1 ID,State,State_User from T30227_GHSQ_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_GHSQ_GetListByid As String = "select * from T30228_GHSQ_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_GHSQ_SelListByid As String = "select BL.*,T.CR_BianDuiBian,T.CR_KeZhong,T.Client_Name,T.BZ_Name,T.BZC_Name,T.BZC_No from T30228_GHSQ_List BL left join (select P.CR_BianDuiBian,P.CR_KeZhong,P.GH,P.Client_ID,P.BZ_ID,P.BZC_ID,BZ_Name,BZC_Name,BZC_No,Client_Name from  T30000_Produce_Gd P  left join T10110_Client C on P.Client_ID=C.ID left join T10002_BZ BZ on P.BZ_ID=BZ.ID left join T10003_BZC BZC on P.BZC_ID=BZC.ID ) T on T.GH=BL.GH where BL.ID=@ID"

    'Private Const SQL_GHSQ_SelListByid As String = " select * from T30226_GHSQ_List BL  left join T10002_BZ BZ on BL.BZ_ID=BZ.ID where ID=@ID "
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_GHSQ_CheckID As String = "select * from T30227_GHSQ_Table  where ID=@ID"
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_GHSQ_CheckIsModify As String = "select count(*) from T30228_GHSQ_List  where @ID=ID and Line=@Line"
    ''' <summary>
    ''' 获取改黑申请单信息
    ''' </summary>
    ''' <remarks></remarks>
    Public Const SQL_BBGHSQ_GetByID_WithName As String = "select P.*,C.Client_Name,BZ.BZ_Name,BZC.BZC_Name,BZC.BZC_No,l.ZL from T30000_Produce_Gd P  left join T10110_Client C on P.Client_ID=C.ID left join T10002_BZ BZ on P.BZ_ID=BZ.ID left join T10003_BZC BZC on P.BZC_ID=BZC.ID left join T40101_PBRK_List L on p.GH=L.GH   where P.GH=@GH  "
...
#Region "单一张单内容查询"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_GHSQ_GetByid, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_SelListById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_GHSQ_SelListByid, "@ID", sId)
    End Function
#End Region
...
#Region "添加修改删除"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_CheckIsModify(ByVal ID As String, ByVal Line As Integer) As Boolean
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", ID)
        P.Add("Line", Line)
        Dim R As MsgReturn = SqlStrToOneStr(SQL_GHSQ_CheckIsModify, P)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return True
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_CheckID(ByVal ID As String) As Boolean
        Dim R As MsgReturn = SqlStrToOneStr(SQL_GHSQ_CheckID, "@ID", ID)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return False
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim GHSQId As String = dtTable.Rows(0)("ID")
        Dim IsInsert As Boolean = False
        paraMap.Add("ID", GHSQId)
        Try
            sqlMap.Add("Table", SQL_GHSQ_GetByid)
            sqlMap.Add("List", SQL_GHSQ_GetListByid)
            ...
            If msg.DtList("Table").Rows.Count = 0 Then
                ...
                Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & GHSQId & "'," & BillType.GHSQ & ")"
                DtToUpDate(msg, TmSQL)
                R.Msg = "" & GHSQ_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & GHSQ_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & GHSQ_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim GHSQId As String = dtTable.Rows(0)("ID")
        If LID <> GHSQId AndAlso GHSQ_CheckID(GHSQId) Then
            R.IsOk = False
            R.Msg = "" & GHSQ_DB_NAME & "[" & GHSQId & "]已经存在!请双击编号文本框,获取新编号!"
            Return R
        End If
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Audited Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & GHSQ_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被审核不能再修改!"
                    Return R
                End If
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Invoid Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & GHSQ_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
                    Return R
                End If
                DvToDt(dtList, msg.DtList("List"), New List(Of String))
                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & GHSQId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.GHSQ
                If LID <> GHSQId Then
                    DtToUpDate(msg, "Update T30227_GHSQ_Table set id='" & GHSQId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
                    dtTable.Rows(0)("ID") = GHSQId
                    R.Msg = "" & GHSQ_DB_NAME & "[" & LID & "]已经修改为[" & GHSQId & "]!"
                Else
                    DtToUpDate(msg)
                    R.Msg = "" & GHSQ_DB_NAME & "[" & LID & "]修改成功!"
                End If
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & GHSQ_DB_NAME & "[" & LID & "]不存在"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & GHSQ_DB_NAME & "[" & LID & "]修改错误"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' 删除日报表
    ''' </summary>
    ''' <param name="GHSQId"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_Del(ByVal GHSQId As String)
        Return RunSQL(SQL_GHSQ_DelByid, "@ID", GHSQId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        ...
        paraMap.Add("ID", _ID)
        Try
            sqlMap.Add("Table", SQL_GHSQ_GetStateByid)
            msg = SqlStrToDt(sqlMap, paraMap)
            If msg.DtList("Table").Rows.Count = 1 Then
                If msg.DtList("Table").Rows(0)("State") <> Enum_State.Audited Then
                    If msg.DtList("Table").Rows(0)("State") <> State Then
                        ...
                        DtToUpDate(msg)
                        R.IsOk = True
                        R.Msg = "" & GHSQ_DB_NAME & "[" & _ID & "]" & OStr & "成功!"
                    Else
                        R.IsOk = False
                        R.Msg = "" & GHSQ_DB_NAME & "[" & _ID & "]已经被" & OStr & "!不能再" & OStr & "!"
                    End If
                Else
                    R.IsOk = False
                    R.Msg = "" & GHSQ_DB_NAME & "[" & _ID & "]已经被审核,不能" & OStr & "!"
                End If
            Else
                R.IsOk = False
                R.Msg = "" & GHSQ_DB_NAME & "[" & _ID & "]不存在,不能" & OStr & "!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & GHSQ_DB_NAME & "[" & _ID & "]" & OStr & "错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        Dim R As MsgReturn
        ...
        Dim msg As DtReturnMsg = SqlStrToDt("select state from  T30227_GHSQ_Table where ID=@ID  ", "ID", _ID)
        If msg.IsOk AndAlso msg.Dt.Rows.Count > 0 AndAlso IsNull(msg.Dt.Rows(0)("State"), False) = state Then
            R = New MsgReturn
            R.Msg = GHSQ_DB_NAME & s & "失败！[" & _ID & "]已被" & s
            Return R
        End If

        R = SqlStrToOneStr("Update T30227_GHSQ_Table set State=@IsAudited,State_User=@State_User where ID=@ID  ", paraMap)

        If R.IsOk Then
            R.Msg = GHSQ_DB_NAME & s & "成功！"
        Else
            R.Msg = GHSQ_DB_NAME & s & "失败！" & R.Msg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function GHSQ_Payment(ByVal _ID As String, ByVal IsPayment As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("ID", _ID)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BBGHSQ_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_BBGHSQ_GetByID_WithName, "@GH", sId)
    End Function
