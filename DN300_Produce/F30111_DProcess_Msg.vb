...
Public Class F30111_DProcess_Msg
    Dim line As Integer
...
#Region "窗口定义"
    ...
    Protected Sub Me_Refresh(ByVal GH As String, ByVal Line As String)
        Dim msg As DtReturnMsg = Dao.Get_DProcess(GH, Line)
        If msg.IsOk Then
            dtList = msg.Dt
            SetForm(msg.Dt)
        End If
...
#Region "表单信息"
    ...
    Protected Sub SetForm(ByVal dt As DataTable)
        ...
        If dt.Rows.Count = 1 Then
            ...
            End If
            TB_Dqkg.Text = Dr("Dqkg")
            TB_Dhkg.Text = Dr("Dhkg")
            TB_SXCKg.Text = IsNull(Dr("SXCKg"), "")
            TB_Jijia.Text = Dr("Jijia")
            TB_Shangchao.Text = Dr("Shangchao")
            TB_Xiachao.Text = Dr("Xiachao")
            TB_Wendu.Text = Dr("Wendu")
            TB_Chesu.Text = Dr("Chesu")
            TB_Fengsu.Text = Dr("Fengsu")
            TB_Peifang.Text = Dr("Peifang")
            CB_JTH.Text = IsNull(Dr("JTH"), "")
            TB_W.Text = Dr("W")
            TB_L.Text = Dr("L")
            TB_N.Text = Dr("N")
            TB_xihao.Text = Dr("xihao")
            CB_Remark.Text = IsNull(Dr("Work_Name"), "")
            CB_JiZheng.Text = Dr("JiZheng")
            TB_DxCount.Text = FormatNum(IsNull(Dr("DxCount"), 0))
            TB_DSZL.Text = FormatNum(IsNull(Dr("DSZL"), ""))
            If IsNull(Dr("Bengci"), False) = True Then
    ...
    Protected Function GetForm() As DataTable
        ...
        If Not dt Is Nothing Then
            ...
            dr("GH") = TB_GH2.Text
            dr("Dqkg") = TB_Dqkg.Text
            dr("Dhkg") = TB_Dhkg.Text
            dr("SXCKg") = TB_SXCKg.Text
            dr("Jijia") = Val(TB_Jijia.Text)
            dr("Shangchao") = Val(TB_Shangchao.Text)
            dr("Xiachao") = Val(TB_Xiachao.Text)
            dr("Wendu") = Val(TB_Wendu.Text)
            dr("Chesu") = Val(TB_Chesu.Text)
            dr("Fengsu") = Val(TB_Fengsu.Text)
            dr("Peifang") = TB_Peifang.Text
            dr("W") = TB_W.Text
            dr("L") = TB_L.Text
            dr("N") = TB_N.Text
            dr("xihao") = TB_xihao.Text
            dr("Remark") = CB_Remark.SelectedValue
            dr("sDate") = DTP_Date.Text
            dr("JiZheng") = CB_JiZheng.Text
            dr("Bengci") = RB_morning.Checked
            dr("Yqkg") = TB_Yckg.Text
            dr("DxCount") = Val(TB_DxCount.Text)
            dr("isInvoid") = LabelState.Visible
            dr("DSZL") = Val(TB_DSZL.Text)
            dr("JTH") = CB_JTH.Text
            dt.Rows.Add(dr)
        End If
        Return dt
...
#Region "工具栏按钮"
    ...
    Protected Sub SaveDProcess(Optional ByVal Retrun As Boolean = False)
        ...
        If Me.Mode = Mode_Enum.Add Then
            R = Dao.DProcess_Add(Dt, stat)
        Else
            R = Dao.DProcess_Save(Dt, line, stat)
        End If
        If R.IsOk Then
    ...
    Private Sub CB_Remark_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_Remark.SelectedIndexChanged '备注选中预定时状态选中预定
        If CB_Remark.Text = "预定" AndAlso RB_YuDing.Enabled = True Then
...
#Region "选缸号"
    ...
    Sub Get_GH(ByVal id As String)
        ...
        TB_DxCount.Text = TB_CR_LuoSeBzCount.Text
        ...
        TB_DSZL.Text = IsNull(Dr("PB_ZLSum"), 0)
        BZC_No.Text = IsNull(Dr("BZC_No"), "")
        BZC_Name.Text = IsNull(Dr("BZC_Name"), "")
        TB_Yckg.Text = getYckg(IsNull(Dr("CR_ShiYong"), ""), IsNull(Dr("CR_BianDuiBian"), ""), IsNull(Dr("CR_KeZhong"), ""))
        ...
        End If
        GHState()
        CB_JiZheng.DataSource = Dao.Get_Getjizheng.Dt
...
Partial Friend Class Dao
    ...
    Private Const SQL_DProcess_GetGH As String = "select top 1 * from T30110_DProcess where GH=@GH"
    Private Const SQL_DProcess_GetGHID As String = "select top 1 * from T30110_DProcess where GH=@GH and Line=@Line"
    Private Const SQL_DProcess_Getjizheng As String = " SELECT Employee_No,Employee_Name FROM T15001_Employee WHERE (Employee_FileType = '正式') AND (Employee_Dept = 'D006002') AND (Employee_Job = 19) OR  (Employee_FileType = '正式') AND (Employee_Dept LIKE 'D006%') AND ( Employee_Job = 3)"

    Public Shared Function Get_DProcess(ByVal GH As String, ByVal Line As Integer) As DtReturnMsg
    ...
    Public Shared Function Get_Getjizheng() As DtReturnMsg
        Return SqlStrToDt(SQL_DProcess_Getjizheng)
    End Function

    Public Shared Function DProcess_Add(ByVal dt As DataTable, ByVal stat As Enum_ProduceState) As MsgReturn
        ...
        If dt.Rows.Count <> 1 Then
            ...
            returnMsg.Msg = "定型工艺添加失败!"
            Return returnMsg
        End If
        GH = dt.Rows(0)("GH")

        Dim SQL As New Dictionary(Of String, String)
        SQL.Add("T", SQL_DProcess_GetGH)
        Dim p As New Dictionary(Of String, Object)
        p.Add("@GH", GH)
        ...
        Using H As New DtHelper(SQL, p)
            ...
            End If
            If returnMsg.IsOk = True And stat = 0 Then
                returnMsg.Msg = "定型工艺[" & dt.Rows(0)("GH") & "]添加成功!"

            ElseIf returnMsg.IsOk = True And stat > 0 Then
                returnMsg.Msg = "定型工艺[" & dt.Rows(0)("GH") & "]添加成功!缸号状态设置为" & ComFun.GetProduceStateName(stat)
    ...
    Public Shared Function DProcess_Save(ByVal dt As DataTable, ByVal id As Integer, ByVal stat As Enum_ProduceState) As MsgReturn
        Dim Cnn As New SqlClient.SqlConnection
        Dim Da As New SqlClient.SqlDataAdapter
        Dim GH As String = ""
        ...
        P.Add("UPD_User", User_Name)
        Dim SQL As New Dictionary(Of String, String)
        SQL.Add("T", SQL_DProcess_GetGHID)
        Using H As New DtHelper(SQL, P)
            ...
            If H.IsOk AndAlso H.DtList("T").Rows.Count = 1 Then
                ...
                End If
                If returnMsg.IsOk = True And stat = 0 Then
                    returnMsg.Msg = "定型工艺[" & dt.Rows(0)("GH") & "]修改成功!"
            ...
            Else
                returnMsg.IsOk = False
                returnMsg.Msg = "定型工艺[" & dt.Rows(0)("GH") & "已经存在!"
            End If
        End Using
        Return returnMsg
    End Function
