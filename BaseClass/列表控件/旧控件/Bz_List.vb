...
Public Class Bz_List
    ...
    Overrides Sub GetSearch()
        ...
        Search = StrConv(Search, vbNarrow)
        ...
        Select Case _SearchType
            Case ENum_SearchType.ALL '全部模式
                ...
                End If
                OrderStr = SQL_BZ_GetSearch_Order
            ...
            Case ENum_SearchType.Client
                ...
                If NotTop Then
                    ...
                    GS = GS.Replace("top 10", "")
                End If
                OrderStr = SQL_BZ_GetSearch_Client_Order
    ...
    '    Public Event Bz_Sel(ByVal BZ_No As String, ByVal Bz_Name As String, ByVal ID As String)
    '    Public Event GetSearchEvent(ByRef BZ_No As String, ByRef ID As String, ByVal Cancel As Boolean)
    ...
    '    Private Sub Bz_List_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.GotFocus
    '        Fg1.Focus()
    '    End Sub
    ...
    '    Public Sub BZ_SelOne()
    '        If Fg1.RowSel > 0 And Me.Fg1.Focused = False Then
    '            RaiseEvent Bz_Sel(Fg1.Item(Fg1.RowSel, "BZ_No"), Fg1.Item(Fg1.RowSel, "BZ_Name"), Fg1.Item(Fg1.RowSel, "ID"))
    '        End If
    '    End Sub
    ...
    '    ''' 把控件事件绑定到TextBox上
    '    ''' </summary>
    '    ''' <param name="TB_BZ_No"></param>
    '    ''' <param name="TB_BZ_Name"></param>
    '    ''' <param name="NextControl">下一个控件</param>
    '    ''' <remarks></remarks>
    '    Public Sub Set_TextBox(ByVal TB_BZ_No As TextBox, ByVal TB_BZ_Name As Windows.Forms.Control, ByVal NextControl As Windows.Forms.Control)
    '        Me.TB_No = TB_BZ_No
    '        Me.TB_Name = TB_BZ_Name
    '        Me.NextControl = NextControl
    '        AddHandler Me.Bz_Sel, AddressOf Bz_List2_Bz_Sel
    '        AddHandler Me.GetFocusState, AddressOf Bz_List2_GetFocusState
    '        AddHandler Me.GetSearchEvent, AddressOf Bz_List2_GetSearchEvent
    ...
    '            Dim Dtr As DtReturnMsg = SqlStrToDt(SQL_BZ_GetSearch_GetByID, "@ID", ID)
    '            If Dtr.IsOk Then
    '                If Dtr.Dt.Rows.Count > 0 Then
    '                    TB_No.Text = Dtr.Dt.Rows(0)("BZ_No")
    '                    TB_No.Tag = Dtr.Dt.Rows(0)("ID")
    '                    If Not TB_Name Is Nothing Then TB_Name.Text = Dtr.Dt.Rows(0)("BZ_Name")
    '                    DtSetFG(Dtr.Dt)
    '                    Return Dtr.Dt.Rows(0)("BZ_Name")
    '                Else
    '                    Return ""
    '                End If
    '            Else
    ...
    '    Private Sub TB_BZ_No_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TB_No.GotFocus
    '        TB_LastKeys = 0
    '        If TB_No.ReadOnly = False Then
    '            TextBox_Show_Bz_List2()
    '        End If
    '    End Sub

    '    Private Sub TB_BZ_No_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TB_No.KeyDown
    ...
    '    Private Sub TB_BZ_No_KeyPress(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles TB_No.KeyPress
    '        If TB_No.ReadOnly = True OrElse TB_No.Enabled = False Then
    '            Me.Visible = False
    '            Exit Sub
    '        End If
    ...
    '    Private Sub TB_BZ_No_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles TB_No.LostFocus
    '        If TB_No.ReadOnly = True OrElse TB_No.Enabled = False Then
    '            Me.Visible = False
    '            Exit Sub
    '        End If
    ...
    '    Private Sub Bz_List2_Bz_Sel(ByVal BZ_No As String, ByVal Bz_Name As String, ByVal ID As String)
    '        TB_No.Text = BZ_No
    '        TB_No.Tag = ID
    '        TB_Name.Text = Bz_Name
    '        If Bz_Name <> "" Then
    '            If Not NextControl Is Nothing AndAlso TB_LastKeys = Keys.Enter Then NextControl.Focus()
    '        End If
    '    End Sub

    '    Private Sub Bz_List2_GetFocusState(ByRef HasFocused As Boolean)
    ...
    '    Private Sub Bz_List2_GetSearchEvent(ByRef BZ_No As String, ByRef ID As String, ByVal Cancel As Boolean)
    '        BZ_No = TB_No.Text
    '        ID = TB_No.Tag
    '    End Sub
    '#End Region
    ...
    '    Private Col_BZ_No As String = "BZ_No"
    '    Private Col_BZ_Name As String = "BZ_Name"
    '    Private Col_BZ_ID As String = "BZ_ID"
    '    ''' <summary>
    '    ''' 把控件事件绑定到FG上
    '    ''' </summary>
    '    ''' <param name="FGX"></param>
    '    ''' <param name="Col_BZ_No">FG上的字段</param>
    '    ''' <param name="Col_BZ_Name">FG上的字段</param>
    '    ''' <remarks></remarks>
    '    Public Sub Set_FG(ByVal FGX As PClass.FG, Optional ByVal Col_BZ_No As String = "BZ_No", Optional ByVal Col_BZ_Name As String = "BZ_Name", Optional ByVal Col_BZ_ID As String = "BZ_ID")
    '        Me.Col_BZ_No = Col_BZ_No
    '        Me.Col_BZ_Name = Col_BZ_Name
    '        Me.Col_BZ_ID = Col_BZ_ID
    '        Me.FGX = FGX
    '        AddHandler Me.Bz_Sel, AddressOf Me_BZ_Sel
    ...
    '        If e.Col = FGX.Cols(Col_BZ_No).Index Then
    '            Me.StartSearch()
    '            If FGX.LastText <> IsNull(FGX.Editor.Text, "") Then FGX.Item(e.Row, Col_BZ_Name) = ""
    '        End If
    '    End Sub
    '    Sub Show_BZ_List()
    '        If FGX.ColSel = FGX.Cols(Col_BZ_No).Index Then
    ...
    '    Private Sub Me_BZ_Sel(ByVal BZ_No As String, ByVal BZ_Name As String, ByVal Id As String)
    '        FGX.Item(FGX.RowSel, Col_BZ_No) = BZ_No
    '        FGX.Item(FGX.RowSel, Col_BZ_Name) = BZ_Name
    '        FGX.Item(FGX.RowSel, Col_BZ_ID) = Id
    '        If BZ_Name <> "" Then
    '            FGX.GotoNextCell(Col_BZ_No)
    '        End If
    '    End Sub
    ...
    '    Private Sub Me_GetSearchEvent(ByRef BZ_No As String, ByRef ID As String, ByVal Cancel As Boolean)
    '        If FGX.RowSel > 0 Then
    '            If FGX.Editor Is Nothing Then
    '                BZ_No = IsNull(FGX.Item(FGX.RowSel, Col_BZ_No), "")
    '            Else
    '                BZ_No = FGX.Editor.Text
    '            End If
    '            ID = IsNull(FGX.Item(FGX.RowSel, Col_BZ_ID), "")
    '        Else
    '            Cancel = True
    '        End If
    '    End Sub
