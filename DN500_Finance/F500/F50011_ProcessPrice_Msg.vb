...
Public Class F50011_ProcessPrice_Msg
    Dim LId As String = ""
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        ComFun.DelBillNewID(BillType.ProcessPrice, Bill_ID)
    End Sub

    Private Sub Form_Load() Handles Me.Me_Load
        FormCheckRight()
        Ch_Name = GetBillTypeName(ID)
        Dao.Process_DB_NAME = Ch_Name
        Me.Title = Ch_Name
        ...
        FG1.Cols("ProcessSort").Editor = CB_Sort
        CB_Sort.DataSource = Dao.Process_GetSort.Dt
        ''业务员
    ...
    Protected Sub Me_Refresh()
        Dim msgTable As DtReturnMsg = Dao.Process_GetById(Bill_ID)
        ...
        Select Case Mode
            Case Mode_Enum.Add
                If CheckRight(ID, ClassMain.Add) = False Then Me.ShowErrMsg("没有权限添加" & Ch_Name & "!", True) : Exit Sub
                Dim msg As DtReturnMsg = Dao.Process_SelListById_All(0)
            ...
            Case Mode_Enum.Modify
                ...
                Dim msg As DtReturnMsg = Dao.Process_SelListById_All(Bill_ID)
            ...
            Case 0
                ...
                For i As Integer = 1 To FG1.Rows.Count - 1
                    If FG1.Item(i, "IsComfirm") = False AndAlso FG1.Item(i, "IsLastPrice") = True Then
                        If dtList.Select("BZ_ID=" & FG1.Item(i, "BZ_ID") & " and IsComfirm=1").Length = 0 Then
...
#Region "表单信息"
    ...
    Protected Function GetForm(Optional ByVal isPrice As Boolean = False) As DataTable
        ...
        For i As Integer = 1 To FG1.Rows.Count - 1
            ...
            dr("ProcessSort") = FG1.Item(i, "ProcessSort")
            If Mode = Mode_Enum.Add Then
                dr("IsLastPrice") = True
                dr("Price_Time") = serverTiume
...
#Region "工具栏按钮"
    ...
    Protected Sub SaveProcess(Optional ByVal IsProcess As Boolean = False)
        ...
        If Me.Mode = Mode_Enum.Add Then
            R = Dao.Process_Add(Dt, dtList)
        Else
            R = Dao.Process_Save(LId, Dt, dtList)
        End If
        If R.IsOk Then
            LId = TB_ID.Text
...
#End Region
    ...
    Private Sub Fg1_NextCell(ByVal e As PClass.FG.NextCellEventArgs) Handles FG1.NextCell
        Select Case e.Col
            ...
            Case "BZ_No"
                e.ToCol = FG1.Cols("ProcessSort").Index
...
#Region "获取新单号"
    ...
    Sub GetNewID()
        ...
        If TB_ID.ReadOnly = False Then
            ...
            Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.ProcessPrice, DTP_sDate.Value, Bill_ID, TB_Client_Name.Tag, 4)
            If R.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType, ByVal UComfirm As Boolean)
        ...
        Dim R As New R50010_ProcessPrice
...
#Region "审核状态"
    ...
    Protected Sub UnAudit()
        Dao.Price_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.Process_Audited(TB_ID.Text, False)
        If msg.IsOk Then
            ShowOk(msg.Msg)
            Me_Refresh()
    ...
    Protected Sub DelProcess()
        Dao.Price_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.Process_Del(TB_ID.Text)
        If msg.IsOk Then
            LastForm.ReturnId = TB_ID.Text
            Me.Close()
        Else
    ...
    Sub ProcessInValid()
        Dao.Price_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.Process_InValid(TB_ID.Text, True)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
    ...
    Sub ProcessValid()
        Dao.Price_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.Process_InValid(TB_ID.Text, False)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
...
Partial Friend Class Dao
    ...
    Private Const SQL_Process_GetSort As String = "Select ProcessSort,Price from T50020_ProcessSort "
    Public Shared Function Process_GetSort() As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_Process_GetSort)
    End Function
...
#Region "常量"
    Protected Friend Shared Process_DB_NAME As String = "报价单"
    ''' <summary>
    ''' 删除单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Process_DelByid As String = "Delete from T50010_ProcessPrice_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Process_GetByid As String = "select top 1 * from T50010_ProcessPrice_Table  where ID=@ID "

    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Process_GetByid_WithName As String = "select top 1 T. * ,C.Client_No,C.Client_Name from T50010_ProcessPrice_Table T Left join T10110_Client C on T.Client_ID=C.ID  where T.ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Process_SelByid As String = " select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name ,C.Client_Name ,C.Client_No" & _
                                                   " from T50010_ProcessPrice_Table S " & _
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Process_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T50010_ProcessPrice_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Process_GetListByid As String = "select * from T50011_ProcessPrice_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_Process_SelListByid As String = " select L.*,BZ_No,BZ_Name " & _
                                                            "  from  T50011_ProcessPrice_List L   " & _
    ...
    ''' <remarks></remarks>0
    ''' 
    Private Const SQL_Process_SelListByid_ShowComfirm As String = " select L.*,BZ_No,BZ_Name " & _
                                                          "  from  T50011_ProcessPrice_List L   " & _
                                                          "   left join T10002_BZ BZ on L.BZ_ID=BZ.ID " & _
    ...
    Private Const SQL_Process_SelListByid_ALL As String = " select L.*,BZ_No,BZ_Name " & _
                                                         "  from  T50011_ProcessPrice_List L   " & _
                                                         "   left join T10002_BZ BZ on L.BZ_ID=BZ.ID " & _
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Process_CheckID As String = "select count(*) from T50010_ProcessPrice_Table  where ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Process_CheckIsModify As String = "select count(*) from T50011_ProcessPrice_List where ID=@ID and Line=@Line"
...
#Region "单一张单内容查询"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_Process_GetByid_WithName, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_SelListById(ByVal sId As String, ByVal UnComfirm As Boolean) As DtReturnMsg
        Dim p As New Dictionary(Of String, Object)
        p.Add("ID", sId)

        If UnComfirm Then
            Return PClass.PClass.SqlStrToDt(SQL_Process_SelListByid_ShowComfirm, p)
        Else
            Return PClass.PClass.SqlStrToDt(SQL_Process_SelListByid_ALL, p)
        End If
...
#Region "添加修改删除"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_CheckIsModify(ByVal ID As String, ByVal Line As Integer) As Boolean
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", ID)
        P.Add("Line", Line)
        Dim R As MsgReturn = SqlStrToOneStr(SQL_Process_CheckIsModify, P)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return True
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_CheckID(ByVal ID As String) As Boolean
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        ...
        Try
            ...
            sqlMap.Add("Table", SQL_Process_GetByid)
            sqlMap.Add("List", SQL_Process_GetListByid)
            ...
            If msg.DtList("Table").Rows.Count = 0 Then
                ...
                Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & ProcessId & "'," & BillType.ProcessPrice & ")"
                DtToUpDate(msg, TmSQL)
                R.Msg = "" & Process_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & Process_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & Process_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        ...
        If LID <> ProcessId AndAlso Process_CheckID(ProcessId) Then
            R.IsOk = False
            R.Msg = "" & Process_DB_NAME & "[" & ProcessId & "]已经存在!请双击编号文本框,获取新编号!"
            Return R
        End If
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Audited Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & Process_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被审核不能再修改!"
                    Return R
                End If
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Invoid Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & Process_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
                    Return R
                End If
                DvToDt(dtList, msg.DtList("List"), New List(Of String))
                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & ProcessId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.ProcessPrice
                If LID <> ProcessId Then
                    DtToUpDate(msg, "Update T50010_ProcessPrice_Table set id='" & ProcessId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
                    dtTable.Rows(0)("ID") = ProcessId
                    R.Msg = "" & Process_DB_NAME & "[" & LID & "]已经修改为[" & ProcessId & "]!"
                Else
                    DtToUpDate(msg)
                    R.Msg = "" & Process_DB_NAME & "[" & LID & "]修改成功!"
                End If
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & Process_DB_NAME & "[" & LID & "]不存在"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & Process_DB_NAME & "[" & LID & "]修改错误"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_Del(ByVal ProcessId As String)
        Return RunSQL(SQL_Process_DelByid, "@ID", ProcessId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("ID", _ID)
        paraMap.Add("State", IIf(IsFei, -1, 0))
        paraMap.Add("State_User", User_Display)

        Dim R As MsgReturn = SqlStrToOneStr("Update T50010_ProcessPrice_Table set State=@State,State_User=@State_User where ID=@ID  ", paraMap)
        Dim s As String = IIf(IsFei, "作废", "还原")
        If R.IsOk Then
            R.Msg = Process_DB_NAME & s & "成功！"
        Else
            R.Msg = Process_DB_NAME & s & "失败！" & R.Msg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        Dim R As MsgReturn
        ...
        '判断状态
        Dim msg As DtReturnMsg = SqlStrToDt("select state from  T50010_ProcessPrice_Table where ID=@ID  ", "ID", _ID)
        If msg.IsOk AndAlso msg.Dt.Rows.Count > 0 AndAlso IsNull(msg.Dt.Rows(0)("State"), False) = state Then
            R = New MsgReturn
            R.Msg = Process_DB_NAME & s & "失败！[" & _ID & "]已被" & s
            Return R
        End If

        R = SqlStrToOneStr("Update T50010_ProcessPrice_Table set State=@IsAudited,State_User=@State_User where ID=@ID  ", paraMap)
...
#End Region
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Shared Function Process_SelListById_All(ByVal sId As String) As DtReturnMsg
...
#Region "根据色号获取染部配方成本"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_GetPFCost_ByBZC(ByVal BZC_ID As Integer, ByVal BZ_ID As Integer) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("BZC_ID", BZC_ID)
        P.Add("BZ_ID", BZ_ID)
