...
Public Class F50000_Price
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.Price_GetConditionNames()
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.Price_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.InnerPrice_GetCondition3()
    ...
    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.Price_GetByOption(GetFindOtions)
        If msg.IsOk Then
...
#Region "控件事件"
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的" & Dao.Price_DB_NAME)
            Exit Sub
        End If
        Dim F As New F50001_Price_Msg(FG1.Item(FG1.RowSel, "ID"))
        With F
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Dao.Price_DB_NAME & "[" & FG1.Item(FG1.RowSel, "ID") & "]?", AddressOf DelPrice)
    End Sub
    ...
    Protected Sub DelPrice()
        Dim msg As MsgReturn = Dao.Price_Del(FG1.Item(FG1.RowSel, "ID"))
        If msg.IsOk Then
...
#Region "搜索工具栏"
    Private Sub Cb_Client_Col_Sel(ByVal Col_No As System.String, ByVal Col_Name As System.String, ByVal ID As System.String) Handles Cb_Client.Col_Sel
        CB_BZC.SearchType = cSearchType.ENum_SearchType.Client
        CB_BZC.SearchID = Cb_Client.IDValue
    End Sub


    Private Sub Cb_Client_SetEmpty() Handles Cb_Client.SetEmpty
        CB_BZC.SearchType = cSearchType.ENum_SearchType.ALL
        CB_BZC.SearchID = 0
    End Sub

   

    Private Sub CB_BZC_Col_Sel(ByVal Col_No As System.String, ByVal Col_Name As System.String, ByVal ID As System.String) Handles CB_BZC.Col_Sel
        Lable_color.Text = Col_Name
        Focus()
    End Sub

    'Private Sub CB_BZC_SetEmpty() Handles Cb_Client.SetEmpty
    '    CB_BZC.SearchType = cSearchType.ENum_SearchType.ALL
    '    CB_BZC.SearchID = 0
    'End Sub

    Private Sub CB_BZC_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles CB_BZC.TextChanged
        Lable_color.Text = ""
    End Sub
    ...
    Private Sub CB_ConditionName2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_ConditionName2.SelectedIndexChanged
        ...
        Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_Price_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
        If msg.IsOk Then
    ...
    Protected Function GetFindOtions() As OptionList
        ...
        If Val(CB_BZC.IDValue) = 0 AndAlso Val(Cb_Client.IDValue) = 0 Then
            fo = New FindOption
            fo.DB_Field = "sDate"
        ...
        If Val(CB_BZC.IDValue) <> 0 Then
            fo = New FindOption
            fo.DB_Field = "BZC_ID"
            fo.Value = Val(CB_BZC.IDValue)
            fo.Field_Operator = Enum_Operator.Operator_Equal
            'fo.Field_Operator = Enum_Operator.Operator_Like_Both
            'fo.SQL = "exists(select TL.BZC_ID from T10003_BZC TW,T50001_Price_List TL where %BZC_ID% and TL.BZC_ID=TW.ID and T.id=TL.id) "
            fo.SQL = "exists(select TL.BZC_ID from T50001_Price_List TL where %BZC_ID% and  T.id=TL.id) "
            fo.Sign = "%BZC_ID%"
            oList.FoList.Add(fo)
        End If
...
#Region "报表事件"
    ...
    End Sub
    Private Sub Cmd_PreviewC_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Preview_Un.Click
    ...
    Private Sub Cmd_Client_List_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Client_List.Click
        PrintClient(False)
    End Sub
    Private Sub Cmd_Client_List_CK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Client_List_CK.Click
        PrintClient(True)
    End Sub
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType, ByVal CK As Boolean, ByVal UComfirm As Boolean)
        ...
        R.StartList(FG1.Item(FG1.RowSel, "ID"), DoOperator, CK, UComfirm)
    End Sub

    Protected Sub PrintClient(ByVal ck As Boolean)
        If Cb_Client.Text = "" Then
            ShowErrMsg("请选择一个客户")
            Exit Sub
        End If
        Dim R As New R50000_Price_Client
        R.StartList(Cb_Client.IDAsInt, Cb_Client.GetByTextBoxTag, DP_Start.Value, DP_End.Value, ck)
    End Sub
...
#Region "审核状态"
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.Price_Audited(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
            Else
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.Price_SelListById(IsNull(FG1.Item(FG1.RowSel, "ID"), ""), False)
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
            Fg2.RowSetForce("BZC_ID", CB_BZC.IDAsInt)
        Else
        End If
    End Sub
...
#End Region
    ...
    'Private Sub CB_BZC_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles CB_BZC.KeyDown
    ...
    Private Sub CB_BZC_KeyPress(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyPressEventArgs) Handles CB_BZC.KeyPress
        If e.KeyChar = Microsoft.VisualBasic.ChrW(13) Then
            Me.Me_Refresh()
        End If
    End Sub

    Private Sub F50000_Price_RightToLeftChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.RightToLeftChanged
...
Partial Friend Class Dao
    ...
    '===================出库单信息==============
    Public Const SQL_InnerPrice_Get_WithName = "select T.*,C.Client_Name from T50000_InnerPrice_Table T left join T10110_Client C on T.Client_ID=C.ID  "

    Public Const SQL_InnerPrice_OrderBy As String = " order by sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_GetConditionNames() As List(Of FindOption)
        ...
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "exists(select TW.BZC_Name from T10003_BZC TW,T50001_InnerPrice_List TL where %BZC_Name% and TL.BZC_ID=TW.ID and T.id=TL.id)"
        fo.Sign = "%BZC_Name%"
        foList.Add(fo)
        ...
        fo.DB_Field = "BZ_Name"
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "exists(select TW.BZ_Name from T10002_BZ TW,T50001_InnerPrice_List TL where %BZ_Name% and TL.BZ_ID=TW.ID and T.id=TL.id)"
        fo.Sign = "%BZ_Name%"
        foList.Add(fo)
        ...
        'fo.DB_Field = " "
        'fo.Field_Operator = Enum_Operator.Operator_Like_Both
        'fo.SQL = "(T50000_InnerPrice_Table.ID %Like% or  SumQty %Like% or SumAmount %Like% or Remark %Like%  )"
        'fo.Sign = "%Like%"
        'foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_InnerPrice_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_InnerPrice_OrderBy)
