...
Public Class F50030_InnerPrice
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_Condition3.ComboBox.DataSource = Dao.Price_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F50031_InnerPrice_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F50031_InnerPrice_Msg(FG1.Item(FG1.RowSel, "ID"))
        With F
...
#End Region
    ...
    Protected Sub InnerPrint(ByVal Confirm As String, ByVal DoOperator As OperatorType)
        ...
        R.Start(FG1.Item(FG1.RowSel, "ID"), Confirm, DoOperator)
    End Sub
...
Partial Friend Class Dao
    ...
    '===================出库单信息==============
    Public Const SQL_Price_Get_WithName = "select T.*,C.Client_Name from T50000_Price_Table T left join T10110_Client C on T.Client_ID=C.ID  "

    Public Const SQL_Price_OrderBy As String = " order by sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Price_GetConditionNames() As List(Of FindOption)
        ...
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "exists(select TW.BZC_Name from T10003_BZC TW,T50001_Price_List TL where %BZC_Name% and TL.BZC_ID=TW.ID and T.id=TL.id)"
        fo.Sign = "%BZC_Name%"
        foList.Add(fo)
        ...
        'fo.DB_Field = " "
        'fo.Field_Operator = Enum_Operator.Operator_Like_Both
        'fo.SQL = "(T50000_Price_Table.ID %Like% or  SumQty %Like% or SumAmount %Like% or Remark %Like%  )"
        'fo.Sign = "%Like%"
        'foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Price_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        'fo.Field = "全部"
        'fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Price_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_Price_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_Price_OrderBy)
