...
Public Class F50010_ProcessPrice
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.Process_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.Process_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F50011_ProcessPrice_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F50011_ProcessPrice_Msg(FG1.Item(FG1.RowSel, "ID"))
        With F
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Dao.Process_DB_NAME & "[" & FG1.Item(FG1.RowSel, "ID") & "]?", AddressOf DelProcess)
    End Sub
    ...
    Protected Sub DelProcess()
        Dim msg As MsgReturn = Dao.Process_Del(FG1.Item(FG1.RowSel, "ID"))
        If msg.IsOk Then
            Me_Refresh()
        Else
            ShowErrMsg(msg.Msg)
...
#Region "搜索工具栏"
    ...
    Private Sub CB_BZC_Col_Sel(ByVal Col_No As System.String, ByVal Col_Name As System.String, ByVal ID As System.String)
        Lable_color.Text = Col_Name
    End Sub

    Private Sub CB_BZC_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Lable_color.Text = ""
    End Sub
    ...
    Private Sub CB_ConditionName2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_ConditionName2.SelectedIndexChanged
        ...
        Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_Process_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
        If msg.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType, ByVal UComfirm As Boolean)
        ...
        R.StartList(FG1.Item(FG1.RowSel, "ID"), DoOperator, UComfirm)
    End Sub
...
#Region "审核状态"
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.Process_Audited(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
            Else
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.Process_SelListById(IsNull(FG1.Item(FG1.RowSel, "ID"), ""), False)
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
        Else
        End If
...
#End Region
    ...
    Private Sub Cb_Client_Col_Sel(ByVal Col_No As System.String, ByVal Col_Name As System.String, ByVal ID As System.String)
...
Partial Friend Class Dao
    ...
    '===================出库单信息==============
    Public Const SQL_Process_Get_WithName = "select T.*,C.Client_Name from T50010_ProcessPrice_Table T left join T10110_Client C on T.Client_ID=C.ID  "
    Public Const SQL_SGProcess_Get_WithName = "select T.*,C.Client_Name from T50012_SGPrice_Table T left join T10110_Client C on T.Client_ID=C.ID  "

    Public Const SQL_Process_OrderBy As String = " order by sDate "
    ...
    Public Shared Function Process_GetConditionNames() As List(Of FindOption)
        ...
        fo.SQL = "exists(select TW.BZ_Name from T10002_BZ TW,T50011_ProcessPrice_List TL where %BZ_Name% and TL.BZ_ID=TW.ID and T.id=TL.id)"
        fo.Sign = "%BZ_Name%"
        foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function Process_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
