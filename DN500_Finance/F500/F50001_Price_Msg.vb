...
#Region "BZ选择"
    ...
    Private Sub CB_BZ_FG_GetSearchEvent(ByRef Col_No As String, ByRef ID As String, ByVal Cancel As Boolean) Handles CB_BZ_FG.GetSearchEvent
        ...
        CB_BZ_FG.IDValue = IsNull(FG1.Item(FG1.RowSel, "BZ_ID"), "")
    End Sub
    Sub FG_BZ_No()
...
#Region "BZC选择"
    ...
    Private Sub CB_BZC_FG_GetSearchEvent(ByRef Col_No As String, ByRef ID As String, ByVal Cancel As Boolean) Handles CB_BZC_FG.GetSearchEvent
        CB_BZC_FG.IDValue = IsNull(FG1.Item(FG1.RowSel, "BZC_ID"), "")
...
#Region "常量"
    Protected Friend Shared InnerPrice_DB_NAME As String = "报价单"
    ''' <summary>
    ''' 删除单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_InnerPrice_DelByid As String = "Delete from T50000_InnerPrice_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_InnerPrice_GetByid As String = "select top 1 * from T50000_InnerPrice_Table  where ID=@ID "

    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_InnerPrice_GetByid_WithName As String = "select top 1 T. * ,C.Client_No,C.Client_Name from T50000_InnerPrice_Table T Left join T10110_Client C on T.Client_ID=C.ID  where T.ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_InnerPrice_SelByid As String = " select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name ,C.Client_Name ,C.Client_No" & _
                                                   " from T50000_InnerPrice_Table S " & _
                                                    ...
                                                    "left join T10110_Client C on S.Client_ID=C.ID " & _
                                                     "where S.ID=@ID "
    'select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T50000_InnerPrice_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_InnerPrice_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T50000_InnerPrice_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_InnerPrice_GetListByid As String = "select * from T50001_InnerPrice_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_InnerPrice_SelListByid As String = " select L.*,BZC_No,BZC_Name,BZ_No,Client_Bzc,BZ_Name " & _
                                                            "  from  T50001_InnerPrice_List L left join T10003_BZC BZC on L.BZC_ID=BZC.ID  " & _
    ...
    ''' <remarks></remarks>0
    ''' 
    Private Const SQL_InnerPrice_SelListByid_ShowComfirm As String = " select L.*,BZC_No,BZC_Name,Client_Bzc,BZC_Name+'/GY-'+right('000000'+CAST(BZC_No as Varchar(20)),6)  as BZC, BZ_No,BZ_Name " & _
                                                          "  from  T50001_InnerPrice_List L left join T10003_BZC BZC on L.BZC_ID=BZC.ID  " & _
                                                          "   left join T10002_BZ BZ on L.BZ_ID=BZ.ID " & _
                                                          " where L.ID=@ID and IsLastInnerPrice=1 order by L.ID ,L.Line "
    ...
    'Private Const SQL_InnerPrice_SelListByid_ShowComfirm As String = " select L.*,BZC_No,BZC_Name,Client_Bzc,BZC_Name+'/'+CAST(BZC_No as Varchar(20))  as BZC, BZ_No,BZ_Name " & _
    '                                                        "  from  T50001_InnerPrice_List L left join T10003_BZC BZC on L.BZC_ID=BZC.ID  " & _
    '                                                        "   left join T10002_BZ BZ on L.BZ_ID=BZ.ID " & _
    '                                                        " where L.ID=@ID and L.IsComfirm=@IsComfirm and IsLastInnerPrice=1 order by L.ID ,L.Line "
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_InnerPrice_CheckID As String = "select count(*) from T50000_InnerPrice_Table  where ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_InnerPrice_CheckIsModify As String = "select count(*) from T50001_InnerPrice_List  where ID=@ID and Line=@Line"
...
#Region "单一张单内容查询"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_InnerPrice_GetByid_WithName, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_SelListById(ByVal sId As String) As DtReturnMsg
        Dim p As New Dictionary(Of String, Object)
        p.Add("ID", sId)
        Return PClass.PClass.SqlStrToDt(SQL_InnerPrice_SelListByid_ShowComfirm, p)
    End Function
...
#Region "添加修改删除"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_CheckIsModify(ByVal ID As String, ByVal Line As Integer) As Boolean
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", ID)
        P.Add("Line", Line)
        Dim R As MsgReturn = SqlStrToOneStr(SQL_InnerPrice_CheckIsModify, P)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return True
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_CheckID(ByVal ID As String) As Boolean
        Dim R As MsgReturn = SqlStrToOneStr(SQL_InnerPrice_CheckID, "@ID", ID)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return False
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim InnerPriceId As String = dtTable.Rows(0)("ID")
        Dim IsInsert As Boolean = False
        paraMap.Add("ID", InnerPriceId)
        Try
            ...
            '        R.IsOk = False
            '        R.Msg = InnerPrice_DB_NAME & "[" & InnerPriceId & "]添加失败!" & vbCrLf & Rx.Msg
            '        Return R
            '    End If
            'End If

            sqlMap.Add("Table", SQL_InnerPrice_GetByid)
            sqlMap.Add("List", SQL_InnerPrice_GetListByid)
            ...
            If msg.DtList("Table").Rows.Count = 0 Then
                ...
                Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & InnerPriceId & "'," & BillType.InnerPrice & ")"
                DtToUpDate(msg, TmSQL)
                R.Msg = "" & InnerPrice_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & InnerPrice_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & InnerPrice_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim InnerPriceId As String = dtTable.Rows(0)("ID")
        If LID <> InnerPriceId AndAlso InnerPrice_CheckID(InnerPriceId) Then
            R.IsOk = False
            R.Msg = "" & InnerPrice_DB_NAME & "[" & InnerPriceId & "]已经存在!请双击编号文本框,获取新编号!"
            Return R
        End If
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Audited Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & InnerPrice_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被审核不能再修改!"
                    Return R
                End If
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Invoid Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & InnerPrice_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
                    Return R
                End If
                DvToDt(dtList, msg.DtList("List"), New List(Of String))
                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & InnerPriceId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.InnerPrice
                If LID <> InnerPriceId Then
                    DtToUpDate(msg, "Update T50000_InnerPrice_Table set id='" & InnerPriceId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
                    dtTable.Rows(0)("ID") = InnerPriceId
                    R.Msg = "" & InnerPrice_DB_NAME & "[" & LID & "]已经修改为[" & InnerPriceId & "]!"
                Else
                    DtToUpDate(msg)
                    R.Msg = "" & InnerPrice_DB_NAME & "[" & LID & "]修改成功!"
                End If
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & InnerPrice_DB_NAME & "[" & LID & "]不存在"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & InnerPrice_DB_NAME & "[" & LID & "]修改错误"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' 删除出库单
    ''' </summary>
    ''' <param name="InnerPriceId"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_Del(ByVal InnerPriceId As String)
        Return RunSQL(SQL_InnerPrice_DelByid, "@ID", InnerPriceId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("ID", _ID)
        paraMap.Add("State", IIf(IsFei, -1, 0))
        paraMap.Add("State_User", User_Display)

        Dim R As MsgReturn = SqlStrToOneStr("Update T50000_InnerPrice_Table set State=@State,State_User=@State_User where ID=@ID  ", paraMap)
        Dim s As String = IIf(IsFei, "作废", "还原")
        If R.IsOk Then
            R.Msg = InnerPrice_DB_NAME & s & "成功！"
        Else
            R.Msg = InnerPrice_DB_NAME & s & "失败！" & R.Msg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("ID", _ID)
        paraMap.Add("IsAudited", IsAudited)
        paraMap.Add("State_User", User_Display)

        Dim R As MsgReturn = SqlStrToOneStr("Update T50000_InnerPrice_Table set State=@IsAudited,State_User=@State_User where ID=@ID  ", paraMap)
...
#End Region
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_SelListById_All(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_InnerPrice_SelListByid, "@ID", sId)
    End Function
...
#Region "根据色号获取染部配方成本"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function InnerPrice_GetPFCost_ByBZC(ByVal BZC_ID As Integer, ByVal BZ_ID As Integer) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("BZC_ID", BZC_ID)
        P.Add("BZ_ID", BZ_ID)
