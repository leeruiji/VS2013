...
Public Class F40301_CPRK_Msg
    ...
    Private Sub F40301_CPRK_Msg_AfterLoad() Handles Me.AfterLoad
    ...
    Private Sub Form_Msg_Load() Handles Me.Me_Load
        ...
        'Dim msg As DtReturnMsg = Dao.CPRK_GetDATE_ByID()
        'If msg.IsOk AndAlso msg.Dt.Rows.Count > 0 Then
        '    DTR_Date_CPRK.Value = IsNull(msg.Dt.Rows(0)("Date_CPRK"), "")
        'Else
...
#Region "工具栏按钮"
    ...
    Private Sub Cmd_Modify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Modify.Click
        If CheckForm() Then
            ...
            If DTR_Date_CPRK.Value.Date <> D Then
                ShowConfirm("你选择的入库日期是[" & DTR_Date_CPRK.Value.Date.ToString("yyyy-MM-dd") & "]和单前日期[" & D.Date.ToString("yyyy-MM-dd") & "]不相同,是否继续保存" & Ch_Name & "?", "继续保存", "改为当天", "取消", AddressOf showSave, AddressOf SaveConfirmAsDate, AddressOf Me.NoSave)
            Else
                showSave()
            End If
        End If
    ...
    Sub showSave()
        ShowConfirm("是否保存" & Ch_Name & "[" & TB_GH.Text & "] 的修改?", AddressOf SaveCPRK)
    End Sub
    Sub showSaveConfirm()
    ...
    Protected Sub SaveAndConfirm(Optional ByVal Audit As Boolean = False)
        ...
        R = Dao.CPRK_Save(TB_GH.Tag, DTR_Date_CPRK.Value, LDT, Val(TB_CPRK_Qty.Text), Val(TB_RemainQty.Text) = 0)

        If R.IsOk Then
            LastForm.ReturnId = TB_GH.Text
            If Dao.CPRK_UpdateFormState(Enum_BZSHState.Store_Comfirm, TB_GH.Text) Then
                ShowOk("确认成功", False)
                Get_GH(Bill_ID)
            Else
                ShowErrMsg("确认失败")
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SaveCPRK(Optional ByVal Audit As Boolean = False)
        Dim R As MsgReturn
    ...
    Protected Function CheckForm() As Boolean
        ...
        '    ShowErrMsg(Ch_Name & "入库条数不能为0", AddressOf TB_CW.Focus)
        '    Return False
        'End If
        If TB_RemainQty.Text <> "0" Then
            ...
            Return False
        End If
        TB_GH.Text = ComFun.FixRK(TB_GH.Text)
        Dim R As MsgReturn = ComFun.CheckRK(TB_GH.Text, LastID <> "")
        If R.IsOk = False Then
            ShowErrMsg(R.Msg, AddressOf TB_GH.Focus)
            Return False
    ...
    Protected Sub UnComfirm()
        If FormState = Enum_BZSHState.Store_Comfirm Then
            If Dao.CPRK_UpdateFormState(Enum_BZSHState.AddNew, TB_GH.Text) Then
                RunSQL("update T30000_Produce_Gd set Store_CPRK ='',State=80 where GH='" & TB_GH.Text & " '")
                '  Dao.CPRK_CleanStore(TB_GH.Text, "")
                ShowOk("取消确认成功！")
            Else
                ShowErrMsg("取消确认失败！")
            End If
...
#Region "选缸号"
    ...
    Sub Get_GH(ByVal ID As String)
        ...
        End If
        ID = Mr.Msg
        Dim R As DtReturnMsg = Dao.CPRK_GetGH_ByID(ID)
        If R.IsOk = False Then
        ...
        DTR_Date_CPRK.text = IsNull(Dr("Date_CPRK"), "")
        ...
        TB_CPRK_User.Text = IsNull(Dr("CPRK_User"), "")
        ...
        Dim RDt As DtReturnMsg = Dao.CPRK_GetListByGH(TB_GH.Text)
        If RDt.IsOk Then
            SetListBox(RDt.Dt)
        End If
        CaculateSumAmount()
...
#Region "取消入库"
    ...
    Sub CancelPB()
        Dim R As MsgReturn = Dao.CPRK_Cancel(Bill_ID)
        If R.IsOk Then
            ShowOk(R.Msg)
        Else
            IsShowErr = True
...
#End Region
    ...
    Private Sub Cmd_Audit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Audit.Click
        ...
        If FormState = Enum_BZSHState.Store_Comfirm Then
            ...
            If Dao.CPRK_UpdateFormState(Enum_BZSHState.Audited, TB_GH.Text) Then
                ShowOk("财务审核成功!")
                Get_GH(Bill_ID)
            Else
                ShowErrMsg("财务审核失败!")
...
#End Region
    ...
    Protected Sub SetForm(ByVal dt As DataTable)
        ...
        Else
            ...
            FormState = IsNull(dt.Rows(0)("CPRK_State"), Enum_BZSHState.AddNew)
        End If
