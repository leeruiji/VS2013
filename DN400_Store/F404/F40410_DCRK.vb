...
Public Class F40410_DCRK
    ...
    Private Sub F40410_DCRK_Me_Load() Handles Me.Me_Load
        FormCheckRight()
        ...
        CB_isDCRK.ComboBox.DisplayMember = "Field"
        CB_isDCRK.ComboBox.ValueMember = "DB_Field"
        CB_isDCRK.ComboBox.DataSource = Dao.BCPRK_GetCondition3
        CB_isDCRK.ComboBox.SelectedIndex = 0
        Me_Refresh()
    End Sub
...
#Region "刷新FG"
    ...
    Protected Sub GetData(ByVal oList As OptionList)
        Dim msg As DtReturnMsg = Dao.DCRK_GetByOption(oList)
...
#End Region
    ...
    Private Sub F40000_DCRK_KeyPress(ByVal sender As Object, ByRef e As System.Windows.Forms.KeyPressEventArgs) Handles Me.Form_KeyPress
...
#Region "控件事件"
    ...
    Private Sub Cmd_Modify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Modify.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F40411_DCRK_Msg(FG1.SelectItem("CPRK_ID"))
        With F
    ...
    Sub ModifyCPRK(ByVal GH As String)
        Dim F As New F40411_DCRK_Msg(GH)
        With F
            .Mode = Mode_Enum.Modify
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
...
#Region "搜索工具栏"
    ...
    Protected Function GetFindOtions() As OptionList
        ...
        If CB_isDCRK.SelectedIndex > 0 Then
            fo = New FindOption
            fo.DB_Field = "IsNull(isBCPRK, 0)"
            fo.Value = CB_isDCRK.ComboBox.SelectedValue
            fo.Field_Operator = Enum_Operator.Operator_Equal
            oList.FoList.Add(fo)
        End If
...
Partial Friend Class Dao
    ...
    Public Const SQL_ProduceGd_Get_DCRK As String = "select p.GH,isFD,BzcMsg,p.State,p.PB_User,ClientBzc,p.Date_KaiDan,p.Date_PeiBu,CR_LuoSeBzCount,PB_CountSum,CPRK_Qty,KaiDan,Client_No,Store_CPRK, C.Client_Name,BZ_No,BZ_Name ,BZC_No,BZC_Name,BZ_Spec,CPRK_User,BZC_PF from T30000_Produce_Gd P left join T10110_Client C on P.Client_ID=C.ID left join T10002_BZ BZ on P.BZ_ID=BZ.ID left join T10003_BZC BZC on P.BZC_ID=BZC.ID "

    Public Const SQL_DCRK_Get_WithName = "select distinct( L.GH), P.*,Client_No,S.Client_Name,BZ_No,BZ_Name from T40100_PBRK_Table P left join T10110_Client S on  P.Client_ID=S.ID left join T10002_BZ BZ on BZ.ID =P.BZ_ID left Join T40101_PBRK_List L on P.ID=L.ID"
    Public Const SQL_DCRK_GetPBRK_Table_ByID = "select top 1 p.ID,sDate,P.Client_ID,BZ_ID,Notice,Remark,ShaPi,State,BZ_No,BZ_Name from T40100_PBRK_Table P left join T10002_BZ B on B.ID=P.BZ_ID where P.ID=@ID"
    Public Const SQL_DCRK_GetPBRK_List_ByID = "select * from T40101_PBRK_List where ID=@ID"
    Public Const SQL_DCRK_GetGH_ByID = "select G.*,C.Client_No,C.Client_Name,BZ.Bz_No,Bz.Bz_Name,BZC.BZC_No,BZC.BZC_Name from(select top 1 * from T30000_Produce_Gd where GH=@GH)G left join T10110_Client C On C.id=G.Client_id  left join T10002_BZ BZ On BZ.id=G.BZ_id   left join T10003_BZC BZC On BZC.id=G.BZC_id "
    Public Const SQL_DCRK_GetListByGH = "select * from T40400_BCPRK where GH=@GH"
...
#Region "半成品入库"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function DCRK_GetListByGH(ByVal GH As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_DCRK_GetListByGH, "@GH", GH)
    End Function


    Public Shared Function DCRK_GetPBRK_Table_ByID(ByVal ID As String) As DtReturnMsg
        Return SqlStrToDt(SQL_DCRK_GetPBRK_Table_ByID, "@ID", ID)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function DCRK_Cancel(ByVal GH As String) As MsgReturn
        Dim R As New MsgReturn
        Dim S As New Dictionary(Of String, String)
        Dim P As New Dictionary(Of String, Object)
        S.Add("G", "Select top 1 GH,isBCPRK,CR_LuoSeBzCount,Store_CPRK,Date_CPRK,CPRK_Qty,CPRK_User from T30000_Produce_Gd where GH=@GH")
        ...
        Using H As New DtHelper(S, P)
            If H.IsOk = True Then
                If H.DtList("G").Rows.Count > 0 Then
                    ...
                    H.DtList("G").Rows(0)("Date_CPRK") = DBNull.Value
                    H.DtList("G").Rows(0)("CPRK_Qty") = DBNull.Value
                    H.DtList("G").Rows(0)("CPRK_User") = DBNull.Value
    ...
    Public Shared Function DCRK_GetGH_ByID(ByVal ID As String) As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function DCRK_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_ProduceGd_Get_DCRK)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function DCRK_GetConditionNames() As List(Of FindOption)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function DCRK_Save(ByVal GH As String, ByVal PB_Date As Date, ByVal dtList As DataTable, ByVal CPRK_Qty As Integer, ByVal IsNum As Boolean) As MsgReturn
        ...
        Dim P As New Dictionary(Of String, Object)
        SQL.Add("P", "select top 1 GH,isBCPRK,CPRK_User,Date_CPRK,CPRK_Qty,Store_CPRK from T30000_Produce_Gd where GH=@GH")
        ...
        Using H As New DtHelper(SQL, P)
            If H.IsOk Then
                If H.DtList("P").Rows.Count > 0 Then
                    If IsNum Then
                        H.DtList("P").Rows(0)("isBCPRK") = True
