...
#Region "选缸号"
    ...
    Sub Get_GH(ByVal ID As String)
        ...
        If IsNull(Dr("State"), 0) = Enum_ProduceState.RuKu Then
            ShowErrMsg("缸号[" & ID & "]已成品入库,不能再入库!", AddressOf TB_GH.Focus)
            Cmd_Modify.Visible = False
            GB_List.Enabled = False
            Btn_RemoveRow.Visible = False
        End If
        ...
        Dim RDt As DtReturnMsg = Dao.DCRK_GetListByGH(TB_GH.Text)
        If RDt.IsOk Then
            SetListBox(RDt.Dt)
        End If
        CaculateSumAmount()
...
#Region "取消入库"
    ...
    Sub CancelPB()
        Dim R As MsgReturn = Dao.DCRK_Cancel(Bill_ID)
        If R.IsOk Then
            ShowOk(R.Msg)
        Else
            IsShowErr = True
