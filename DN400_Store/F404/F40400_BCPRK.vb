...
Public Class F40400_BCPRK
    ...
    Private Sub F40400_BCPRK_Me_Load() Handles Me.Me_Load
        FormCheckRight()
        ...
        CB_isBCPRK.ComboBox.DisplayMember = "Field"
        CB_isBCPRK.ComboBox.ValueMember = "DB_Field"
        CB_isBCPRK.ComboBox.DataSource = Dao.BCPRK_GetCondition3
        CB_isBCPRK.ComboBox.SelectedIndex = 0
        Me_Refresh()
    End Sub
...
#Region "刷新FG"
    ...
    Protected Sub GetData(ByVal oList As OptionList)
        Dim msg As DtReturnMsg = Dao.BCPRK_GetByOption(oList)
...
#End Region
    ...
    Protected Sub ConvertState()
        ...
        For Each dr In Me.dtProduce.Rows
            ...
            Select Case IsNull(dr("BCPRK_State"), Enum_BZSHState.AddNew)
    ...
    Private Sub F40000_BCPRK_KeyPress(ByVal sender As Object, ByRef e As System.Windows.Forms.KeyPressEventArgs) Handles Me.Form_KeyPress
...
#Region "控件事件"
    ...
    Private Sub Cmd_Modify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Modify.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F40401_BCPRK_Msg(FG1.SelectItem("BCPRK_ID"))
        With F
    ...
    Sub ModifyCPRK(ByVal GH As String)
        Dim F As New F40401_BCPRK_Msg(GH)
        With F
            .Mode = Mode_Enum.Modify
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Function GetFindOtions() As OptionList
        ...
        ElseIf CB_State.SelectedIndex = 3 Then        '按入库日期搜索()
            For Each fo In oList.FoList
                If fo.DB_Field = "Date_BCPRK" Then
                    fo.DB_Field = "Date_BCPRK"
                    Exit For
                End If
            Next
        ...
        If CB_isBCPRK.SelectedIndex > 0 Then
            fo = New FindOption
            fo.DB_Field = "IsNull(isBCPRK, 0)"
            fo.Value = CB_isBCPRK.ComboBox.SelectedValue
            fo.Field_Operator = Enum_Operator.Operator_Equal
            oList.FoList.Add(fo)
        End If
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ' Dim R As New R40001_CPRK
        ' R.Start(FG1.Item(FG1.RowSel, "CPRK_ID"), DoOperator)
    End Sub
...
Partial Friend Class Dao
    ...
    Public Const SQL_ProduceGd_Get_BCPRK As String = "select p.GH,isFD,BzcMsg,p.State,p.PB_User,ClientBzc,p.Date_BCPRK,p.Date_PeiBu,CR_LuoSeBzCount,PB_CountSum,BCPRK_Qty,KaiDan,Client_No,Store_BCPRK, C.Client_Name,BZ_No,BZ_Name ,BZC_No,BZC_Name,BZ_Spec,BCPRK_User,BZC_PF,BCPRK_State from T30000_Produce_Gd P left join T10110_Client C on P.Client_ID=C.ID left join T10002_BZ BZ on P.BZ_ID=BZ.ID left join T10003_BZC BZC on P.BZC_ID=BZC.ID "

    Public Const SQL_BCPRK_Get_WithName = "select distinct( L.GH), P.*,Client_No,S.Client_Name,BZ_No,BZ_Name from T40100_PBRK_Table P left join T10110_Client S on  P.Client_ID=S.ID left join T10002_BZ BZ on BZ.ID =P.BZ_ID left Join T40101_PBRK_List L on P.ID=L.ID"
    Public Const SQL_BCPRK_GetPBRK_Table_ByID = "select top 1 p.ID,sDate,P.Client_ID,BZ_ID,Notice,Remark,ShaPi,State,BZ_No,BZ_Name from T40100_PBRK_Table P left join T10002_BZ B on B.ID=P.BZ_ID where P.ID=@ID"
    Public Const SQL_BCPRK_GetPBRK_List_ByID = "select * from T40101_PBRK_List where ID=@ID"
    Public Const SQL_BCPRK_GetGH_ByID = "select G.*,C.Client_No,C.Client_Name,BZ.Bz_No,Bz.Bz_Name,BZC.BZC_No,BZC.BZC_Name from(select top 1 * from T30000_Produce_Gd where GH=@GH)G left join T10110_Client C On C.id=G.Client_id  left join T10002_BZ BZ On BZ.id=G.BZ_id   left join T10003_BZC BZC On BZC.id=G.BZC_id "
    Public Const SQL_BCPRK_GetListByGH = "select * from T40400_BCPRK where GH=@GH"
    Public Const SQL_BCPRK_CleanStore = "update T30000_Produce_Gd set Store_CPRK =@Store_CPRK where GH=@GH"
...
#Region "半成品入库"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BCPRK_GetListByGH(ByVal GH As String) As DtReturnMsg

        Return PClass.PClass.SqlStrToDt(SQL_BCPRK_GetListByGH, "@GH", GH)
    End Function
    ''' <summary>
    ''' 半成品入库的清空库存
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BCPRK_CleanStore(ByVal _GH As String, ByVal Store_CPRK As String) As DtReturnMsg
        Dim P As New Dictionary(Of String, Object)
        P.Add("@GH", _GH)
        P.Add("@Store_CPRK", Store_CPRK)
        Return PClass.PClass.SqlStrToDt(SQL_BCPRK_CleanStore, P)
    End Function


    Public Shared Function BCPRK_GetPBRK_Table_ByID(ByVal ID As String) As DtReturnMsg
        Return SqlStrToDt(SQL_BCPRK_GetPBRK_Table_ByID, "@ID", ID)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BCPRK_Cancel(ByVal GH As String) As MsgReturn
        Dim R As New MsgReturn
        Dim S As New Dictionary(Of String, String)
        Dim P As New Dictionary(Of String, Object)
        S.Add("G", "Select top 1 GH,isBCPRK,CR_LuoSeBzCount,Store_CPRK,Date_BCPRK,BCPRK_Qty,BCPRK_User from T30000_Produce_Gd where GH=@GH")
        S.Add("C", "select * from T40400_BCPRK where GH=@GH")
        P.Add("GH", GH)
            ...
            If H.IsOk = True Then
                If H.DtList("G").Rows.Count > 0 Then
                    ...
                    For Each Row As DataRow In H.DtList("C").Rows
                        Row.Delete()
                    Next
                    H.DtList("G").Rows(0)("isBCPRK") = False
                    H.DtList("G").Rows(0)("Store_BCPRK") = DBNull.Value
                    H.DtList("G").Rows(0)("Date_BCPRK") = DBNull.Value
                    H.DtList("G").Rows(0)("BCPRK_Qty") = DBNull.Value
                    H.DtList("G").Rows(0)("BCPRK_User") = DBNull.Value
    ...
    Public Shared Function BCPRK_GetGH_ByID(ByVal ID As String) As DtReturnMsg
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BCPRK_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_ProduceGd_Get_BCPRK)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BCPRK_GetConditionNames() As List(Of FindOption)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BCPRK_Save(ByVal GH As String, ByVal PB_Date As Date, ByVal dtList As DataTable, ByVal CPRK_Qty As Integer, ByVal IsNum As Boolean) As MsgReturn
        ...
        Dim P As New Dictionary(Of String, Object)
        SQL.Add("P", "select top 1 GH,isBCPRK,BCPRK_User,Date_BCPRK,BCPRK_Qty,Store_CPRK from T30000_Produce_Gd where GH=@GH")
        SQL.Add("C", "select * from T40400_BCPRK where GH=@GH")
        P.Add("GH", GH)
        Using H As New DtHelper(SQL, P)
            If H.IsOk Then
                If H.DtList("P").Rows.Count > 0 Then
       
                    'If H.DtList("P").Rows(0)("State") >= BaseClass.Enum_ProduceState.ChuGang And H.DtList("P").Rows(0)("State") < BaseClass.Enum_ProduceState.ChengJian Then
                    If IsNum Then
                        ' H.DtList("P").Rows(0)("State") = BaseClass.Enum_ProduceState.RuKu
                        H.DtList("P").Rows(0)("BCPRK_User") = User_Display
                        'H.DtList("P").Rows(0)("Date_CPRK") = PB_Date.Date

                    End If

                    H.DtList("P").Rows(0)("Date_BCPRK") = PB_Date.Date
                    H.DtList("P").Rows(0)("Store_CPRK") = Store_CPRK
                    H.DtList("P").Rows(0)("BCPRK_Qty") = CPRK_Qty
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function BCPRK_UpdateFormState(ByVal _State As Enum_BZSHState, ByVal _GH As String) As Boolean
        Dim p As New Dictionary(Of String, Object)

        p.Add("GH", _GH)
        Dim sql As New StringBuilder
        Dim R As MsgReturn = SqlStrToOneStr("select BCPRK_State from  T30000_Produce_Gd  where GH=@GH ", p)
        If R.IsOk Then

            p.Add("FormState", _State)
            sql.AppendLine("update T30000_Produce_Gd set BCPRK_State =@FormState where GH=@GH")
            If _State = Enum_BZSHState.Store_Comfirm AndAlso Val(R.Msg) = Enum_BZSHState.AddNew Then

                sql.AppendLine("insert into T40520_PB_StoreNo  (StoreNo,ID,Qty,BZType) select StoreNo,GH,Qty,2 from T40400_BCPRK  where GH=@GH ")
                sql.AppendLine("insert into T40521_PB_Detail  (StoreNo,ID,GH,InQty,BillType,BillName) select StoreNo,GH,GH,Qty,40400,'半成品入库单' from T40400_BCPRK  where GH=@GH ")
            ...
            ElseIf _State = Enum_BZSHState.AddNew AndAlso Val(R.Msg) = Enum_BZSHState.Store_Comfirm Then
                ...
                sql.AppendLine("delete from T40521_PB_Detail  where billtype=40400 and ID=@GH ")
            End If
