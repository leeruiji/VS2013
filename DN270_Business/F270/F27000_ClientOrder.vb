...
Public Class F27000_ClientOrder
    ...
    Private Sub F27000_ClientOrder_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        isRowChang = False
    End Sub
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.StoreIn_GetConditionNames()
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.StoreIn_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.StoreIn_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub

    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.ClientOrder_GetByOption(GetFindOtions)
        If msg.IsOk Then
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F27001_ClientOrder_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F27001_ClientOrder_Msg(FG1.Item(FG1.RowSel, "ID"))
        With F
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Ch_Name & "[" & FG1.Item(FG1.RowSel, "ID") & "]?", AddressOf DelClientOrder)
    End Sub
    ...
    Protected Sub DelClientOrder()
        Dao.ClientOrder_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.ClientOrder_Del(FG1.Item(FG1.RowSel, "ID"))
        If msg.IsOk Then
...
#Region "搜索工具栏"
    ...
    '    Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_StoreIn_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
    '    If msg.IsOk Then
    ...
    Protected Function GetFindOtions() As OptionList
        ...
        'fo = New FindOption
        'fo.DB_Field = "StoreInType"
        'fo.Value = Me.StoreInType
...
#Region "审核状态"
    ...
    Private Sub Cmd_Audit_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Audit.Click
        ...
        For i = 1 To FG1.Rows.Count - 1
            If IsNull(FG1.Item(i, "IsChecked"), False) = True Then
                If IsNull(FG1.Item(i, "ConDDate"), False) = True Then
                    idList.Add(FG1.Item(i, "ID"))
                Else
                    ShowErrMsg(Ch_Name & FG1.Item(i, "ID") & "没有确认送货日期，不能被审核！")
                    Exit Sub
                End If
            End If
        Next
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.ClientOrder_Audited(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
            Else
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.ClientOrder_SelListById(IsNull(FG1.Item(FG1.RowSel, "ID"), ""))
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
            SumFG2.ReSum()
        Else
...
Partial Friend Class Dao
    ...
    '===================入库单信息==============
    Public Const SQL_ClientOrder_GetByOption = "select CT.*,C.Client_No from T27000_ClientOrder_Table CT left join T10110_Client C on CT.Client_ID=C.ID"

    Public Const SQL_ClientOrder_OrderBy As String = " order by CT.sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_GetConditionNames() As List(Of FindOption)
        ...
        fo = New FindOption
        fo.Field = "物料名称"
        fo.DB_Field = "TW.WL_Name"
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "exists(select * from T10001_WL TW,T27001_ClientOrder_List CL where %WL_Name% and CL.wl_id=TW.ID and CT.id=CL.id)"
        fo.Sign = "%WL_Name%"
        foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ClientOrder_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_ClientOrder_GetByOption)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_ClientOrder_OrderBy)
        Return PClass.PClass.SqlStrToDt(sqlBuider.ToString, paraMap)
    End Function


    'Public Shared Function GetStoreIn_From(ByVal FID As Integer, ByVal ID As Integer) As PClass.BaseForm
    '    Dim F As New F20201_StoreIn_Msg(ID)
    '    With F
    '        .Mode = Mode_Enum.Modify
    '        .P_F_RS_ID = ""
    '        .P_F_RS_ID2 = ""
