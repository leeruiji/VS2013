...
Public Class F27210_ReturnGoods
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.ReturnGoods_GetConditionNames()
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.ReturnGoods_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.ReturnGoods_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub

    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.ReturnGoods_GetByOption(GetFindOtions)
        If msg.IsOk Then
            ...
            For Each dr In msg.Dt.Rows
                ...
                dr("StateName") = BaseClass.ComFun.GetStoreStateName(_state)

                dr("IsChecked") = False
            Next
            Dim Dt As DataTable
            If SelectReturn Then
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F27211_ReturnGoods_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
        ...
        AddHandler VF.ClosedX, AddressOf Edit_Return
        VF.Show()
    End Sub
    ...
    Private Sub FG1_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles FG1.Click
        If FG1.ColSel = FG1.Cols("IsChecked").Index AndAlso FG1.Rows.Count > 1 Then
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的" & Dao.ReturnGoods_DB_NAME)
            Exit Sub
        End If
        Dim F As New F27211_ReturnGoods_Msg(FG1.Item(FG1.RowSel, "ID"))
        With F
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Dao.ReturnGoods_DB_NAME & "[" & FG1.Item(FG1.RowSel, "ID") & "]?", AddressOf DelReturnGoods)
    End Sub
    ...
    Protected Sub DelReturnGoods()
        Dim msg As MsgReturn = Dao.ReturnGoods_Del(FG1.Item(FG1.RowSel, "ID"))
        If msg.IsOk Then
...
#Region "搜索工具栏"
    ...
    Private Sub CB_ConditionName2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_ConditionName2.SelectedIndexChanged
        ...
        Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_ReturnGoods_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
        If msg.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        ...
        '    Exit Sub
        'End If
        'Dim R As New R27210_ReturnGoods
        'R.Start(FG1.Item(FG1.RowSel, "ID"), DoOperator, Me.ReturnGoodsType)
    End Sub
...
#Region "审核状态"
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.ReturnGoods_Audit(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
            Else
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.ReturnGoods_SelListById(IsNull(FG1.Item(FG1.RowSel, "ID"), ""))
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
            SumFG2.ReSum()
        Else
...
Partial Friend Class Dao
    ...
    '===================出库单信息==============
    Public Const SQL_ReturnGoods_Get_WithName = "select * from T27210_ReturnGoods_Table "

    Public Const SQL_ReturnGoods_OrderBy As String = " order by sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_GetConditionNames() As List(Of FindOption)
        ...
        fo.SQL = "exists(select * from T10001_WL TW,T27211_ReturnGoods_List TL where %WL_Name% and TL.wl_id=TW.ID and T27210_ReturnGoods_Table.id=TL.id)"
        fo.Sign = "%WL_Name%"
        foList.Add(fo)
        ...
        fo.DB_Field = " "
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "(T27210_ReturnGoods_Table.ID %Like% or  SumQty %Like% or SumAmount %Like% or Remark %Like%  )"
        fo.Sign = "%Like%"
        foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
        ...
        fo = New FindOption
        fo.Field = "新建"
        fo.DB_Field = Enum_State_Upgrade.AddNew
        foList.Add(fo)

        fo = New FindOption
        fo.Field = "已确认"
        fo.DB_Field = Enum_State_Upgrade.Comfirm
        foList.Add(fo)

        fo = New FindOption
        fo.Field = "已审核"
        fo.DB_Field = Enum_State_Upgrade.Audited
        foList.Add(fo)

        fo = New FindOption
        fo.Field = "已作废"
        fo.DB_Field = Enum_State_Upgrade.Invoid
        foList.Add(fo)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_ReturnGoods_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_ReturnGoods_OrderBy)
