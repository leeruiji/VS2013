...
Public Class F27211_ReturnGoods_Msg
    Dim LId As String = ""
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        ComFun.DelBillNewID(BillType.ReturnGoods, Bill_ID)
    End Sub
    ...
    Protected Sub Me_Refresh()
        Dim msgTable As DtReturnMsg = Dao.ReturnGoods_GetById(Bill_Id)
        ...
        Select Case Mode
            Case Mode_Enum.Add
                ...
                Dim msg As DtReturnMsg = Dao.ReturnGoods_SelListById(0)
            ...
            Case Mode_Enum.Modify
                Dim msg As DtReturnMsg = Dao.ReturnGoods_SelListById(Bill_ID)
...
#Region "工具栏按钮"
    ...
    Private Sub Cmd_Modify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Modify.Click
        ...
        If CheckForm() Then
            If Mode = Mode_Enum.Add Then
                ShowConfirm("是否保存新" & Ch_Name & "?", AddressOf SaveReturnGoods)
            Else
                If CheckIsModify() Then
                    ShowConfirm(Ch_Name & "[" & TB_ID.Text & "] 已经被其他人修改过,是否覆盖别人的修改?", AddressOf SaveReturnGoods)
                Else
                    ShowConfirm("是否保存" & Ch_Name & "[" & TB_ID.Text & "] 的修改?", AddressOf SaveReturnGoods)
                End If
            End If
        End If
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SaveReturnGoods(Optional ByVal Confirm As Boolean = False)
        Dim R As MsgReturn
        Dim Dt As DataTable = GetForm()
        If Me.Mode = Mode_Enum.Add Then
            R = Dao.ReturnGoods_Add(Dt, dtList)
        Else
            R = Dao.ReturnGoods_Save(LId, Dt, dtList)
        End If
        If R.IsOk Then
            LId = TB_ID.Text
            ...
            If Confirm Then
                Dim msg As MsgReturn = Dao.ReturnGoods_Confirmed(TB_ID.Text, True)
                If msg.IsOk Then
                    Bill_ID = TB_ID.Text
                    Me_Refresh()
                    ShowOk(msg.Msg)
    ...
    Protected Function CheckForm() As Boolean
        ...
        If TB_ID.Text = "" Then
            ...
            Return False
        End If
        If (Mode = Mode_Enum.Add Or (Mode = Mode_Enum.Modify And LId <> TB_ID.Text)) AndAlso Dao.ReturnGoods_CheckID(TB_ID.Text) Then
...
#Region "获取新单号"
    ...
    Sub GetNewID()
        Dao.ReturnGoods_DB_NAME = Ch_Name
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.ReturnGoods, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        ...
        'Dim R As New R20300_ReturnGoods
        'R.Start(TB_ID.Text, DoOperator, Me.ReturnGoodsType)
    End Sub
...
#Region "确认状态"
    ...
    Protected Sub UnConfirm()
        Dao.ReturnGoods_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.ReturnGoods_Confirmed(TB_ID.Text, False)
        If msg.IsOk Then
            ShowOk(msg.Msg)
            Me_Refresh()
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        ShowConfirm("是否删除" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf DelReturnGoods)
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub DelReturnGoods()
        Dao.ReturnGoods_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.ReturnGoods_Del(TB_ID.Text)
        If msg.IsOk Then
            LastForm.ReturnId = TB_ID.Text
            Me.Close()
        Else
    ...
    Private Sub Cmd_SetInValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetInValid.Click
        ShowConfirm("是否作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf ReturnGoodsInValid)
    End Sub


    Sub ReturnGoodsInValid()
        Dao.ReturnGoods_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.ReturnGoods_InValid(TB_ID.Text, True)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
    ...
    Private Sub Cmd_SetValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetValid.Click
        ShowConfirm("是否反作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf ReturnGoodsValid)
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Sub ReturnGoodsValid()
        Dao.ReturnGoods_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.ReturnGoods_InValid(TB_ID.Text, False)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
    ...
    ''' <remarks></remarks>
    Private Sub Audit()
        Dim Msg As MsgReturn = Dao.ReturnGoods_Audit(TB_ID.Text, True)
        If Msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
            ShowOk("退货单审核成功")
    ...
    ''' <remarks></remarks>
    Private Sub UnAudit()
        Dim Msg As MsgReturn = Dao.ReturnGoods_Audit(TB_ID.Text, False)
        If Msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
            ShowOk("退货单反审核成功")
        Else
            ShowErrMsg("退货单单反审核失败")
        End If
    End Sub
...
#Region "检查其他人有没有修改过"
    ...
    Private Function CheckIsModify() As Boolean
        If Mode <> Mode_Enum.Add Then
            Return Dao.ReturnGoods_CheckIsModify(LId, LastLine)
        End If
    End Function
...
#Region "常量"
    Protected Friend Shared ReturnGoods_DB_NAME As String = "退货单"
    ''' <summary>
    ''' 删除单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ReturnGoods_DelByid As String = "Delete from T27210_ReturnGoods_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ReturnGoods_GetByid As String = "select top 1 * from T27210_ReturnGoods_Table  where ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ReturnGoods_SelByid As String = "select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T27210_ReturnGoods_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ReturnGoods_GetStateByid As String = "select top 1 ID,State,State_User from T27210_ReturnGoods_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ReturnGoods_GetListByid As String = "select * from T27211_ReturnGoods_List  where ID=@ID "
    ...
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_ReturnGoods_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec from (select * from T27211_ReturnGoods_List  where ID=@ID)L left join T10001_WL WL on WL.id=l.WL_ID  "
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ReturnAudit As String = " Update T27210_ReturnGoods_Table set State=@State , State_User=@State_User,Audited_Date=@Audited_Date Where ID=@ID "

    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ReturnGoods_CheckID As String = "select count(*) from T27210_ReturnGoods_Table  where ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_ReturnGoods_CheckIsModify As String = "select count(*) from T27211_ReturnGoods_List  where ID=@ID and Line=@Line"
...
#Region "单一张单内容查询"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_ReturnGoods_GetByid, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_SelListById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_ReturnGoods_SelListByid, "@ID", sId)
    End Function
...
#Region "添加修改删除"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_CheckIsModify(ByVal ID As String, ByVal Line As Integer) As Boolean
        Dim P As New Dictionary(Of String, Object)
        P.Add("ID", ID)
        P.Add("Line", Line)
        Dim R As MsgReturn = SqlStrToOneStr(SQL_ReturnGoods_CheckIsModify, P)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return True
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_CheckID(ByVal ID As String) As Boolean
        Dim R As MsgReturn = SqlStrToOneStr(SQL_ReturnGoods_CheckID, "@ID", ID)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return False
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim ReturnGoodsId As String = dtTable.Rows(0)("ID")
        Dim IsInsert As Boolean = False
        paraMap.Add("ID", ReturnGoodsId)
        Try
            sqlMap.Add("Table", SQL_ReturnGoods_GetByid)
            sqlMap.Add("List", SQL_ReturnGoods_GetListByid)
            ...
            If msg.DtList("Table").Rows.Count = 0 Then
                ...
                Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & ReturnGoodsId & "'," & BillType.ReturnGoods & ")"
                DtToUpDate(msg, TmSQL)
                R.Msg = "" & ReturnGoods_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & ReturnGoods_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & ReturnGoods_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim ReturnGoodsId As String = dtTable.Rows(0)("ID")
        If LID <> ReturnGoodsId AndAlso ReturnGoods_CheckID(ReturnGoodsId) Then
            R.IsOk = False
            R.Msg = "" & ReturnGoods_DB_NAME & "[" & ReturnGoodsId & "]已经存在!请双击编号文本框,获取新编号!"
            Return R
        End If
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If msg.DtList("Table").Rows(0)("State") = Enum_StroeOutstate.Comfirm Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & ReturnGoods_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被确认不能再修改!"
                    Return R
                End If
                If msg.DtList("Table").Rows(0)("State") = Enum_StroeOutstate.Invoid Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & ReturnGoods_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
                    Return R
                End If
                DvToDt(dtList, msg.DtList("List"), New List(Of String))
                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & ReturnGoodsId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.ReturnGoods
                If LID <> ReturnGoodsId Then
                    DtToUpDate(msg, "Update T27210_ReturnGoods_Table set id='" & ReturnGoodsId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
                    dtTable.Rows(0)("ID") = ReturnGoodsId
                    R.Msg = "" & ReturnGoods_DB_NAME & "[" & LID & "]已经修改为[" & ReturnGoodsId & "]!"
                Else
                    DtToUpDate(msg)
                    R.Msg = "" & ReturnGoods_DB_NAME & "[" & LID & "]修改成功!"
                End If
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & ReturnGoods_DB_NAME & "[" & LID & "]不存在"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & ReturnGoods_DB_NAME & "[" & LID & "]修改错误"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' 删除退货单
    ''' </summary>
    ''' <param name="ReturnGoodsId"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_Del(ByVal ReturnGoodsId As String)
        Return RunSQL(SQL_ReturnGoods_DelByid, "@ID", ReturnGoodsId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        ...
        paraMap.Add("ID", _ID)
        Try
            sqlMap.Add("Table", SQL_ReturnGoods_GetStateByid)
            msg = SqlStrToDt(sqlMap, paraMap)
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                Else
                    R.IsOk = False
                    R.Msg = "" & ReturnGoods_DB_NAME & "[" & _ID & "]已经被确认,不能" & OStr & "!"
                End If
            Else
                R.IsOk = False
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_Audit(ByVal _ID As String, ByVal IsAudit As Boolean) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function ReturnGoods_Confirmed(ByVal _ID As String, ByVal IsConfirmed As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("ID", _ID)
        paraMap.Add("IsConfirmed", IsConfirmed)
        paraMap.Add("State_User", User_Display)

        Dim R As MsgReturn = SqlStrToOneStr("P27210_ReturnGoods_Confirmed", paraMap, True)
        If R.IsOk Then
