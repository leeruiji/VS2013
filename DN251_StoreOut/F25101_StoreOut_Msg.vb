...
Public Class F25101_StoreOut_Msg
    Dim StoreOutID As String = ""
    Dim dtList As DataTable
    Dim dtTable As DataTable

    Dim Dt_Area As DataTable
    ...
    Private Sub F20001_StoreOut_Msg_Form_KeyDown(ByVal sender As Object, ByRef e As System.Windows.Forms.KeyEventArgs) Handles Me.Form_KeyDown
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        Dt_Area = BaseClass.ComFun.GetAreaDt
        CB_Area.DataSource = Dt_Area
    ...
    Protected Sub Me_Refresh()
        Dim msgTable As DtReturnMsg = StoreOut_GetById(StoreOutID)
        If msgTable.IsOk Then
            dtTable = msgTable.Dt
            SetForm(msgTable.Dt)
        End If
        Dim msg As DtReturnMsg = StoreOut_GetListById(StoreOutID)
        If msg.IsOk Then
            dtList = msg.Dt
...
#Region "表单信息"
    ...
    Protected Function GetForm() As DataTable
        ...
        If Not dt Is Nothing AndAlso Not TB_ID.Text = "" Then
            ...
            dr("StoreOut_ID") = TB_ID.Text
            dr("StoreOut_Date") = DP_Date.Value.ToString("yyyy-MM-dd")
            dr("Project") = TB_Project.Text
            dr("Builder") = TB_Builder.Text
            dr("Area") = CB_Area.Text


            dr("SumAmount") = TB_SumAmount.Text
            dr("SumQty") = TB_SumQty.Text
            dr("Paid") = Val(TB_Paid.Text)
            dr("Unpaid") = Val(TB_Unpaid.Text)
            dr("DisCount") = Val(TB_Discount.Text)
    ...
    Protected Sub SetForm(ByVal dt As DataTable)
        ...
        If dt.Rows.Count = 1 Then
            TB_ID.Text = IsNull(dt.Rows(0)("StoreOut_ID"), "")
            If Not dt.Rows(0)("StoreOut_Date") Is DBNull.Value Then
                DP_Date.Value = dt.Rows(0)("StoreOut_Date")
            End If
            TB_Project.Text = IsNull(dt.Rows(0)("Project"), "")
            TB_Builder.Text = IsNull(dt.Rows(0)("Builder"), "")
            CB_Area.Text = IsNull(dt.Rows(0)("Area"), "")


            TB_SumQty.Text = FormatNum(IsNull(dt.Rows(0)("SumQty"), 0))
            TB_SumAmount.Text = FormatMoney(IsNull(dt.Rows(0)("SumAmount"), 0))
            TB_Paid.Text = FormatMoney(IsNull(dt.Rows(0)("Paid"), 0))
            TB_Unpaid.Text = FormatMoney(IsNull(dt.Rows(0)("Unpaid"), 0))
            TB_Discount.Text = FormatMoney(IsNull(dt.Rows(0)("DisCount"), 0))
    ...
    Protected Sub CaculateSumAmount()
        ...
        For i As Integer = 1 To Fg1.Rows.Count - 1
            ...
            Fg1.Item(i, "QTY") = qty
            amount = FormatMoney(qty * dprice)
            Fg1.Item(i, "Amount") = amount
            sumAmout = sumAmout + amount
            sunQty = sunQty + qty
        Next
        TB_SumAmount.Text = FormatMoney(sumAmout)
...
#Region "表单事件"
    ...
    Private Sub TB_SumAmount_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TB_SumAmount.TextChanged, TB_Discount.TextChanged
        CaculatUnpaid()
    End Sub

    Protected Sub CaculatUnpaid()
        Dim sumAmout As Double = Val(TB_SumAmount.Text)
        Dim paid As Double = Val(TB_Paid.Text)
        Dim Unpaid As Double = Val(TB_Unpaid.Text)
        Dim discount As Double = Val(TB_Discount.Text)

        TB_Unpaid.Text = FormatMoney(sumAmout - discount - paid)
    End Sub
...
#Region "工具栏按钮"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SaveStoreOut()
        If CheckForm() Then
            dtList.AcceptChanges()
            Dim R As DtListReturnMsg
            If Me.Mode = Mode_Enum.Add Then
                If StoreOutID = TB_ID.Text Then GetNewID()
                R = StoreOut_Add(GetForm(), dtList)
            Else
                R = StoreOut_Save(GetForm(), dtList)
            End If
            If R.IsOk Then
    ...
    Protected Function CheckForm() As Boolean
        ...
        If TB_Project.Text = "" Then
            TB_ID.Focus()
            ShowMsg(Ch_Name & "项目不能为空", Me.Title)
            Return False
        End If
        If CB_Area.Text = "" Then
            TB_ID.Focus()
            ShowMsg(Ch_Name & "地区不能为空", Me.Title)
            Return False
        End If
    ...
    Protected Sub DelStoreOut()
        Dim msg As MsgReturn = StoreOut_Del(TB_ID.Text)
        If msg.IsOk Then
            Me.Close()
        Else
...
#Region "FG 事件"
    ...
    Private Sub Fg1_KeyDownEdit(ByVal sender As Object, ByVal e As C1.Win.C1FlexGrid.KeyEditEventArgs) Handles Fg1.KeyDownEdit
        ...
        If lastKey = Keys.Add AndAlso Fg1.Cols("Goods_No").Index = e.Col AndAlso Fg1.CanEditing Then
            ShowGoodsSel()
        End If
        If (lastKey = Keys.Down OrElse lastKey = Keys.Up) AndAlso (Fg1.Cols("Goods_Spec").Index = e.Col OrElse Fg1.Cols("Goods_Thick").Index = e.Col) AndAlso Fg1.CanEditing Then
    ...
    Private Sub Fg1_KeyPressEdit(ByVal sender As Object, ByVal e As C1.Win.C1FlexGrid.KeyPressEditEventArgs) Handles Fg1.KeyPressEdit
        If Fg1.Cols("Goods_Spec").Index = e.Col And e.Row > 0 Then
            TimerCombox.Start()
            Dim s As String
...
#Region "规格和厚度列表"
    ...
    Private Sub ListBox_Show_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ListBox_Show.Click
        If Fg1.ColSel = ListBox_Show.Tag AndAlso Fg1.RowSel = ListBox_Show.AccessibleName Then
            Fg1.Item(Fg1.RowSel, Fg1.ColSel) = ListBox_Show.Text
            Fg1.Focus()
            lastKey = Keys.Enter
...
#Region "获取新单号"
    ...
    Sub GetNewID()
        If Mode = Mode_Enum.Add Then
            Dim msg As RetrunNewIdMsg = StoreOut_GetNewId(DP_Date.Value)
            If msg.IsOk Then
                DP_Date.MinDate = msg.MaxDate.AddDays(1)
                DP_Date.Value = msg.NewIdDate
                StoreOutID = msg.NewID
                TB_ID.Text = StoreOutID
