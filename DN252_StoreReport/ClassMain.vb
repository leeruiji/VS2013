...
#End Region
    ...
    Public Sub New(ByVal AppDes As String) '类创建事件
        ...
        ''---------主模块------------
        ModuleX = New MainModuleTpye
        ModuleX.ID = 252                   '===需修改=========== 模块ID
        ModuleX.Caption = "库存报表"      '===需修改=========== 模块名
        CModule.Add(ModuleX)


        '---------主窗口----------
        FormX = New DLLTpye
        FormX.Caption = "采购明细"         '===需修改=========== 窗口菜单名
        FormX.FormName = "F20010_Purchase_Summary"    '===需修改=========== 窗口调用名
        FormX.Img = My.Resources.StoreHalf    '===需修改=========== 菜单图标
        ...
        FormX.Right.Add(New RightTpye With {.RightName = Print, .RightShow = "报表打印"})
        CForm.Add(FormX)


        '---------主窗口----------
        FormX = New DLLTpye
        FormX.Caption = "出库明细"         '===需修改=========== 窗口菜单名
        FormX.FormName = "F25110_StoreOut_Summary"    '===需修改=========== 窗口调用名
        FormX.Img = My.Resources.StoreGoods    '===需修改=========== 菜单图标
        FormX.ID = CInt(FormX.FormName.Substring(1, 5))
        FormX.IsShow = True                '===需修改=========== 显示在菜单上直接调用,false 可以间接调用
        FormX.LoadMode = 0                 '加载模式 0为MDI窗口 1为固定窗口例如修改密码窗口之类,运行时锁死其他窗口
        FormX.DllAss_Index = 0
        ...
        '---------主窗口----------
        FormX = New DLLTpye
        FormX.Caption = "半成品库存"         '===需修改=========== 窗口菜单名
        FormX.FormName = "F25200_StoreHalf"    '===需修改=========== 窗口调用名
        FormX.Img = My.Resources.StoreHalf    '===需修改=========== 菜单图标
        FormX.ID = CInt(FormX.FormName.Substring(1, 5))
        FormX.IsShow = True                '===需修改=========== 显示在菜单上直接调用,false 可以间接调用
        FormX.LoadMode = 0                 '加载模式 0为MDI窗口 1为固定窗口例如修改密码窗口之类,运行时锁死其他窗口
        ...
        '---------主窗口----------
        FormX = New DLLTpye
        FormX.Caption = "成品库存"         '===需修改=========== 窗口菜单名
        FormX.FormName = "F25201_StoreGoods"    '===需修改=========== 窗口调用名
        FormX.Img = My.Resources.StoreGoods    '===需修改=========== 菜单图标
        FormX.ID = CInt(FormX.FormName.Substring(1, 5))
        FormX.IsShow = True                '===需修改=========== 显示在菜单上直接调用,false 可以间接调用
        FormX.LoadMode = 0                 '加载模式 0为MDI窗口 1为固定窗口例如修改密码窗口之类,运行时锁死其他窗口
