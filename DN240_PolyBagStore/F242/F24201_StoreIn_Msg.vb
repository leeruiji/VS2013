...
Public Class F24201_StoreIn_Msg
    Dim LId As String = ""
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        ComFun.DelBillNewID(BillType.StoreIn_Bag, Bill_ID)
    End Sub

    Private Sub Form_Load() Handles Me.Me_Load
        Fg1.Cols("Store_Name").Editor = CB_Unit_LL
        If Bill_Id = "" AndAlso P_F_RS_ID <> "" Then
            Bill_Id = P_F_RS_ID
        End If
        Fg1.Cols("WL_Unit").Editor = CB_Unit_LL

        Dim remarkType As Enum_RemarkType
        Select Case ID
            Case 20200
                Dao.StoreIn_DB_NAME = Ch_Name
                LB_Workshop.Visible = False
                CB_Dept_No.Visible = False
                StoreInType = Enum_StoreIn_Type.Other
                remarkType = Enum_RemarkType.StoreIn_Reason
                ID = 20200
            Case 20210
                Fg1.Cols("Price").AllowEditing = False
                Dao.StoreIn_DB_NAME = Ch_Name
                StoreInType = Enum_StoreIn_Type.TuiLiao
                LB_Produce.Visible = False
                TB_Produce_ID.Visible = False
                Cmd_ProduceSel.Visible = False
    ...
    Protected Sub Me_Refresh()
        Dim msgTable As DtReturnMsg = Dao.StoreIn_GetById(Bill_Id)
        ...
        Select Case Mode
            Case Mode_Enum.Add
                ...
                Dim msg As DtReturnMsg = Dao.StoreIn_SelListById(0)
            ...
            Case Mode_Enum.Modify
                Dim msg As DtReturnMsg = Dao.StoreIn_SelListById(Bill_Id)
...
#Region "表单信息"
    ...
    Protected Sub SetForm(ByVal dt As DataTable)
        ...
        If IsBidings = False Then
            ...
            'TB_Payed.DataBindings.Add("Text", dtTable, TB_Payed.Name.Substring(3), True, DataSourceUpdateMode.OnPropertyChanged, FormatSharp("money"), 0)

            '  TB_UnPayed.DataBindings.Add("Text", dtTable, TB_UnPayed.Name.Substring(3), True, DataSourceUpdateMode.OnPropertyChanged, FormatSharp("money"), 0)
...
#Region "工具栏按钮"
    ...
    Private Sub Cmd_Modify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Modify.Click
        ...
        If CheckForm() Then
            If Mode = Mode_Enum.Add Then
                ShowConfirm("是否保存新" & Ch_Name & "?", AddressOf SaveStoreIn)
            Else
                If CheckIsModify() Then
                    ShowConfirm(Ch_Name & "[" & TB_ID.Text & "] 已经被其他人修改过,是否覆盖别人的修改?", AddressOf SaveStoreIn)
                Else
                    ShowConfirm("是否保存" & Ch_Name & "[" & TB_ID.Text & "] 的修改?", AddressOf SaveStoreIn)
                End If
            End If
        End If
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SaveStoreIn(Optional ByVal Audit As Boolean = False)
        Dim R As MsgReturn
        ...
        Dt.Rows(0).Item("StoreInType") = Me.StoreInType

        If Me.Mode = Mode_Enum.Add Then
            R = Dao.StoreIn_Add(Dt, dtList)
        Else
            R = Dao.StoreIn_Save(LId, Dt, dtList)
        End If
        If R.IsOk Then
            LId = TB_ID.Text
            ...
            If Audit Then
                Dim msg As MsgReturn = Dao.StoreIn_Audited(TB_ID.Text, True)
                If msg.IsOk Then
                    Bill_Id = TB_ID.Text
                    Me_Refresh()
                    ShowOk(msg.Msg)
    ...
    Protected Function CheckForm() As Boolean
        ...
        If TB_ID.Text = "" Then
            ...
            Return False
        End If
        If (Mode = Mode_Enum.Add Or (Mode = Mode_Enum.Modify And LId <> TB_ID.Text)) AndAlso Dao.StoreIn_CheckID(TB_ID.Text) Then
            ShowErrMsg(Ch_Name & "单号[" & TB_ID.Text & "]已经存在!", AddressOf TB_ID.Focus)
            Return False
        End If
        If CB_Reason.Text = "" Then
            ShowErrMsg("入库原因不能为空!", AddressOf CB_Reason.Focus)
            Return False
        End If
        If Me.StoreInType = Enum_StoreIn_Type.TuiLiao AndAlso CB_Dept_No.SelectedIndex < 0 Then
    ...
    Private Sub Cmd_ChooseBill_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_ChooseBill.Click
        ShowBill()
    End Sub
#End Region

#Region "引用单据"
    Protected Sub ShowBill(Optional ByVal _no As String = "")
        Dim F As BaseForm = LoadFormIDToChild(20000, Me)
        If F Is Nothing Then Exit Sub
        ...
        If Fg1.LastKey = Keys.Enter Then
            ...
            Select Case Fg1.Cols(e.Col).Name
                ...
                Case "Price", "Qty"
                    ...
                    If Fg1.Cols(e.Col).Name = "Qty" AndAlso Fg1.Rows.Count > 2 Then
                        Fg1.Item(Fg1.Rows.Count - 1, "WL_ID") = Fg1.Item(Fg1.Rows.Count - 2, "WL_ID")
                        Fg1.Item(Fg1.Rows.Count - 1, "WL_Name") = Fg1.Item(Fg1.Rows.Count - 2, "WL_Name")
                        Fg1.Item(Fg1.Rows.Count - 1, "Price") = Fg1.Item(Fg1.Rows.Count - 2, "Price")
                        Fg1.Item(Fg1.Rows.Count - 1, "WL_Spec") = Fg1.Item(Fg1.Rows.Count - 2, "WL_Spec")
                        Fg1.Item(Fg1.Rows.Count - 1, "Amount") = Fg1.Item(Fg1.Rows.Count - 2, "Amount")
                        Fg1.Item(Fg1.Rows.Count - 1, "WL_Unit") = Fg1.Item(Fg1.Rows.Count - 2, "WL_Unit")
                        Fg1.Item(Fg1.Rows.Count - 1, "WL_No") = Fg1.Item(Fg1.Rows.Count - 2, "WL_No")
                    End If
                Case Else
                    Fg1.GotoNextCell(e.Col)
            End Select
...
#Region "物料选择"
    ...
    Protected Sub SetGoods()
        If Not Me.ReturnObj Is Nothing AndAlso Fg1.CanEditing AndAlso Fg1.RowSel > 0 Then
            ...
            '    Fg1.Item(Fg1.RowSel, "WL_Unit_LL2") = IsNull(dr("WL_Unit_LL"), "")
            Fg1.Item(Fg1.RowSel, "WL_Spec") = IsNull(dr("WL_Spec"), "")

            If IsNull(dr("WL_Cost"), 0) = 0 Then
                Fg1.Item(Fg1.RowSel, "Price") = Format(IsNull(dr("Wl_Price"), 0)) '* ComFun.Unit_Convert(Fg1.Item(Fg1.RowSel, "WL_Unit_LL"), Fg1.Item(Fg1.RowSel, "WL_Unit_LL2")), "0.###")
            Else
                Fg1.Item(Fg1.RowSel, "Price") = Format(IsNull(dr("WL_Cost"), 0)) '* ComFun.Unit_Convert(Fg1.Item(Fg1.RowSel, "WL_Unit_LL"), Fg1.Item(Fg1.RowSel, "WL_Unit_LL2")), "0.###")
            End If
            Fg1.Item(Fg1.RowSel, "Cost") = IsNull(dr("WL_Cost"), 0)
            CB_Unit_LL.SelectedValue = IsNull(dr("WL_Store_ID"), 0)
            Fg1.Item(Fg1.RowSel, "Store_Name") = CB_Unit_LL.Text
    ...
    Protected Sub GetWLByNo(ByVal _No As String)
        Dim msg As DtReturnMsg = Dao.PolyBag_GetByNo(_No)
    ...
    Sub GetNewID()
        ...
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.StoreIn_Bag, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        If Me.Mode = Mode_Enum.Add Then
            Exit Sub
        End If
        Dim R As New R24200_StoreIn
        R.Start(TB_ID.Text, DoOperator, Me.StoreInType)
    End Sub
...
#Region "审核状态"
    ...
    Protected Sub UnAudit()
        Dao.StoreIn_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.StoreIn_Audited(TB_ID.Text, False)
        If msg.IsOk Then
            ShowOk(msg.Msg)
            Me_Refresh()
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        ShowConfirm("是否删除" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf DelStoreIn)
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub DelStoreIn()
        Dao.StoreIn_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.StoreIn_Del(TB_ID.Text)
        If msg.IsOk Then
            LastForm.ReturnId = TB_ID.Text
            Me.Close()
        Else
    ...
    Private Sub Cmd_SetInValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetInValid.Click
        ShowConfirm("是否作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf StoreInInValid)
    End Sub


    Sub StoreInInValid()
        Dao.StoreIn_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.StoreIn_InValid(TB_ID.Text, True)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
    ...
    Private Sub Cmd_SetValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetValid.Click
        ShowConfirm("是否反作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf StoreInValid)
    End Sub
    ...
    Sub StoreInValid()
        Dao.StoreIn_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.StoreIn_InValid(TB_ID.Text, False)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
...
#Region "常量"
    Protected Friend Shared StoreIn_DB_NAME As String = "入库单"

    ''' <summary>
    ''' 删除单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_DelByid As String = "Delete from T24200_StoreIn_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_GetByid As String = "select top 1 * from T24200_StoreIn_Table  where ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_SelByid As String = "select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T24200_StoreIn_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T24200_StoreIn_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_GetListByid As String = "select * from T24201_StoreIn_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_StoreIn_SelListByid As String = "select L.*,L.WL_Unit_LL as WL_Unit_LL2,L.WL_Unit_LL as WL_Unit_LL2,WL.WL_No,WL.WL_Name,WL.WL_Spec from (select * from T24201_StoreIn_List  where ID=@ID)L left join T24001_PolyBag WL on WL.id=l.WL_ID"
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_CheckID As String = "select count(*) from T24200_StoreIn_Table  where ID=@ID"

    ''' <summary>
    ''' 检查其他人有没有修改过
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreIn_CheckIsModify As String = "select count(*) from T24201_StoreIn_list  where ID=@ID and Line=@Line"

    Private Const SQL_PolyBag_GetByNo As String = "select Top 1 Id,WL_No,WL_Name,WL_Unit,WL_Spec,WL_Store_ID,WL_Cost,WL_Type_ID,WL_Price from T24001_PolyBag where WL_No=@WL_No and isnull(WL_Disable,0)<>1"
...
#Region "单一张单内容查询"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_StoreIn_GetByid, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_SelListById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_StoreIn_SelListByid, "@ID", sId)
    End Function
#End Region
...
#Region "添加修改删除"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_CheckID(ByVal ID As String) As Boolean
        Dim R As MsgReturn = SqlStrToOneStr(SQL_StoreIn_CheckID, "@ID", ID)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return False
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim StoreInId As String = dtTable.Rows(0)("ID")
        Dim IsInsert As Boolean = False
        paraMap.Add("ID", StoreInId)
        Try
            ...
            If IsNull(dtTable.Rows(0)("Produce_ID"), "") <> "" Then
                ...
                If Rx.IsOk = False Then
                    R.IsOk = False
                    R.Msg = StoreIn_DB_NAME & "[" & StoreInId & "]添加失败!" & vbCrLf & Rx.Msg
                    Return R
                End If
            End If

            sqlMap.Add("Table", SQL_StoreIn_GetByid)
            sqlMap.Add("List", SQL_StoreIn_GetListByid)
            ...
            If msg.DtList("Table").Rows.Count = 0 Then
                ...
                Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & StoreInId & "'," & BillType.StoreIn & ")"
                DtToUpDate(msg, TmSQL)
                R.Msg = "" & StoreIn_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & StoreIn_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & StoreIn_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim StoreInId As String = dtTable.Rows(0)("ID")
        If LID <> StoreInId AndAlso StoreIn_CheckID(StoreInId) Then
            R.IsOk = False
            R.Msg = "" & StoreIn_DB_NAME & "[" & StoreInId & "]已经存在!请双击编号文本框,获取新编号!"
            Return R
        End If
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Audited Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & StoreIn_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被审核不能再修改!"
                    Return R
                End If
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Invoid Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & StoreIn_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
                    Return R
                End If
                DvToDt(dtList, msg.DtList("List"), New List(Of String))
                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & StoreInId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.StoreIn
                If LID <> StoreInId Then
                    DtToUpDate(msg, "Update T24200_StoreIn_Table set id='" & StoreInId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
                    dtTable.Rows(0)("ID") = StoreInId
                    R.Msg = "" & StoreIn_DB_NAME & "[" & LID & "]已经修改为[" & StoreInId & "]!"
                Else
                    DtToUpDate(msg)
                    R.Msg = "" & StoreIn_DB_NAME & "[" & LID & "]修改成功!"
                End If
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & StoreIn_DB_NAME & "[" & LID & "]不存在"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & StoreIn_DB_NAME & "[" & LID & "]修改错误"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' 删除入库单
    ''' </summary>
    ''' <param name="StoreInId"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_Del(ByVal StoreInId As String)
        Return RunSQL(SQL_StoreIn_DelByid, "@ID", StoreInId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        ...
        paraMap.Add("ID", _ID)
        Try
            sqlMap.Add("Table", SQL_StoreIn_GetStateByid)
            msg = SqlStrToDt(sqlMap, paraMap)
            If msg.DtList("Table").Rows.Count = 1 Then
                If msg.DtList("Table").Rows(0)("State") <> Enum_State.Audited Then
                    '检查加工单状态
                    If IsFei = False AndAlso IsNull(msg.DtList("Table").Rows(0)("Produce_ID"), "") <> "" Then
                        ...
                        If Rx.IsOk = False Then
                            R.IsOk = False
                            R.Msg = StoreIn_DB_NAME & "[" & _ID & "]" & OStr & "失败!" & vbCrLf & Rx.Msg.Replace("不能被引用", "")
                            Return R
                        End If
                    End If
                    ...
                    If msg.DtList("Table").Rows(0)("State") <> State Then
                        ...
                        DtToUpDate(msg)
                        R.IsOk = True
                        R.Msg = "" & StoreIn_DB_NAME & "[" & _ID & "]" & OStr & "成功!"
                    Else
                        R.IsOk = False
                        R.Msg = "" & StoreIn_DB_NAME & "[" & _ID & "]已经被" & OStr & "!不能再" & OStr & "!"
                    End If
                Else
                    R.IsOk = False
                    R.Msg = "" & StoreIn_DB_NAME & "[" & _ID & "]已经被审核,不能" & OStr & "!"
                End If
            Else
                R.IsOk = False
                R.Msg = "" & StoreIn_DB_NAME & "[" & _ID & "]不存在,不能" & OStr & "!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & StoreIn_DB_NAME & "[" & _ID & "]" & OStr & "错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StoreIn_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("ID", _ID)
        paraMap.Add("IsAudited", IsAudited)
        paraMap.Add("State_User", User_Display)
        Dim R As MsgReturn = SqlStrToOneStr("P24200_StoreIn_Audited", paraMap, True)
        If R.IsOk Then
