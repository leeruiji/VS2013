...
Public Class F24000_PolyBag
    ...
    Public Sub New()
        ...
        GoodsTypeTree1.SetRootID(GoodsTypeTree.NODE_PolyBag)
    ...
    Private Sub Form_WL_Me_Load() Handles MyBase.Me_Load
        FG1.IniFormat()
        FormCheckRight()
        FG1.IniColsSize()
    ...
    Private Sub F10000_WL_Form_KeyDown(ByVal sender As Object, ByRef e As System.Windows.Forms.KeyEventArgs) Handles MyBase.Form_KeyDown
    ...
    Private Sub F10000_WL_Form_KeyPress(ByVal sender As Object, ByRef e As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.Form_KeyPress
...
#Region "控件事件"
    ...
    Protected Sub AddGoods()
        If GoodsTypeTree1.TV_GoodsType.SelectedNode Is Nothing Then
            ...
            Exit Sub
        End If
        Dim F As New F24001_PolyBag_Msg(GoodsTypeTree1.TV_GoodsType.SelectedNode.Name, GoodsTypeTree1.TV_GoodsType.SelectedNode.Text, GoodsTypeTree1.TV_GoodsType.SelectedNode.Tag, "0")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ID
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ModifyGoods()
        If FG1.RowSel <= 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F24001_PolyBag_Msg(FG1.SelectItem("ID"))
        With F
...
#Region "报表"
    ...
    Protected Sub Print(ByVal type As PClass.CReport.OperatorType)
        If dtGoods Is Nothing Then
            Exit Sub
        End If
        Dim rptLoader As New R24000_Goods_List()
...
#Region "显示明细"
    ...
    Private Sub FG1_SelectRowChange(ByVal Row As Integer) Handles FG1.SelectRowChange
        If FG1.SelectItem Is Nothing OrElse IsNull(FG1.SelectItem("ID"), 0) = 0 Then
            Exit Sub
        End If
    ...
    Private Sub Cmd_ShowList_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_ShowList.Click
        If Cmd_ShowList.Checked Then
            If MeIsLoad Then
                Dim x As Integer = My.Settings.MxDistanceWL
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DataTable = Dao.WL_GetStoreList(IsNull(FG1.SelectItem("ID"), 0))
        If msg IsNot Nothing Then
            LB_Store.DataSource = msg
            LB_Store.DisplayMember = "Msg"
            LB_Store.ValueMember = "Qty"
        Else
            LB_Store.DataSource = Nothing
        End If
    End Sub

    Private Sub SplitContainer1_SplitterMoved(ByVal sender As Object, ByVal e As System.Windows.Forms.SplitterEventArgs) Handles SplitContainer1.SplitterMoved
        If MeIsLoad Then
            My.Settings.MxDistanceWL = SplitContainer1.SplitterDistance
            My.Settings.Save()
        End If
    End Sub
#End Region
...
#Region "SQL"
    Public Const SQL_WL_GetGoodsType = "select top 1 WL_Type_ID from T24001_PolyBag where WL_No=@WL_No"

    Public Const SQL_WL_CheckDel As String = " select id, WL_No,WL_Name,WL_Spec from T24001_PolyBag where  ID=@ID   and   (   exists (select top 1 * from T24003_Store_Detail where WL_ID=@ID )  or   exists(select  top 1 * from T10011_BZC_PFList where WL_ID=@ID)  )"


    Public Const SQL_WL_NameCheckDuplicate = "select count(*) from T24001_PolyBag where WL_No=@WL_No and id<>@id"

    Public Const SQL_WL_Get As String = "select G.*,T.GoodsType_Name from T24001_PolyBag G left join T24000_GoodsType T on G.WL_Type_ID=T.GoodsType_ID   "

    Public Const SQL_WL_GetByID As String = "select * from T24001_PolyBag where ID=@ID"
    Public Const SQL_WL_GetByNo As String = "select * from T24001_PolyBag where WL_No=@WL_No"

    Public Const SQL_WL_SelByID As String = "select top 1 G.*,T.GoodsType_Name from T24001_PolyBag G left join T24000_GoodsType T on G.WL_Type_ID=T.GoodsType_ID   where G.ID=@ID"
    Public Const SQL_WL_OrderBy As String = " order by WL_Type_ID, WL_No "

    Public Const SQL_WL_DelByid As String = "Delete from  T24001_PolyBag where ID=@ID "
    Public Const SQL_WL_Enable As String = "update T24001_PolyBag set WL_Disable=0 where ID=@ID "
#End Region

#Region "物料"

    Public Shared Function WL_GetStoreList(ByVal WL_ID As Integer) As DataTable
        If WL_ID = 0 Then Return Nothing
        Dim R As DtReturnMsg = SqlStrToDt("select * from T24006_Stock where WL_ID=@WL_ID", "WL_ID", WL_ID)
        If R.IsOk Then
            R.Dt.Columns.Add("Msg")
            For Each Row As DataRow In R.Dt.Rows
                Row("Msg") = Row("Qty") & IIf(Row("Num") > 1, " X " & Row("Num"), "")
            Next
            Return R.Dt
        Else
            Return Nothing
    ...
    Public Shared Function WL_GetNewID(ByVal startNo As String) As String
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("@DB_Str", "T24001_PolyBag")
