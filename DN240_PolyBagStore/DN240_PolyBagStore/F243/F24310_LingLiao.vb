...
Partial Friend Class Dao
    ...
    '===================出库单信息==============
    Public Const SQL_LingLiao_Get_WithName = "select T.*,C.Client_Name,BZ_Name from T21310_LingLiao_Table T left join T10110_Client C on T.Client_ID=C.ID  left join T10002_BZ BZ on BZ.ID=T.BZ_ID"
    ...
    Public Shared Function LingLiaoSummary_GetConditionNames() As List(Of FindOption)
        ...
        fo.SQL = "exists(select * from T21001_Metal TW,T21311_LingLiao_List TL where %WL_Name% and TL.wl_id=TW.ID and T21310_LingLiao_Table.id=TL.id)"
        fo.Sign = "%WL_Name%"
        foList.Add(fo)
        ...
        fo.DB_Field = " "
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "(T21310_LingLiao_Table.ID %Like% or  SumQty %Like% or SumAmount %Like% or Remark %Like%  )"
        fo.Sign = "%Like%"
        foList.Add(fo)
