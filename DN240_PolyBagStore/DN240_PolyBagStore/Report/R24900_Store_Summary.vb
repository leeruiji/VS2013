...
Partial Friend Class Dao
    ...
    Public Shared Function StoreSummary_Get(ByVal startDate As Date, ByVal endDate As Date, ByVal Olist As List(Of FindOption)) As DtReturnMsg
        ...
        sqlBuider.Append(" from  T21001_Metal  W ")
        sqlBuider.Append(" left join ( ")
        sqlBuider.Append(" select WL_ID,sum(Qty*StoreINorOut) StartQty,sum(Qty*StoreINorOut*Cost) StartAmount from T21003_Store_Detail where sDate < @StartDate  ")
        sqlBuider.Append(" group by WL_ID)S on W.id=s.Wl_id ")
        sqlBuider.Append(" left join ( ")
        sqlBuider.Append(" select WL_ID,sum(Qty) InQty,sum(Qty*StoreINorOut*Cost) InAmount from T21003_Store_Detail where sDate between @StartDate and @EndDate ")
        sqlBuider.Append(" and StoreINorOut =1 ")
        sqlBuider.Append(" group by WL_ID)I on W.id=i.Wl_id ")

        sqlBuider.Append(" left join ( ")
        sqlBuider.Append(" select WL_ID,sum(Qty*StoreINorOut) OutQty,sum(Qty*StoreINorOut*Cost) OutAmount from T21003_Store_Detail where sDate between @StartDate and @EndDate ")

        sqlBuider.Append(" and StoreINorOut =-1 ")
        sqlBuider.Append(" group by WL_ID)O on W.id=o.Wl_id ")
        sqlBuider.Append(" left join ( ")
        sqlBuider.Append(" select WL_ID,sum(Qty*StoreINorOut) EndQty,sum(Qty*StoreINorOut*Cost) EndAmount from T21003_Store_Detail where sDate <= @EndDate   ")
        sqlBuider.Append(" group by WL_ID)E on W.id=e.Wl_id  ")
        sqlBuider.Append(" left join (select T.GoodsType_id,isnull(L.GoodsType_Name,T.GoodsType_Name)GoodsType_Name from T21000_GoodsType T  ")
        sqlBuider.Append(" left join   ")
        sqlBuider.Append("  (select  L.GoodsType_id, (A.GoodsType_Name+'.'+L.GoodsType_name)GoodsType_Name from  ")
        sqlBuider.Append(" T21000_GoodsType L,T21000_GoodsType A ")
