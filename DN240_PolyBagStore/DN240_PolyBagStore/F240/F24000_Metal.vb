...
Public Class F24000_Metal
    ...
    Public Sub New()
        ...
        GoodsTypeTree1.SetRootID(GoodsTypeTree.NODE_Metal)
...
#Region "控件事件"
    ...
    Protected Sub AddGoods()
        If GoodsTypeTree1.TV_GoodsType.SelectedNode Is Nothing Then
            ...
            Exit Sub
        End If
        Dim F As New F24001_Metal_Msg(GoodsTypeTree1.TV_GoodsType.SelectedNode.Name, GoodsTypeTree1.TV_GoodsType.SelectedNode.Text, GoodsTypeTree1.TV_GoodsType.SelectedNode.Tag, "0")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ID
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ModifyGoods()
        If FG1.RowSel <= 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F24001_Metal_Msg(FG1.SelectItem("ID"))
        With F
...
#Region "SQL"
    Public Const SQL_WL_GetGoodsType = "select top 1 WL_Type_ID from T21001_Metal where WL_No=@WL_No"

    Public Const SQL_WL_CheckDel As String = " select id, WL_No,WL_Name,WL_Spec from T21001_Metal where  ID=@ID   and   (   exists (select top 1 * from T21003_Store_Detail where WL_ID=@ID )  or   exists(select  top 1 * from T10011_BZC_PFList where WL_ID=@ID)  )"


    Public Const SQL_WL_NameCheckDuplicate = "select count(*) from T21001_Metal where WL_No=@WL_No and id<>@id"

    Public Const SQL_WL_Get As String = "select G.*,T.GoodsType_Name from T21001_Metal G left join T21000_GoodsType T on G.WL_Type_ID=T.GoodsType_ID   "

    Public Const SQL_WL_GetByID As String = "select * from T21001_Metal where ID=@ID"
    Public Const SQL_WL_GetByNo As String = "select * from T21001_Metal where WL_No=@WL_No"

    Public Const SQL_WL_SelByID As String = "select top 1 G.*,T.GoodsType_Name from T21001_Metal G left join T21000_GoodsType T on G.WL_Type_ID=T.GoodsType_ID   where G.ID=@ID"
    Public Const SQL_WL_OrderBy As String = " order by WL_Type_ID, WL_No "

    Public Const SQL_WL_DelByid As String = "Delete from  T21001_Metal where ID=@ID "
    Public Const SQL_WL_Enable As String = "update T21001_Metal set WL_Disable=0 where ID=@ID "
#End Region
...
#Region "物料"
    ...
    Public Shared Function WL_GetNewID(ByVal startNo As String) As String
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("@DB_Str", "T21001_Metal")
