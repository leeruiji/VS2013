...
Public Class F24401_StockAdjust_Msg
    Dim LId As String = ""
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        ComFun.DelBillNewID(BillType.StockAdjust, Bill_ID)
    End Sub

    Private Sub Form_Load() Handles Me.Me_Load
        Dim C As New DatePickerValueChangeHandler(DTP_sDate, AddressOf Getisloaded, AddressOf Date_Change)
        '    Fg1.Cols("Store_Name").Editor = CB_Store
        If Bill_ID = "" AndAlso P_F_RS_ID2 <> "" Then
            Bill_ID = P_F_RS_ID
        End If
        FormCheckRight()
        Ch_Name = GetBillTypeName(21400)
        Me.Title = Ch_Name
        Fg1.IniFormat()
    ...
    Protected Sub Me_Refresh()
        Dim msgTable As DtReturnMsg = Dao.StockAdjust_GetById(Bill_ID)
        ...
        Select Case Mode
            Case Mode_Enum.Add
                ...
                Dim msg As DtReturnMsg = Dao.StockAdjust_SelListById(0)
            ...
            Case Mode_Enum.Modify
                Dim msg As DtReturnMsg = Dao.StockAdjust_SelListById(Bill_ID)
...
#Region "表单信息"
    ...
    Protected Function GetForm() As DataTable
        ...
        For i As Integer = 1 To Fg1.Rows.Count - 1
            ldr = dtList.NewRow
            If Fg1.Item(i, "WL_ID") Is Nothing OrElse Val(Fg1.Item(i, "WL_ID")) = 0 Then
                Continue For
            End If
            If dtList.Select("WL_ID=" & Fg1.Item(i, "WL_ID")).Length Then
                ShowErrMsg("物料[" & Fg1.Item(i, "WL_No") & "][" & Fg1.Item(i, "WL_Name") & "]已经存在")
                Return Nothing
            End If
            ldr("ID") = ID
            ldr("WL_ID") = Fg1.Item(i, "WL_ID")
            ldr("Qty") = Val(Fg1.Item(i, "Qty"))
            ldr("Qty_Last") = Val(Fg1.Item(i, "Qty_Last"))
            ldr("Loss") = Val(Fg1.Item(i, "Loss"))
            ldr("Cost") = Val(Fg1.Item(i, "Cost"))
            '  dr("Price") = Val(Fg1.Item(i, "Cost"))
            ldr("LRemark") = Fg1.Item(i, "LRemark")
            dtList.Rows.Add(ldr)
        Next
        dtList.AcceptChanges()
        Return dt
    ...
    Protected Sub SetForm(ByVal dt As DataTable)
        ...
        If IsBidings = False Then
            ...
            TB_SumLoss.DataBindings.Add("Text", dtTable, TB_SumLoss.Name.Substring(3), True, DataSourceUpdateMode.OnPropertyChanged, 0, FormatSharp("num"))
    ...
    '        Fg1.Item(Fg1.RowSel, "Loss") = Val(Fg1.Item(Fg1.RowSel, "Qty_Last")) - Val(Fg1.Item(Fg1.RowSel, "Qty"))
    '    End If
    'End Sub
...
#Region "工具栏按钮"
    ...
    Private Sub Cmd_Modify_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Modify.Click
        ...
        If CheckForm() Then
            If Mode = Mode_Enum.Add Then
                ShowConfirm("是否保存新" & Ch_Name & "?", AddressOf SaveStockAdjust)
            Else
                ' If CheckIsModify() Then
                '  ShowConfirm(Ch_Name & "[" & TB_ID.Text & "] 已经被其他人修改过,是否覆盖别人的修改?", AddressOf SaveStockAdjust)
                '  Else
                ShowConfirm("是否保存" & Ch_Name & "[" & TB_ID.Text & "] 的修改?", AddressOf SaveStockAdjust)
                '  End If
            End If
        End If
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub SaveStockAdjust(Optional ByVal Audit As Boolean = False)
        Dim R As MsgReturn
        ...
        End If
        If Me.Mode = Mode_Enum.Add Then
            R = Dao.StockAdjust_Add(Dt, dtList)
        Else
            R = Dao.StockAdjust_Save(LId, Dt, dtList)
        End If
        If R.IsOk Then
            LId = TB_ID.Text
            ...
            If Audit Then
                Dim msg As MsgReturn = Dao.StockAdjust_Audited(TB_ID.Text, True)
                If msg.IsOk Then
                    Bill_ID = TB_ID.Text
                    Me_Refresh()
                    ShowOk(msg.Msg)
    ...
    Protected Function CheckForm() As Boolean
        ...
        If TB_ID.Text = "" Then
            ...
            Return False
        End If
        If (Mode = Mode_Enum.Add Or (Mode = Mode_Enum.Modify And LId <> TB_ID.Text)) AndAlso Dao.StockAdjust_CheckID(TB_ID.Text) Then
...
#Region "FG 事件"
    ...
    Private Sub CaculateSumQty()
        ...
        Dim SumQty_Last As Double = 0
        Dim SumLoss As Double = 0
        For R As Integer = 1 To Fg1.Rows.Count - 1
            Fg1.Item(Fg1.RowSel, "Loss") = Val(Fg1.Item(Fg1.RowSel, "Qty_Last")) - Val(Fg1.Item(Fg1.RowSel, "Qty"))

            SumLoss = SumLoss + Val(Fg1.Item(R, "Loss"))
            SumQty_Last = SumQty_Last + Val(Fg1.Item(R, "Qty_Last"))
            SumQty = SumQty + Val(Fg1.Item(R, "Qty"))
        Next
        TB_SumQty.Text = FormatNum(SumQty)
        TB_SumLoss.Text = FormatNum(SumLoss)
        TB_Qty_Last.Text = FormatNum(SumQty_Last)
    End Sub
    ...
    Protected Sub GetWLByNo(ByVal _No As String)
        Dim msg As DtReturnMsg = ComFun.PolyBag_GetByNo(_No)
    ...
    Private Sub Fg1_NextCell(ByVal e As PClass.FG.NextCellEventArgs) Handles Fg1.NextCell
        Select Case e.Col
            ...
            Case "LRemark"
                ...
                Else
                    e.ToRow = e.Row
                    e.ToCol = Fg1.Cols("LRemark").Index
                End If
        End Select
    End Sub
...
#Region "物料选择"
    ...
    Protected Sub ShowGoodsSel(Optional ByVal _no As String = "")
        ...
        Dim F As BaseForm = LoadFormIDToChild(24002, Me)
        If F Is Nothing Then Exit Sub
        Fg1.RowSel = Fg1.RowSel
        With F
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = _no
            .P_F_RS_ID3 = Enum_WLSelectMode.MutiRow
            .P_F_RS_ID4 = gType
            .IsSel = True
        End With
        ReturnId = ""
    ...
    Protected Sub SetGoods()
        If Not Me.ReturnObj Is Nothing AndAlso Fg1.CanEditing AndAlso Fg1.RowSel > 0 Then
            If TypeOf (ReturnObj) Is DataTable Then
                Dim dt As DataTable = CType(ReturnObj, DataTable)
                Dim i As Integer = 0
                For Each dr As DataRow In dt.Rows
                    ...
                    End If
                    Fg1.FinishEditing(False)
                    Fg1.Item(Fg1.RowSel, "WL_No") = dr("WL_No")
                    Fg1.Item(Fg1.RowSel, "WL_ID") = dr("ID")
                    Fg1.Item(Fg1.RowSel, "WL_Name") = dr("WL_Name")
                    Fg1.Item(Fg1.RowSel, "WL_Spec") = dr("WL_Spec")
                    Fg1.Item(Fg1.RowSel, "Qty_Last") = dr("WL_Qty")
                    Fg1.Item(Fg1.RowSel, "Cost") = dr("WL_Cost")
                    '    Fg1.Item(Fg1.RowSel, "Price") = dr("WL_Cost")
                    i = i + 1
                Next
            Else
                Dim dr As DataRow = CType(ReturnObj, DataRow)
                Fg1.FinishEditing(False)
                Fg1.Item(Fg1.RowSel, "WL_No") = dr("WL_No")
                Fg1.Item(Fg1.RowSel, "WL_ID") = dr("ID")
                Fg1.Item(Fg1.RowSel, "WL_Name") = dr("WL_Name")
...
#Region "获取新单号"
    ...
    Sub GetNewID()
        Dao.StockAdjust_DB_NAME = Ch_Name
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.StockAdjust, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        ...
        Dim R As New R24400_StockAdjust_List
...
#Region "审核状态"
    ...
    Protected Sub UnAudit()
        Dao.StockAdjust_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.StockAdjust_Audited(TB_ID.Text, False)
        If msg.IsOk Then
            ShowOk(msg.Msg)
            Me_Refresh()
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        ShowConfirm("是否删除" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf DelStockAdjust)
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Protected Sub DelStockAdjust()
        Dao.StockAdjust_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.StockAdjust_Del(TB_ID.Text)
        If msg.IsOk Then
            LastForm.ReturnId = TB_ID.Text
            Me.Close()
        Else
    ...
    Private Sub Cmd_SetInValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetInValid.Click
        ShowConfirm("是否作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf StockAdjustInValid)
    End Sub


    Sub StockAdjustInValid()
        Dao.StockAdjust_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.StockAdjust_InValid(TB_ID.Text, True)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
    ...
    Private Sub Cmd_SetValid_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_SetValid.Click
        ShowConfirm("是否反作废" & Ch_Name & " [" & TB_ID.Text & "]?", AddressOf StockAdjustValid)
    End Sub
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Sub StockAdjustValid()
        Dao.StockAdjust_DB_NAME = Ch_Name
        Dim msg As MsgReturn = Dao.StockAdjust_InValid(TB_ID.Text, False)
        If msg.IsOk Then
            Me_Refresh()
            LastForm.ReturnId = TB_ID.Text
        Else
...
#Region "检查其他人有没有修改过"
    ...
    '    If Mode <> Mode_Enum.Add Then
    '        Return Dao.StockAdjust_CheckIsModify(LId, LastLine)
    '    End If
    'End Function
...
#Region "计算开单日期的库存"
    Private Delegate Sub Del_SetQTy(ByVal dt As DataTable)




    Function Getisloaded() As Boolean
        Return isloaded
    End Function
    Sub Date_Change(ByVal NewDate As Date)
        CaculateWLQty()
        GetNewID()
    End Sub
    Private Sub Cmd_CaculateWLQTy_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_CaculateWLQTy.Click
        CaculateWLQty()
    End Sub

    Private Sub CaculateWLQty()
        Dim idList As List(Of Long) = GetWLIDList()
        If idList.Count <= 0 Then
            Exit Sub
        End If
        Dim T As New Threading.Thread(AddressOf GetWLQty)
        Dim p As New List(Of Object)
        p.Add(idList)
        p.Add(DTP_sDate.Value.Date)
        T.Start(p)
    End Sub

    Private Function GetWLIDList() As List(Of Long)
        Dim idList As New List(Of Long)
        For i As Integer = 1 To Fg1.Rows.Count - 1
            If Val(Fg1.Item(i, "WL_ID")) <> 0 Then
                idList.Add(Fg1.Item(i, "WL_ID"))
            End If
        Next
        Return idList
    End Function
    Private Sub GetWLQty(ByVal para As List(Of Object))
        Dim msg As DtReturnMsg = Dao.StockAdjust_GetWLQty(para(0), para(1))
        If msg.IsOk Then
            Dim d As New Del_SetQTy(AddressOf SetWLQty)
            Fg1.Invoke(d, msg.Dt)
        End If
    End Sub

    Private Sub SetWLQty(ByVal dt As DataTable)
        Dim drs As DataRow()
        For i As Integer = 1 To Fg1.Rows.Count - 1
            If Val(Fg1.Item(i, "WL_ID")) <> 0 Then
                drs = dt.Select("WL_ID=" & Fg1.Item(i, "WL_ID"))
                If drs.Length > 0 Then
                    Fg1.Item(i, "Qty_Last") = drs(0)("WL_Qty")
                Else
                    Fg1.Item(i, "Qty_Last") = 0
                End If
                Fg1.Item(i, "Loss") = Val(Fg1.Item(i, "Qty_Last")) - Val(Fg1.Item(i, "Qty"))
            End If
        Next
    End Sub
#End Region
...
#Region "常量"
    Protected Friend Shared StockAdjust_DB_NAME As String = "库存调整单"
    ''' <summary>
    ''' 删除单据
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_DelByid As String = "Delete from T24400_StockAdjust_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_GetByid As String = "select top 1 * from T24400_StockAdjust_Table  where ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_SelByid As String = "select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T24400_StockAdjust_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T24400_StockAdjust_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_GetListByid As String = "select * from T24401_StockAdjust_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_StockAdjust_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec from (select * from T24401_StockAdjust_List  where ID=@ID)L left join T24001_PolyBag WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StockAdjust_CheckID As String = "select count(*) from T24400_StockAdjust_Table  where ID=@ID"
    ...
    '''' </summary>
    '''' <remarks></remarks>
    '  Private Const SQL_StockAdjust_CheckIsModify As String = "select count(*) from T24401_StockAdjust_List  where ID=@ID and WL_ID=@WL_ID"
#End Region
...
#Region "单一张单内容查询"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_GetById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_StockAdjust_GetByid, "@ID", sId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_SelListById(ByVal sId As String) As DtReturnMsg
        Return PClass.PClass.SqlStrToDt(SQL_StockAdjust_SelListByid, "@ID", sId)
    End Function
#End Region
...
#Region "添加修改删除"
    ...
    '''' <returns></returns>
    '''' <remarks></remarks>
    'Public Shared Function StockAdjust_CheckIsModify(ByVal ID As String, ByVal WL_ID As Integer) As Boolean
    '    Dim P As New Dictionary(Of String, Object)
    '    P.Add("ID", ID)
    '    P.Add("WL_ID", WL_ID)
    '    Dim R As MsgReturn = SqlStrToOneStr(SQL_StockAdjust_CheckIsModify, P)
    '    If R.IsOk Then
    '        If Val(R.Msg) = 0 Then
    '            Return True
    '        Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_CheckID(ByVal ID As String) As Boolean
        Dim R As MsgReturn = SqlStrToOneStr(SQL_StockAdjust_CheckID, "@ID", ID)
        If R.IsOk Then
            If Val(R.Msg) = 0 Then
                Return False
            Else
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_Add(ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim StockAdjustId As String = dtTable.Rows(0)("ID")
        Dim IsInsert As Boolean = False
        paraMap.Add("ID", StockAdjustId)
        Try
            sqlMap.Add("Table", SQL_StockAdjust_GetByid)
            sqlMap.Add("List", SQL_StockAdjust_GetListByid)
            ...
            If msg.DtList("Table").Rows.Count = 0 Then
                ...
                Dim TmSQL As String = "insert into Bill_Barcode (No_ID,NO_Type)values('" & StockAdjustId & "'," & BillType.StockAdjust & ")"
                DtToUpDate(msg, TmSQL)
                R.Msg = "" & StockAdjust_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加成功!"
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & StockAdjust_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经存在!请双击编号文本框,获取新编号!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & StockAdjust_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]添加错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_Save(ByVal LID As String, ByVal dtTable As DataTable, ByVal dtList As DataTable) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        Dim StockAdjustId As String = dtTable.Rows(0)("ID")
        If LID <> StockAdjustId AndAlso StockAdjust_CheckID(StockAdjustId) Then
            R.IsOk = False
            R.Msg = "" & StockAdjust_DB_NAME & "[" & StockAdjustId & "]已经存在!请双击编号文本框,获取新编号!"
            Return R
        End If
        ...
        If IsNull(dtTable.Rows(0)("Produce_ID"), "") <> "" Then
            ...
            If Rx.IsOk = False Then
                R.IsOk = False
                R.Msg = StockAdjust_DB_NAME & "[" & StockAdjustId & "]添加失败!" & vbCrLf & Rx.Msg
                Return R
            End If
        End If
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Audited Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & StockAdjust_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被审核不能再修改!"
                    Return R
                End If
                If msg.DtList("Table").Rows(0)("State") = Enum_State.Invoid Then
                    ...
                    msg.Cnn.Dispose()
                    R.IsOk = False
                    R.Msg = "" & StockAdjust_DB_NAME & "[" & dtTable.Rows(0)("ID") & "]已经被作废不能再修改!"
                    Return R
                End If
                DvToDt(dtList, msg.DtList("List"), New List(Of String))
                DvUpdateToDt(dtTable, msg.DtList("Table"), New List(Of String))
                Dim TmSQL As String = "Update Bill_Barcode set No_ID='" & StockAdjustId & "' where No_ID='" & LID & "' and NO_Type=" & BillType.StockAdjust
                If LID <> StockAdjustId Then
                    DtToUpDate(msg, "Update T24400_StockAdjust_Table set id='" & StockAdjustId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
                    dtTable.Rows(0)("ID") = StockAdjustId
                    R.Msg = "" & StockAdjust_DB_NAME & "[" & LID & "]已经修改为[" & StockAdjustId & "]!"
                Else
                    DtToUpDate(msg)
                    R.Msg = "" & StockAdjust_DB_NAME & "[" & LID & "]修改成功!"
                End If
                R.IsOk = True
            Else
                R.IsOk = False
                R.Msg = "" & StockAdjust_DB_NAME & "[" & LID & "]不存在"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & StockAdjust_DB_NAME & "[" & LID & "]修改错误"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' 删除出库单
    ''' </summary>
    ''' <param name="StockAdjustId"></param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_Del(ByVal StockAdjustId As String)
        Return RunSQL(SQL_StockAdjust_DelByid, "@ID", StockAdjustId)
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_InValid(ByVal _ID As String, ByVal IsFei As Boolean) As MsgReturn
        Dim msg As New DtListReturnMsg
        Dim R As New MsgReturn
        Dim sqlMap As New Dictionary(Of String, String)
        Dim paraMap As New Dictionary(Of String, Object)
        ...
        paraMap.Add("ID", _ID)
        Try
            sqlMap.Add("Table", SQL_StockAdjust_GetStateByid)
            msg = SqlStrToDt(sqlMap, paraMap)
            If msg.DtList("Table").Rows.Count = 1 Then
                If msg.DtList("Table").Rows(0)("State") <> Enum_State.Audited Then
                    '检查加工单状态
                    If IsFei = False AndAlso IsNull(msg.DtList("Table").Rows(0)("Produce_ID"), "") <> "" Then
                        ...
                        If Rx.IsOk = False Then
                            R.IsOk = False
                            R.Msg = StockAdjust_DB_NAME & "[" & _ID & "]" & OStr & "失败!" & vbCrLf & Rx.Msg.Replace("不能被引用", "")
                            Return R
                        End If
                    End If
                    If msg.DtList("Table").Rows(0)("State") <> State Then
                        ...
                        DtToUpDate(msg)
                        R.IsOk = True
                        R.Msg = "" & StockAdjust_DB_NAME & "[" & _ID & "]" & OStr & "成功!"
                    Else
                        R.IsOk = False
                        R.Msg = "" & StockAdjust_DB_NAME & "[" & _ID & "]已经被" & OStr & "!不能再" & OStr & "!"
                    End If
                Else
                    R.IsOk = False
                    R.Msg = "" & StockAdjust_DB_NAME & "[" & _ID & "]已经被审核,不能" & OStr & "!"
                End If
            Else
                R.IsOk = False
                R.Msg = "" & StockAdjust_DB_NAME & "[" & _ID & "]不存在,不能" & OStr & "!"
            End If
            Return R
        Catch ex As Exception
            R.IsOk = False
            R.Msg = "" & StockAdjust_DB_NAME & "[" & _ID & "]" & OStr & "错误!"
            DebugToLog(ex)
            Return R
        Finally
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        Dim paraMap As New Dictionary(Of String, Object)
        paraMap.Add("ID", _ID)
        paraMap.Add("IsAudited", IsAudited)
        paraMap.Add("State_User", User_Display)

        Dim R As MsgReturn = SqlStrToOneStr("P24400_StockAdjust_Audited", paraMap, True)
        If R.IsOk Then
...
#Region "计算选择日期的库存"
    ...
    ''' <remarks></remarks>
    Public Shared Function StockAdjust_GetWLQty(ByVal IdList As List(Of Long), ByVal sDate As Date) As DtReturnMsg
        Dim sql As New StringBuilder()
        sDate = sDate.Date.AddDays(1).AddSeconds(-1)
        sql.AppendLine("select sum(Qty*StoreInOrOut) as WL_Qty,WL_ID from T24003_Store_Detail where sDate<=@sDate and WL_ID in ( ")
        sql.Append(IdList(0))
        For i As Integer = 1 To IdList.Count - 1
            sql.Append(",")
            sql.Append(IdList(i))
        Next
        sql.Append(") group by WL_ID")
        Return SqlStrToDt(sql.ToString, "sDate", sDate)
    End Function
#End Region
