...
Public Class F24301_StoreOut_Msg
    Dim LId As String = ""
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        ComFun.DelBillNewID(BillType.StoreOut_Bag, Bill_ID)
    End Sub
    ...
    Sub GetNewID()
        ...
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.StoreOut_Bag, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreOut_DelByid As String = "Delete from T24300_StoreOut_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreOut_GetByid As String = "select top 1 * from T24300_StoreOut_Table  where ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreOut_SelByid As String = "select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T24300_StoreOut_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreOut_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T24300_StoreOut_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreOut_GetListByid As String = "select * from T24301_StoreOut_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_StoreOut_SelListByid As String = "select L.*,L.WL_Unit_LL as WL_Unit_LL2,WL.WL_No,WL.WL_Name,WL.WL_Spec from (select * from T24301_StoreOut_List  where ID=@ID)L left join T24001_PolyBag WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreOut_CheckID As String = "select count(*) from T24300_StoreOut_Table  where ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_StoreOut_CheckIsModify As String = "select count(*) from T24301_StoreOut_List  where ID=@ID and Line=@Line"
#End Region
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> StoreOutId Then
                    DtToUpDate(msg, "Update T24300_StoreOut_Table set id='" & StoreOutId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function StoreOut_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P24300_StoreOut_Audited", paraMap, True)
        If R.IsOk Then
