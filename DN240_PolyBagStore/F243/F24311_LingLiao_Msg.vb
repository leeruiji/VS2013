...
Public Class F24311_LingLiao_Msg
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_LingLiao_DelByid As String = "Delete from T24310_LingLiao_Table  where ID=@ID and State=0 "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_LingLiao_GetByid As String = "select top 1 * from T24310_LingLiao_Table  where ID=@ID "

    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_LingLiao_GetByid_WithName As String = "select top 1T. * ,C.Client_Name,(Select top 1 GY_No + '.' + GY_Name from  T10004_GY where T10004_GY.ID=T.GY_ID1) as GY_Name1,(Select top 1 GY_No + '.' + GY_Name from  T10004_GY where T10004_GY.ID=T.GY_ID2) as GY_Name2 from T24310_LingLiao_Table T Left join T10110_Client C on T.Client_ID=C.ID  where T.ID=@ID "
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_LingLiao_SelByid As String = " select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name,BZ.BZ_Name ,C.Client_Name ,C.Client_No" & _
                                                   " from T24310_LingLiao_Table S " & _
                                                    ...
                                                    "left join T10002_BZ BZ on BZ.id=S.BZ_ID " & _
                                                     "where S.ID=@ID "
    'select top 1 S.*,E.Employee_Name as OperatorName ,W.Dept_Name as Dept_Name from T24310_LingLiao_Table S  left join T15001_Employee E on S.Operator=E.ID left join T15000_Department W on S.Dept_No=W.Dept_No where S.ID=@ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_LingLiao_GetStateByid As String = "select top 1 ID,State,State_User,Produce_ID from T24310_LingLiao_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_LingLiao_GetListByid As String = "select * from T24311_LingLiao_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_LingLiao_SelListByid As String = "select L.*, WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Cost from (select * from T24311_LingLiao_List  where ID=@ID)L left join T24001_PolyBag WL on WL.id=l.WL_ID order by L.ID ,L.Line "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_LingLiao_CheckID As String = "select count(*) from T24310_LingLiao_Table  where ID=@ID"
    ...
    '  Private Const SQL_Produce_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Unit,WL.WL_Store_ID from (select * from T30011_Produce_List  where ID=@ID)L left join T24001_PolyBag WL on WL.id=l.WL_id  where isnull(WL_Ignore,0)=0 order by WL.WL_Type_ID,WL.WL_Name"

    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_LingLiao_CheckIsModify As String = "select count(*) from T24311_LingLiao_List  where ID=@ID and Line=@Line"
    ...
    Public Const SQL_RBPF_PF_GeList_WithName = "select  P.ID,p.DyingStep,P.WL_ID,P.Qty as sPercent,W.WL_No,W.WL_Name,W.WL_Unit,W.WL_Unit_LL,W.WL_Spec,W.WL_Cost as Cost,W.WL_Cost as Price,W.IsZJ,IsPageTwo,GYSetID  as Step,W.WL_Percent   from T10013_RB_PFList P " & _
             "left join T24001_PolyBag W on P.WL_ID=W.ID where IsGY=0 and  BZC_ID=@BZC_ID and P.ID =(select top 1 PF_ID from ( " & _
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> LingLiaoId Then
                    DtToUpDate(msg, "Update T24310_LingLiao_Table set id='" & LingLiaoId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function LingLiao_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P24310_LingLiao_Audited", paraMap, True)
        If R.IsOk Then
...
#End Region
    ...
    Public Shared Function LingLiao_CountJiaLiao(ByVal _ID As String) As Integer
        Dim R As MsgReturn = SqlStrToOneStr("select Count(*) from T24310_LingLiao_Table where MainBill=@ID and IsJiaLiao=1 ", "ID", _ID)
        Return Val(R.Msg)
    End Function
End Class