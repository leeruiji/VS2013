...
Public Class F24101_Purchase_Msg
    ...
    Private Sub Form_Me_Closing(ByVal sender As Object, ByRef e As System.ComponentModel.CancelEventArgs) Handles Me.Me_Closing
        ComFun.DelBillNewID(BillType.Purchase_Bag, Bill_ID)
    End Sub
...
#Region "表单信息"
    ...
    Protected Function GetForm() As DataTable
        ...
        For i As Integer = 1 To Fg1.Rows.Count - 1
            dr = dtList.NewRow
            dr("ID") = ID
            dr("Store_ID") = IsNothing(Fg1.Item(i, "Store_ID"), 0)
    ...
    Sub GetNewID()
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.Purchase_Bag, DTP_sDate.Value, Bill_ID)
                If R.IsOk Then
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_DelByid As String = "Delete from T24100_Puchase_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetByid As String = "select top 1 * from T24100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_SelByid As String = "select T.*,C.Supplier_Name From (Select top 1 * from T24100_Puchase_Table where ID=@ID)  T left join T24004_Supplier C on C.ID=T.Supplier_ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetStateByid As String = "select top 1 ID,State,State_User from T24100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetListByid As String = "select * from T24101_Puchase_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_Purchase_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Unit from (select * from T24101_Puchase_List  where ID=@ID)L left join T24001_PolyBag WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_CheckID As String = "select count(*) from T24100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_CheckIsModify As String = "select count(*) from T24101_Puchase_List  where @ID=ID and Line=@Line"
#End Region
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> PurchaseId Then
                    DtToUpDate(msg, "Update T24100_Puchase_Table set id='" & PurchaseId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function Purchase_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P24100_Puchase_Audited", paraMap, True)
        If R.IsOk Then
