...
Public Class F22110_PurchaseReturn
    ...
    Private Sub Form_Me_Load() Handles Me.Me_Load
        ...
        CB_ConditionName1.ComboBox.DataSource = Dao.PurchaseReturn_GetConditionNames()
        ...
        CB_ConditionName2.ComboBox.DataSource = Dao.PurchaseReturn_GetConditionNames()
        CB_Condition3.ComboBox.DisplayMember = "Field"
        CB_Condition3.ComboBox.ValueMember = "DB_Field"

        CB_Condition3.ComboBox.DataSource = Dao.PurchaseReturn_GetCondition3
        Me_Refresh()
        isLoaded = True
    End Sub

    Protected Sub Me_Refresh()
        Dim msg As DtReturnMsg = Dao.PurchaseReturn_GetByOption(GetFindOtions)
        If msg.IsOk Then
...
#Region "控件事件"
    ...
    Private Sub Cmd_Add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Add.Click
        Dim F As New F22111_PurchaseReturn_Msg("")
        With F
            .Mode = Mode_Enum.Add
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
    ...
    Protected Sub ShowModify()
        If FG1.RowSel < 0 Then
            ShowErrMsg("请选择一个要修改的" & Dao.PurchaseReturn_DB_NAME)
            Exit Sub
        End If
        Dim F As New F22111_PurchaseReturn_Msg(FG1.SelectItem("ID"))
        With F
    ...
    Private Sub Cmd_Del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_Del.Click
        If FG1.RowSel < 0 Then
            ...
            Exit Sub
        End If
        ShowConfirm("是否删除" & Dao.PurchaseReturn_DB_NAME & "[" & FG1.SelectItem("ID") & "]?", AddressOf DelPurchaseReturn)
    End Sub
    ...
    Protected Sub DelPurchaseReturn()
        Dim msg As MsgReturn = Dao.PurchaseReturn_Del(FG1.SelectItem("ID"))
        If msg.IsOk Then
            Me_Refresh()
        Else
            ShowErrMsg(msg.Msg)
...
#Region "搜索工具栏"
    ...
    Private Sub CB_ConditionName2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CB_ConditionName2.SelectedIndexChanged
        ...
        Dim msg As DtReturnMsg = OptionClass.OptionClass_GetConditionValues("(" & Dao.SQL_PurchaseReturn_Get_WithName & "where 1=1 " & OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap) & ")Con", CB_ConditionName2.ComboBox.SelectedValue, "", paraMap)
        If msg.IsOk Then
...
#Region "审核状态"
    ...
    Protected Sub ShenHe()
        ...
        For Each _ID As String In idList
            msg = Dao.PurchaseReturn_Audited(_ID, True)
            If msg.IsOk Then
                okCount = okCount + 1
...
#Region "显示明细"
    ...
    Private Sub Mx_ReFresh()
        ...
        If isRowChang = False Then
            ...
            Exit Sub
        End If
        Dim msg As DtReturnMsg = Dao.PurchaseReturn_SelListById(IsNull(FG1.SelectItem("ID"), ""))
        If msg.IsOk Then
            Fg2.DtToFG(msg.Dt)
            SumFG2.ReSum()
        Else
...
#End Region
    ...
    Private Sub DP_End_ValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DP_End.ValueChanged
...
Partial Friend Class Dao
    ...
    '===================采购单信息==============
    Public Const SQL_PurchaseReturn_Get_WithName = "select  P.*,Supplier_No,S.Supplier_Name from T22110_PurchaseReturn_Table P left join T22004_Supplier S on  P.Supplier_ID=S.ID "

    Public Const SQL_PurchaseReturn_OrderBy As String = " order by sDate "
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PurchaseReturn_GetConditionNames() As List(Of FindOption)
        ...
        fo.SQL = "exists(select * from T22001_kitchen TW,T22111_PurchaseReturn_List TL where %WL_Name% and TL.wl_id=TW.ID and P.id=TL.id)"
        fo.Sign = "%WL_Name%"
        foList.Add(fo)
        ...
        fo.DB_Field = " "
        fo.Field_Operator = Enum_Operator.Operator_Like_Both
        fo.SQL = "(P.ID %Like% or Supplier_Name  %Like% or SumQty %Like% or SumAmount %Like% or Remark %Like%)"
        fo.Sign = "%Like%"
        foList.Add(fo)
        Return foList
    End Function
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PurchaseReturn_GetCondition3() As List(Of FindOption)
        Dim foList As New List(Of FindOption)
        Dim fo As New FindOption
        fo.Field = "全部"
        fo.DB_Field = ""
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Public Shared Function PurchaseReturn_GetByOption(ByVal oList As OptionList) As DtReturnMsg
        Dim paraMap As New Dictionary(Of String, Object)
        Dim sqlBuider As New StringBuilder
        sqlBuider.Append(SQL_PurchaseReturn_Get_WithName)
        sqlBuider.Append("  WHERE 1=1  ")
        sqlBuider.Append(BaseClass.OptionClass.OptionClass_BuidSqlConditon(oList.FoList, paraMap))
        sqlBuider.Append(SQL_PurchaseReturn_OrderBy)
