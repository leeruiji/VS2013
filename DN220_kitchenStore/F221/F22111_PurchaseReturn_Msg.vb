...
Public Class F22111_PurchaseReturn_Msg
    Dim LId As String = ""
    ...
    Sub GetNewID()
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.PurchaseReturn_KT, DTP_sDate.Value, Bill_Id)
                If R.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        If Me.Mode = Mode_Enum.Add Then
            Exit Sub
        End If
        Dim R As New R22111_PurchaseReturn_List
...
#Region "引用采购单"
    Private Sub Cmd_ChoosePurchase_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Cmd_ChoosePurchase.Click
        If (CB_Supplier.IDValue) = 0 Then
            ...
            Exit Sub
        End If
        Dim F As New F22102_Purchase_Sel(CB_Supplier.IDValue)
        With F
            .Mode = Mode_Enum.Modify
            .P_F_RS_ID = ""
            .P_F_RS_ID2 = ""
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_DelByid As String = "Delete from T22110_PurchaseReturn_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_GetByid As String = "select top 1 * from T22110_PurchaseReturn_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_SelByid As String = "select T.*,C.Supplier_Name From (Select top 1 * from T22110_PurchaseReturn_Table where ID=@ID)  T left join T22004_Supplier C on C.ID=T.Supplier_ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_GetStateByid As String = "select top 1 ID,State,State_User from T22110_PurchaseReturn_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_GetListByid As String = "select * from T22111_PurchaseReturn_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_PurchaseReturn_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Unit from (select * from T22111_PurchaseReturn_List  where ID=@ID)L left join T22001_kitchen WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_CheckID As String = "select count(*) from T22110_PurchaseReturn_Table  where ID=@ID"

    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_PurchaseReturn_CheckIsModify As String = "select count(*) from T22111_PurchaseReturn_List  where ID=@ID and Line=@Line"
#End Region
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> PurchaseReturnId Then
                    DtToUpDate(msg, "Update T22110_PurchaseReturn_Table set id='" & PurchaseReturnId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function PurchaseReturn_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P22110_PurchaseReturn_Audited", paraMap, True)
        If R.IsOk Then
    ...
    Public Shared Function PurchaseReturn_CheckCanReturn(ByVal dtList As DataTable, Optional ByVal isAdd As Boolean = True) As MsgReturn
        ...
        sqlBuider.Append(" select L.ID,Line,L.WL_ID,WL.WL_Name, L.Qty-isnull(SumRLQty,0) as CanReturn    from (select P.ID,Line,Qty,WL_ID from T22101_Puchase_List PL left join T22100_Puchase_Table P on PL.ID=P.ID ")
        ...
        Next
        sqlBuider.Append("'') ")
        sqlBuider.Append("  and  P.State=1 )L left join (Select Purchase_ID, WL_ID, sum(isnull(Qty,0)) as SumRLQty from  T22111_PurchaseReturn_List ")
        '如果是编辑状态,则不计算本退货单数量
        If isAdd = False Then
        ...
        sqlBuider.Append(" left join T22001_kitchen WL on WL.id=l.WL_ID order by L.ID")
