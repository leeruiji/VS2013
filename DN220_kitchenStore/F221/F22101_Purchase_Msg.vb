...
Public Class F22101_Purchase_Msg
    Dim LId As String = ""
    ...
    Sub GetNewID()
        If TB_ID.ReadOnly = False Then
            If DTP_sDate.Value <> Bill_Id_Date Then
                Dim R As RetrunNewIdMsg = ComFun.GetBillNewID(BillType.Purchase_KT, DTP_sDate.Value, Bill_Id)
                If R.IsOk Then
...
#Region "报表事件"
    ...
    Protected Sub Print(ByVal DoOperator As OperatorType)
        If Me.Mode = Mode_Enum.Add Then
            Exit Sub
        End If
        Dim R As New R22101_Purchase_List
...
#Region "常量"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_DelByid As String = "Delete from T22100_Puchase_Table  where ID=@ID and State=0"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetByid As String = "select top 1 * from T22100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单头
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_SelByid As String = "select T.*,C.Supplier_Name From (Select top 1 * from T22100_Puchase_Table where ID=@ID)  T left join T22004_Supplier C on C.ID=T.Supplier_ID"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetStateByid As String = "select top 1 ID,State,State_User from T22100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 获取单体
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_GetListByid As String = "select * from T22101_Puchase_List  where ID=@ID "
    ''' <summary>
    ''' 获取单体,用于显示
    ''' </summary>
    ''' <remarks></remarks>0
    Private Const SQL_Purchase_SelListByid As String = "select L.*,WL.WL_No,WL.WL_Name,WL.WL_Spec,WL.WL_Unit from (select * from T22101_Puchase_List  where ID=@ID)L left join T22001_kitchen WL on WL.id=l.WL_ID  "
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_CheckID As String = "select count(*) from T22100_Puchase_Table  where ID=@ID"
    ''' <summary>
    ''' 检查ID的重复性
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Purchase_CheckIsModify As String = "select count(*) from T22101_Puchase_List  where @ID=ID and Line=@Line"
    ''' <summary>
    ''' 获取内容
    ''' </summary>
    ''' <remarks></remarks>
    Private Const SQL_Kitchen_GetByNo As String = "select Top 1 Id,WL_No,WL_Name,WL_Unit,WL_Spec,WL_Store_ID,WL_Cost,WL_Type_ID,WL_Price from T22001_kitchen where WL_No=@WL_No and isnull(WL_Disable,0)<>1"
...
#Region "添加修改删除"
        ...
        Try
            ...
            If msg.DtList("Table").Rows.Count = 1 Then
                ...
                If LID <> PurchaseId Then
                    DtToUpDate(msg, "Update T22100_Puchase_Table set id='" & PurchaseId & "' where id='" & LID & "'" & vbCrLf & TmSQL)
    ...
    Public Shared Function Purchase_Audited(ByVal _ID As String, ByVal IsAudited As Boolean) As MsgReturn
        ...
        Dim R As MsgReturn = SqlStrToOneStr("P22100_Puchase_Audited", paraMap, True)
        If R.IsOk Then
